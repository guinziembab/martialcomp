...
def generate_qr_code_data(entity_type, entity_id, action='register', data=None):
    ...
    Args:
        ...
        action: Action à effectuer (register, view, etc.)
        data: Données additionnelles à inclure
        
    Returns:
        Un dictionnaire avec les données à encoder
    """
    payload = {
        'type': entity_type,
        'id': entity_id,
...
def _get_organization_by_type_and_id(entity_type, entity_id):
    ...
    try:
        if entity_type == 'club':
            ...
            from competitions.models import CoachProfile
...
def create_qr_code_image(url, size=10, border=4):
    """
    Crée une image QR code à partir d'une URL.
...
def create_qr_code_with_logo(url, entity, size=10, border=4):
    ...
    qr = qrcode.QRCode(
        version=1,
        error_correction=qrcode.constants.ERROR_CORRECTION_H,  # Haute correction pour logo
        box_size=size,
        border=border,
    )
    qr.add_data(url)
...
def generate_qr_code_for_entity(entity, entity_type, action='register', data=None, use_subdomain=True, with_logo=False):
    ...
    Args:
        ...
        entity_type: Type d'entité ('club', 'federation', 'coach')
        action: Action à effectuer lors du scan
...
def generate_qr_code_for_model(model_instance, field_name='qr_code', action='register', use_subdomain=True):
    ...
    Args:
        ...
        field_name: Nom du champ ImageField où stocker le QR code
        action: Action à effectuer lors du scan
        use_subdomain: Utiliser le sous-domaine de l'organisation
    ...
    # Également sauvegarder l'image sur le disque
