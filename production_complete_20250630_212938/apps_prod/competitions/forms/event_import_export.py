...
class EventImportForm(forms.Form):
    ...
    organization = forms.ModelChoiceField(
        ...
        help_text=_("Tous les événements importés seront associés à cette organisation."),
        required=True
    )
    ...
    def clean_csv_file(self):
        ...
        try:
            ...
            # Vérifier que les dates peuvent être analysées
            rows = []
            line_number = 1
            for row in reader:
                line_number += 1
                ...
                try:
                    ...
                    raise forms.ValidationError(
                        _("Erreur à la ligne %(line)s: format de date invalide"),
                        params={'line': line_number}
                    )
                
                rows.append(row_dict)
            
            # S'assurer qu'il y a au moins un événement à importer
            if not rows:
            ...
            try:
                ...
                if create_reminders:
                    ...
                    reminder = EventReminder(
                        ...
                        message=_("Rappel: l'événement {title} aura lieu demain à {location}.").format(
                            title=event.title,
...
class EventExportForm(forms.Form):
    ...
    start_date = forms.DateField(
        ...
        help_text=_("Exporter les événements à partir de cette date. Laissez vide pour inclure tous les événements passés.")
    )
    ...
    end_date = forms.DateField(
        ...
        required=False,
        widget=forms.DateInput(attrs={'type': 'date'}),
        help_text=_("Exporter les événements jusqu'à cette date. Laissez vide pour inclure tous les événements futurs.")
    )
    ...
    def get_events_queryset(self):
        """
        Renvoie le queryset des événements à exporter selon les filtres appliqués.
        """
