...
class ActionCombatForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if type_action == 'point' and valeur <= 0:
            self.add_error('valeur', _("La valeur d'un point doit être positive."))
        
        if type_action == 'penalite' and valeur >= 0:
            self.add_error('valeur', _("La valeur d'une pénalité doit être négative."))
...
class AttributionPointForm(forms.Form):
    ...
    """
    POINT_CHOICES = [
        (0.25, _('¼ point - Phân Tu Điểm')),
        (0.5, _('½ point - Nửa Điểm')),
        (1.0, _('1 point - Một Điểm')),
        (1.5, _('1½ points - Một Điểm Dưới')),
        (2.0, _('2 points - Hai Điểm')),
    ]
    
    PENALITE_CHOICES = [
        (0, _('Avertissement - Cảnh Cáo')),
        (-0.5, _('Retrait ½ point - Kém Điểm')),
        (-1.0, _('Retrait 1 point - Phạt')),
        (-2.0, _('Disqualification - Phạt Hai/Loại')),
    ]
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        from competitions.models.combat import Combat
