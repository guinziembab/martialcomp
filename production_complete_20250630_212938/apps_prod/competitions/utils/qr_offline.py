...
class OfflineProfileGenerator:
    ...
    def generate_offline_profile(practitioner):
        ...
        # Utiliser une compression JSON plus efficace (pas d'espaces ni de retours à la ligne)
        token_json = json.dumps(profile_data, separators=(',', ':'))
        
        # Encoder en Base64
        token = base64.urlsafe_b64encode(token_json.encode()).decode()
        
        # Préparer les données à retourner avec les noms de champs complets pour l'API
        full_profile_data = {
...
class OfflineQRStorage:
    ...
    Permet de stocker les scans en local jusqu'à la synchronisation avec le serveur.
    """
    
    @staticmethod
    def store_offline_scan(scan_data):
        """
        Génère les données à stocker pour un scan hors-ligne.
        À implémenter côté client (mobile).
    ...
    def merge_offline_scans(offline_scans):
        ...
        Args:
            offline_scans: Liste des scans hors-ligne à synchroniser
        ...
        for scan in offline_scans:
            try:
                # Vérifier si le scan a déjà été synchronisé
                ...
                # Vérifier si un scan similaire existe déjà (vérification plus précise)
