...
def require_organization_access(view_func):
    """
    Décorateur qui vérifie que l'utilisateur a accès à l'objet demandé
    basé sur son organisation.
    
    À utiliser sur les vues qui accèdent à un objet spécifique avec un ID.
    
    Exemple:
    @require_organization_access
    def my_view(request, object_id):
        obj = get_object_or_404(MyModel, pk=object_id)
        # Le décorateur a déjà vérifié que l'utilisateur a accès à cet objet
        ...
    """
    @wraps(view_func)
    def _wrapped_view(request, *args, **kwargs):
        ...
        # Récupérer l'ID de l'objet (assumé être le premier argument position ou un kwarg)
        object_id = None
        if args:
            object_id = args[0]
        else:
        ...
        try:
            ...
            if hasattr(obj, 'is_accessible_by'):
                if not obj.is_accessible_by(request.user):
                    ...
                    raise PermissionDenied(_("Vous n'avez pas accès à cette ressource"))
...
def filter_queryset_for_user(queryset, user):
    """
    Filtre un queryset pour ne montrer que les objets accessibles à l'utilisateur.
    ...
    # C'est plus sûr que de retourner tous les objets sans filtrage
    return queryset.none()


def organization_isolated_view(model):
    """
    Décorateur de classe qui ajoute l'isolation organisationnelle à une vue.
    ...
    def decorator(view_class):
        ...
        # Attacher la nouvelle méthode à la classe
...
class OrganizationIsolationMiddleware:
    """
    Middleware qui vérifie l'isolation organisationnelle pour toutes les requêtes.
    
    Ce middleware surveille les fuites potentielles de données et les tentatives d'accès
    à des ressources d'autres organisations.
    """
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        """
        Vérifie si la vue accède potentiellement à des données d'autres organisations.
        ...
        if any(pattern in view_name.lower() for pattern in ['list', 'index', 'all', 'search']):
            # Log pour surveillance, à transformer en blocage plus tard si nécessaire
...
def mark_view_as_organization_isolated(view_func):
    """
    Marque une vue comme déjà vérifiée pour l'isolation organisationnelle.
