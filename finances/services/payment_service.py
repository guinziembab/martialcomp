...
class PaymentService:
        ...
        if data.get('invoice_id'):
            try:
                ...
                if invoice.status not in ['unpaid', 'sent']:
                    raise ValidationError(_('Cette facture ne peut pas être payée'))
    ...
    def process_payment(payment_attempt_id, processing_data, user):
        ...
        Returns:
            PaymentAttempt: L'objet tentative de paiement mis à jour
            
        Raises:
            ValidationError: Si le paiement ne peut pas être traité
        """
        try:
        ...
        if payment_attempt.status != 'pending':
            raise ValidationError(_('Cette tentative de paiement a déjà été traitée'))
        ...
        with db_transaction.atomic():
            # Mettre à jour la tentative de paiement
            ...
            # Si le paiement est réussi et qu'il est lié à une facture
    ...
    def cancel_payment(payment_attempt_id, reason, user):
        ...
        Raises:
            ValidationError: Si le paiement ne peut pas être annulé
        """
        try:
            payment_attempt = PaymentAttempt.objects.get(pk=payment_attempt_id)
        except PaymentAttempt.DoesNotExist:
        ...
        if payment_attempt.status != 'pending':
            raise ValidationError(_('Seules les tentatives de paiement en attente peuvent être annulées'))
