...
def has_finance_permission(permission_codename):
    ...
    Args:
        permission_codename (str): Le nom de code de la permission à vérifier
...
def can_validate_transaction(user, transaction):
    ...
    Args:
        user (User): L'utilisateur à vérifier
        transaction (Transaction): La transaction à valider
...
def can_view_invoice(user, invoice):
    ...
    Args:
        user (User): L'utilisateur à vérifier
        invoice (Invoice): La facture à vérifier
    ...
    try:
        ...
        if entity_type == 'federation':
            ...
            if is_admin or is_owner:
                ...
                # Pour les permissions plus sensibles, vérifier le rôle d'administrateur
                if hasattr(entity, 'administrators'):
            ...
            if is_manager or is_owner:
                ...
                # Pour les permissions plus sensibles, vérifier le rôle de manager
                if hasattr(entity, 'managers'):
...
def transaction_action_required(action):
    ...
    Args:
        action (str): L'action à vérifier (validate, reject, cancel)
