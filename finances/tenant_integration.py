...
from multitenant.utils import get_tenant_from_request
...
class TenantFinanceManager:
    """
    Gestionnaire des finances spécifiques à un tenant.
    """
    ...
    def initialize_accounts(self) -> bool:
        ...
        try:
            ...
            if Account.objects.filter(default_account=True).exists():
                logger.info(f"Les comptes pour le tenant {self.tenant.name} existent déjà")
                return True
