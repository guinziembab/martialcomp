"""
Modèles de finances spécifiques à l'architecture multi-tenant.
"""
...
from finances.models.accounts import Account
from finances.models.invoices import Invoice


class TenantFinancialSetting(TenantAwareModel):
    """
    Paramètres financiers spécifiques à un tenant.
    """
    ...
    invoice_footer = models.TextField(
        ...
        blank=True,
        help_text=_("Texte à inclure en bas des factures")
    )
    ...
    def get_tax_rate(self) -> Decimal:
        ...
        if hasattr(self.tenant, 'country') and self.tenant.country:
            # Logique spécifique par pays pourrait être implémentée ici
            pass
...
class TenantBillingCycle(TenantAwareModel):
    ...
    invoice_date = models.DateField(
        ...
        null=True,
        blank=True,
        help_text=_("Date à laquelle les factures seront générées")
    )
...
class TenantInvoiceTemplate(TenantAwareModel):
    ...
    header_html = models.TextField(
        _("HTML d'en-tête"),
        blank=True,
        help_text=_("Contenu HTML personnalisé pour l'en-tête de la facture")
    )
    ...
    logo = models.ImageField(
        ...
        null=True,
        blank=True,
        help_text=_("Logo à afficher sur les factures")
    )
