...
class FinancialDashboardView(LoginRequiredMixin, TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        # Vérification personnalisée pour autoriser les rôles spécifiques
        ...
        if hasattr(request.user, 'userprofile'):
            # Autoriser les rôles administratifs
    ...
    def get_context_data(self, **kwargs):
        ...
        if period == '7days':
            ...
            try:
                ...
                # En cas d'erreur, revenir à la période par défaut
                pass
        ...
        if not permissions.get('can_view_all_transactions', False):
            # Filtrer selon l'utilisateur ou d'autres critères...
            transactions = transactions.filter(created_by=self.request.user)
    ...
    def _get_time_series_data(self, start_date, end_date):
        ...
        # Requête et formatage des données selon le groupement
        if grouping == 'day':
            # Agrégation par jour
    ...
    def _get_category_data(self, start_date, end_date):
        ...
        # Limiter à un nombre raisonnable de catégories pour le graphique
...
class FinancialReportView(LoginRequiredMixin, TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # Vérifier les rôles utilisateur
        if hasattr(request.user, 'userprofile'):
            # Autoriser les rôles administratifs pour les rapports
            allowed_roles = ['federation_admin', 'club_admin', 'club_owner', 'club_manager', 'coach']
            if request.user.userprofile.role in allowed_roles:
                return super().dispatch(request, *args, **kwargs)
    ...
    def get_context_data(self, **kwargs):
        ...
        # Récupérer les permissions de l'utilisateur
        permissions = get_financial_permissions(self.request.user)
        # Forcer la permission de voir les rapports car nous avons déjà vérifié l'accès
    ...
    def _generate_balance_sheet(self, end_date):
        """Génère un bilan financier à une date donnée."""
        # Comptes financiers
    ...
    def _generate_cash_flow_report(self, start_date, end_date):
        ...
        cash_inflows = Transaction.objects.filter(
            date__gte=start_date,
            date__lte=end_date,
            account__account_type__in=['cash', 'bank'],
            type='income'
        )
    ...
    def _generate_invoice_report(self, start_date, end_date):
        ...
        # Âge moyen des factures impayées
        today = timezone.now().date()
        ...
        # Répartition des factures par tranche d'âge
        age_ranges = {
