...
from apps.competitions.models import ScoringCriterion
from apps.shop.forms.dashboard import (
...
def club_order_detail(request, order_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, _("Statut de la commande mis Ã  jour."))
...
def club_order_update_status(request, order_id):
    """
    Mise Ã  jour du statut d'une commande via AJAX ou formulaire POST.
    """
    club = request.club
    order = get_object_or_404(Order, id=order_id, club=club)
    ...
    if request.method == 'POST':
        ...
        if new_status in ['processing', 'shipped', 'delivered', 'cancelled']:
            ...
            order.save()
            messages.success(request, _("Statut de la commande mis Ã  jour avec succès."))
        else:
...
def club_order_invoice(request, order_id):
    ...
    for item in order.items.all():
        ...
        p.drawString(5 * inch, y_position, f"{item.price} â‚¬")
        p.drawString(6.5 * inch, y_position, f"{item.total} â‚¬")
        y_position -= 0.3 * inch
    
    # Total
    p.setFont("Helvetica-Bold", 12)
    p.drawString(5 * inch, y_position - 0.5 * inch, "Total:")
    p.drawString(6.5 * inch, y_position - 0.5 * inch, f"{order.total} â‚¬")
...
@club_required
@require_POST
def club_create_standard_categories(request):
    """Action pour créer/compléter les catégories standard."""
    update = request.POST.get('update') in ('1', 'true', 'True')
    try:
        call_command('create_standard_categories', update=update, verbosity=0)
        messages.success(request, _("Catégories standards créées/mises à jour."))
    except Exception as e:
        messages.error(request, str(e))
    return redirect('shop:dashboard:club_product_create')
...
def club_product_edit(request, product_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and image_formset.is_valid() and variation_formset.is_valid():
            ...
            messages.success(request, _("Produit mis Ã  jour avec succès."))
            return redirect('shop:dashboard:club_product_detail', product_id=product.id)
    else:
...
def club_inventory(request):
    ...
    # Mise Ã  jour du stock en masse si formulaire soumis
    if request.method == 'POST':
        ...
        messages.success(request, _("Stock mis Ã  jour pour {} produits.").format(updated))
    ...
    context = {
        ...
        'current_section': 'shop_products'  # Utiliser la mÃªme section que les produits
    }
...
def club_product_detail(request, product_id):
    ...
    context = {
        ...
        'reviews': reviews[:5],  # Limiter Ã  5 avis
        'stats': stats,
        'current_section': 'shop_products'
    }
...
def shop_federation_dashboard(request):
    ...
    if federation_organization:
        ...
        federation_clubs = Club.objects.none()  # QuerySet vide au lieu d'une liste
...
def federation_product_catalog(request):
    ...
    try:
        # Utiliser la nouvelle relation via organizations
        ...
        if federation_org:
            ...
            federation_clubs = Club.objects.filter(organization__in=affiliated_orgs)
        else:
            federation_clubs = Club.objects.filter(old_federation_id=federation.id) if hasattr(Club, 'old_federation_id') else Club.objects.none()
    except:
        federation_clubs = Club.objects.none()
    # Récupérer les produits liés aux disciplines de la fédération
    try:
        federation_disciplines = federation.disciplines.all() if hasattr(federation, 'disciplines') else []
        products = Product.objects.filter(disciplines__in=federation_disciplines, is_active=True).distinct()
    except:
        products = Product.objects.filter(is_active=True)
    ...
    return render(request, 'shop/dashboard/federation/catalog.html', context)
...
def federation_orders(request):
    ...
    """
    # Récupérer la fédération soit depuis la requÃªte, soit depuis les paramètres GET
    federation = getattr(request, 'federation', None)
    if not federation:
        federation_id = request.GET.get('federation_id')
        if federation_id:
    ...
    # Récupérer toutes les commandes liées Ã  la fédération
    try:
        # Utiliser la nouvelle relation via organizations
        federation_org = getattr(federation, 'as_organization', None)
        if federation_org:
            ...
            federation_clubs = Club.objects.filter(organization__in=affiliated_orgs)
            orders = Order.objects.all().order_by('-created_at')  # Commandes générales pour éviter les erreurs
        else:
...
def federation_product_detail(request, product_id):
    ...
    Un produit appartient Ã  une fédération s'il est associé Ã  l'une de ses disciplines.
    """
    federation = request.federation
