...
from apps.shop.forms.catalog import ProductReviewForm
...
def add_review(request, product_slug):
    ...
    # Vérifier si l'utilisateur a déjÃ  laissé un avis
    if ProductReview.objects.filter(product=product, user=request.user).exists():
        messages.warning(request, _("Vous avez déjÃ  laissé un avis pour ce produit."))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if form.cleaned_data.get('image'):
                from apps.shop.models import ReviewImage
                image = ReviewImage(
                    review=review,
...
def edit_review(request, review_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Mettre Ã  jour les notes détaillées si présentes
            if form.cleaned_data.get('quality_rating'):
                review.quality_rating = form.cleaned_data['quality_rating']
            if form.cleaned_data.get('value_rating'):
                review.value_rating = form.cleaned_data['value_rating']
            ...
            messages.success(request, _("Votre avis a été mis Ã  jour avec succès et sera publié après modération."))
