...
class ProductForm(forms.ModelForm):
    ...
    """
    # Champ pour créer une nouvelle catégorie Ã  la volée
    ...
    def __init__(self, *args, **kwargs):
        ...
        for main_cat in main_categories:
            ...
            for sub_cat in sub_categories:
                choices.append((sub_cat.id, f"â†’ {sub_cat.name}"))
        
        self.fields['categories'].choices = choices
        
        # Rendre certains champs optionnels plus clairs
        self.fields['slug'].help_text = _("Laissez vide pour générer automatiquement Ã  partir du nom du produit")
    ...
    def clean_slug(self):
        ...
        if not slug and name:
            # Générer automatiquement le slug Ã  partir du nom
            slug = slugify(name)
        ...
        if slug:
            ...
            if existing_product.exists():
                # Ajouter un suffixe numérique si le slug existe déjÃ 
                counter = 1
                original_slug = slug
    ...
    def clean(self):
        ...
        if sale_price and price and sale_price >= price:
            self.add_error('sale_price', _("Le prix de vente doit Ãªtre inférieur au prix normal."))
        ...
        if new_category_name:
            ...
            # Vérifier si la catégorie n'existe pas déjÃ 
...
class OrderStatusUpdateForm(forms.ModelForm):
    """
    Formulaire pour mettre Ã  jour le statut d'une commande.
    """
    class Meta:
        model = Order
