...
@dataclass
class MigrationTask:
    """Représente une tÃ¢che de migration."""
    club: Club
    priority: int = 0
    retry_count: int = 0
    max_retries: int = 3
...
class MigrationMonitor:
    ...
    def update_current(self, club_name: str):
        """Met Ã  jour le club en cours de migration."""
...
class ProgressiveMigrator:
    ...
    def prepare_migration(self, club_ids: Optional[List[int]] = None):
        """Prépare la migration en créant les tÃ¢ches."""
        if club_ids:
        ...
        # Créer les tÃ¢ches avec priorité
        tasks = []
        for club in clubs:
    ...
    def _migrate_club_safe(self, task: MigrationTask):
        ...
        try:
            ...
            error_msg = f"Ã‰chec migration {club.name}: {str(e)}"
    ...
    def _handle_migration_error(self, task: MigrationTask, error: Exception):
        ...
        error_msg = f"Ã‰chec définitif pour {task.club.name}: {str(error)}"
        self.monitor.record_failure(error_msg)
    ...
    def _monitor_progress(self):
        ...
        while not self.stop_event.is_set():
            ...
            # Vérifier si on doit arrÃªter (trop d'erreurs)
            if self.monitor.clubs_failed > 10:
                print("\nâš ï¸  Trop d'erreurs, arrÃªt de la migration")
                self.stop_event.set()
...
class MigrationOrchestrator:
    ...
    def run_migration(self, dry_run: bool = False, club_ids: Optional[List[int]] = None):
        ...
        print("ðŸš€ Démarrage de la migration progressive...")
        
        # Phase 1: Préparation
        print("\nðŸ“‹ Phase 1: Préparation")
        total_clubs = self.migrator.prepare_migration(club_ids)
        print(f"âœ“ {total_clubs} clubs Ã  migrer")
        ...
        print(f"âœ“ Backup créé: {backup_file}")
        
        # Phase 3: Migration
        print("\nâš™ï¸  Phase 3: Migration")
        try:
