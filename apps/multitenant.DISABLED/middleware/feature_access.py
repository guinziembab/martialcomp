"""
Middleware de contrÃ´le d'accès aux fonctionnalités basé sur l'abonnement
"""
import re
...
class FeatureAccessMiddleware:
    """
    Middleware qui contrÃ´le l'accès aux fonctionnalités en fonction du niveau d'abonnement.
    """
    ...
    def __init__(self, get_response):
        ...
        self.feature_patterns = [
            ...
            # Ajouter d'autres modèles d'URL liés Ã  des fonctionnalités spécifiques
        ]
    ...
    def __call__(self, request):
        ...
        # Récupérer le tenant depuis la requÃªte (défini par un autre middleware)
            ...
            if re.match(pattern, request.path):
                ...
                if not is_available:
                    if request.headers.get('Accept') == 'application/json':
                        ...
                        # Rediriger vers la page de mise Ã  niveau pour les requÃªtes navigateur
                        return redirect(f'/subscriptions/upgrade/?feature={feature_key}')
                
                # Si c'est une fonctionnalité Ã  l'usage, enregistrer l'utilisation
