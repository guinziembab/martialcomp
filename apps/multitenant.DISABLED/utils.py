...
def create_schema_for_tenant(tenant):
    ...
    with connection.cursor() as cursor:
        ...
        # Donner les permissions appropriées Ã  l'utilisateur de la base
...
def tenant_exists(domain=None, slug=None):
    ...
    Args:
        domain: Domaine Ã  vérifier
        slug: Slug Ã  vérifier
...
def get_tenant_for_request(request):
    """
    Obtient le tenant pour une requÃªte donnée.
...
def reset_schema():
    """
    Réinitialise le schéma Ã  'public'.
    """
...
def validate_schema_name(schema_name):
    ...
    """
    if not schema_name:
        raise ValueError("Le nom de schéma ne peut pas Ãªtre vide")
