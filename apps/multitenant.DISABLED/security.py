...
class SecurityAuditor:
    ...
    def __init__(self, tenant=None):
        ...
        Args:
            tenant: Tenant Ã  auditer (None pour auditer tous les tenants)
        """
        self.tenant = tenant
        self.results = {}
        self.violations = []
            ...
            try:
                ...
                with TenantContext(tenant):
                    ...
                    with connection.cursor() as cursor:
                        try:
                            # Essayer d'accéder Ã  une table du schéma public
                            cursor.execute("SELECT * FROM public.auth_user LIMIT 1")
                            
                            # Si nous sommes ici, cela signifie que l'accès a réussi
                            violation = {
                                'tenant': tenant.name,
                                'description': f"Le schéma {tenant.schema_name} peut accéder Ã  public.auth_user",
                                'severity': 'high',
                    ...
                    # Test 2: Essayer d'accéder Ã  d'autres schémas de tenants
                    other_tenants = Tenant.objects.exclude(id=tenant.id).filter(is_active=True)
                    for other_tenant in other_tenants:
                        with connection.cursor() as cursor:
                            try:
                                # Essayer d'accéder Ã  une table dans un autre schéma
                                cursor.execute(f"SELECT * FROM {other_tenant.schema_name}.auth_user LIMIT 1")
                                
                                # Si nous sommes ici, cela signifie que l'accès a réussi
                                violation = {
                                    'tenant': tenant.name,
                                    'description': f"Le schéma {tenant.schema_name} peut accéder Ã  {other_tenant.schema_name}.auth_user",
                                    'severity': 'critical',
                                    'timestamp': timezone.now().isoformat()
                                }
                                self.results['cross_schema_access']['violations'].append(violation)
            ...
            try:
                ...
                with TenantContext(tenant):
                    ...
                    if retrieved_value != value:
                        violation = {
                            'tenant': tenant.name,
                            'description': f"Ã‰chec de récupération de la valeur du cache pour le tenant {tenant.name}",
                            'severity': 'medium',
                            'timestamp': timezone.now().isoformat()
                        }
                    ...
                    for other_tenant in tenants:
                        if other_tenant.id != tenant.id:
                            ...
                            if other_value == test_data[other_tenant.id]['random_value']:
                                violation = {
                                    'tenant': tenant.name,
                                    'description': f"Le tenant {tenant.name} peut accéder Ã  la valeur de cache du tenant {other_tenant.name}",
                                    'severity': 'high',
                                    'timestamp': timezone.now().isoformat()
                                }
                                self.results['cache_isolation']['violations'].append(violation)
                                ...
                                check = {
                                    'tenant': tenant.name,
                                    'description': f"Le tenant {tenant.name} ne peut pas accéder Ã  la valeur de cache du tenant {other_tenant.name}",
                                    'timestamp': timezone.now().isoformat()
                                }
                                self.results['cache_isolation']['checks'].append(check)
    ...
    def audit_file_access(self):
        ...
        try:
            ...
            # TODO: Implémenter un test complet avec des requÃªtes HTTP réelles
            ...
            for tenant in tenants:
                ...
                if os.path.exists(test_files[tenant.id]):
                    ...
                    violation = {
                        'tenant': tenant.name,
                        'description': f"Ã‰chec de création du fichier de test pour {tenant.name}",
                        'severity': 'low',
                        'timestamp': timezone.now().isoformat()
                    }
    ...
    def audit_security_headers(self):
        """
        Vérifie la présence des en-tÃªtes de sécurité HTTP.
        """
        ...
        for header, expected_value in required_headers.items():
            ...
            if actual_value != expected_value and not (settings.DEBUG and header in ['SECURE_SSL_REDIRECT', 'SESSION_COOKIE_SECURE', 'CSRF_COOKIE_SECURE']):
                violation = {
                    'description': f"En-tÃªte de sécurité {header} manquant ou incorrect (attendu: {expected_value}, obtenu: {actual_value})",
                    'severity': 'medium',
                    'timestamp': timezone.now().isoformat()
                }
                ...
                check = {
                    'description': f"En-tÃªte de sécurité {header} correctement configuré",
                    'timestamp': timezone.now().isoformat()
                }
        ...
        for tenant in tenants:
            ...
            try:
                ...
                with TenantContext(tenant):
                    ...
                    # Pour l'instant, nous ajoutons un simple contrÃ´le
...
class TenantSecurityMonitor:
    ...
    def check_tenant_activity(tenant):
        ...
        # - Taux anormalement élevé de requÃªtes API
        # - Accès Ã  des ressources sensibles
        # - Tentatives d'accès Ã  d'autres tenants
    ...
    def check_database_integrity(tenant):
        ...
        Args:
            tenant: Le tenant Ã  vérifier
        
        Returns:
            Résultat du contrÃ´le d'intégrité
        """
        with TenantContext(tenant):
...
def run_security_audit(tenant=None):
    ...
    Args:
        tenant: Le tenant Ã  auditer (None pour tous les tenants)
