...
from apps.multitenant.models import import TenantPayment
from apps.competitions.models import Practitioner, Competition, CompetitionRegistration
from apps.multitenant.models import TenantForm, DomainForm, TenantFeatureForm
...
class TenantUpdateView(LoginRequiredMixin, SuperAdminRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            f"Tenant '{self.object.name}' mis Ã  jour."
        )
        return response
...
class DomainCreateView(LoginRequiredMixin, SuperAdminRequiredMixin, CreateView):
    """Ajouter un domaine Ã  un tenant."""
    model = Domain
...
class DomainUpdateView(LoginRequiredMixin, SuperAdminRequiredMixin, UpdateView):
    """Met Ã  jour un domaine existant."""
    model = Domain
    form_class = DomainForm
    template_name = 'multitenant/admin/domain_form.html'
    ...
    def form_valid(self, form):
        messages.success(self.request, f"Domaine {form.instance.domain} mis Ã  jour avec succès.")
...
class TenantFeatureCreateView(LoginRequiredMixin, SuperAdminRequiredMixin, CreateView):
    """Ajoute une feature Ã  un tenant."""
    model = TenantFeature
