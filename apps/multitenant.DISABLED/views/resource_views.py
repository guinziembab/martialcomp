...
from apps.multitenant.resource_limits import (
...
class UpgradePlanView(LoginRequiredMixin, TenantRequiredMixin, 
                     TenantAwareViewMixin, TemplateView):
    """
    Page de mise Ã  niveau du plan d'abonnement.
    """
    ...
    def get_context_data(self, **kwargs):
        ...
        from apps.multitenant.payments.pricing import get_regional_pricing
    ...
    def post(self, request, *args, **kwargs):
        """
        Traite la demande de mise Ã  niveau.
        """
        ...
        # Vérifier que c'est une mise Ã  niveau
        plan_levels = {
            'essentials': 1,
            'masters': 2,
            'champion': 3,
        ...
        if plan_levels[new_plan] <= plan_levels[tenant.subscription_plan]:
            messages.error(request, _("Veuillez choisir un plan supérieur Ã  votre plan actuel."))
...
class ResourceLimitExceededView(LoginRequiredMixin, TenantRequiredMixin, 
    ...
    def _get_next_plan(self, current_plan):
        """
        Détermine le plan suivant pour la mise Ã  niveau.
        """
        ...
        return 'masters'  # Plan par défaut pour la mise Ã  niveau
