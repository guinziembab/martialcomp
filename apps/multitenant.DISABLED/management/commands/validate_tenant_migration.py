...
from apps.competitions.models import Club, Practitioner, Competition, Registration
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'tenant_schema',
            type=str,
            help='Nom du schéma du tenant Ã  valider'
        )
        parser.add_argument(
            '--full',
            action='store_true',
    ...
    def validate_schema_exists(self, tenant):
        ...
        with connection.cursor() as cursor:
            ...
            if exists:
                self.stdout.write(
                    self.style.SUCCESS(f"âœ“ Schéma {tenant.schema_name} existe")
                )
            else:
                self.stdout.write(
                    self.style.ERROR(f"âœ— Schéma {tenant.schema_name} n'existe pas")
                )
    ...
    def validate_domain_exists(self, tenant):
        ...
        if domains.exists():
            self.stdout.write(
                self.style.SUCCESS(f"âœ“ {domains.count()} domaine(s) configuré(s)")
            )
            for domain in domains:
                self.stdout.write(f"  - {domain.domain} {'(principal)' if domain.is_primary else ''}")
        else:
            self.stdout.write(
                self.style.ERROR("âœ— Aucun domaine configuré")
            )
            self.errors_found = True
        ...
        with connection.cursor() as cursor:
            ...
            for table in required_tables:
                if table in existing_tables:
                    self.stdout.write(self.style.SUCCESS(f"âœ“ Table {table} existe"))
                else:
                    self.stdout.write(self.style.ERROR(f"âœ— Table {table} manquante"))
            ...
            try:
                ...
                with connection.cursor() as cursor:
                    ...
                    if tenant_practitioners == original_practitioners:
                        self.stdout.write(
                            self.style.SUCCESS(
                                f"âœ“ Nombre de pratiquants cohérent: {tenant_practitioners}"
                            )
                        )
                    else:
                        self.stdout.write(
                            self.style.ERROR(
                                f"âœ— Incohérence pratiquants: {tenant_practitioners} "
    ...
    def validate_relationships(self, tenant):
        ...
        with connection.cursor() as cursor:
            ...
            if orphan_registrations == 0:
                self.stdout.write(
                    self.style.SUCCESS("âœ“ Aucune inscription orpheline")
                )
            else:
                self.stdout.write(
                    self.style.ERROR(
                        f"âœ— {orphan_registrations} inscription(s) orpheline(s)"
                    )
                )
                self.errors_found = True
    ...
    def validate_permissions(self, tenant):
        ...
        if admins.exists():
            self.stdout.write(
                self.style.SUCCESS(f"âœ“ {admins.count()} administrateur(s) trouvé(s)")
            )
        else:
            self.stdout.write(
                self.style.ERROR("âœ— Aucun administrateur configuré")
            )
            self.errors_found = True
    ...
    def validate_performance(self, tenant):
        ...
        with connection.cursor() as cursor:
            ...
            if duration < 0.1:
                self.stdout.write(
                    self.style.SUCCESS(f"âœ“ Performance requÃªte simple: {duration:.3f}s")
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f"âš  Performance requÃªte simple: {duration:.3f}s")
                )
            ...
            if duration < 0.5:
                self.stdout.write(
                    self.style.SUCCESS(f"âœ“ Performance jointure: {duration:.3f}s")
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f"âš  Performance jointure: {duration:.3f}s")
                )
