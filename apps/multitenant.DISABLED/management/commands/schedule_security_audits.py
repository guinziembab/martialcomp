...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--tenant',
            type=str,
            help='ID du tenant spécifique Ã  auditer (laisser vide pour audit global)',
        )
        parser.add_argument(
            '--disable',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if disable:
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f"Aucune tÃ¢che planifiée trouvée: {task_name}")
                )
            return
        ...
        if frequency == 'daily':
            ...
            schedule_desc = f"Tous les jours Ã  {hour}h00"
            ...
            schedule_desc = f"Tous les lundis Ã  {hour}h00"
        else:  # monthly
            schedule, created = CrontabSchedule.objects.get_or_create(
                minute='0',
                hour=str(hour),
            ...
            schedule_desc = f"Le 1er de chaque mois Ã  {hour}h00"
        
        # Créer ou mettre Ã  jour la tÃ¢che périodique
        task_kwargs = {
            'tenant_id': tenant_id
        } if tenant_id else {}
        ...
        action = "créée" if created else "mise Ã  jour"
        self.stdout.write(
            self.style.SUCCESS(
                f"TÃ¢che d'audit {action}: {task_name}\n"
                f"Fréquence: {schedule_desc}\n"
                f"Type: {'Tenant spécifique' if tenant_id else 'Global'}"
            )
        )
        
        # Créer également une tÃ¢che de nettoyage mensuelle
        ...
        if cleanup_created:
            self.stdout.write(
                self.style.SUCCESS(
                    f"TÃ¢che de nettoyage créée: {cleanup_task_name} (mensuelle)"
                )
            )
