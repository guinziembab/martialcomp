...
def club_competition_results(request):
    ...
    # Tous les utilisateurs associés Ã  un club peuvent voir ses résultats
    ...
    if not club:
        return render(request, 'competitions/club/error.html', {
            'error_message': _("Vous n'Ãªtes pas associé Ã  un club.")
        })
    
    # Récupérer les compétitions terminées oÃ¹ le club a participé
    today = timezone.now().date()
