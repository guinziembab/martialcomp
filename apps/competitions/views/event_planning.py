...
from apps.competitions.models.event_planning import (
...
from apps.competitions.forms.event_planning import (
...
try:
    from apps.competitions.forms.event_planning import PollQuestionFormSet
    POLL_QUESTIONS_AVAILABLE = True
except ImportError:
    PollQuestionFormSet = None
    POLL_QUESTIONS_AVAILABLE = False
from apps.organizations.models import Organization, OrganizationMember
from apps.competitions.utils.decorators import club_admin_required, federation_admin_required
...
def poll_detail(request, poll_id):
    ...
    # Vérifier si l'utilisateur a accès Ã  ce sondage
    can_view = (
    ...
    if not can_view and not poll.share_url_code:
        return HttpResponseForbidden(_("Vous n'avez pas accès Ã  ce sondage."))
...
def create_poll(request, event_id=None):
    ...
    Peut Ãªtre associé Ã  un événement existant si event_id est fourni.
    """
    event = None
    organization = None
    ...
    if event_id:
        ...
        # Récupérer les organisations de l'utilisateur avec des rÃ´les administratifs
        user_organizations = Organization.objects.filter(
            members__user=request.user,
        ...
        if not user_organizations.exists():
            messages.error(request, _("Vous devez Ãªtre administrateur, manager ou propriétaire d'une organisation pour créer un sondage."))
    ...
    # Liste des organisations pour le formulaire (utiliser la mÃªme logique que ci-dessus)
...
def edit_poll(request, poll_id):
    """
    Ã‰dition d'un sondage existant.
    """
    poll = get_object_or_404(EventPoll, id=poll_id)
    ...
    if poll.status in ['finalized', 'cancelled']:
        messages.error(request, _("Ce sondage ne peut plus Ãªtre modifié car il a été finalisé ou annulé."))
        return redirect('competitions:event_planning:poll_detail', poll_id=poll.id)
...
def poll_respond(request, option_id):
    """
    Soumet une réponse individuelle Ã  une option de sondage.
    """
    ...
    # Vérifier s'il y a déjÃ  une réponse pour cette option et cet utilisateur
    try:
        response = PollResponse.objects.get(
            option=option,
            user=request.user
        )
        # Si le vote multiple n'est pas autorisé et que la réponse existe déjÃ ,
        # on renvoie une erreur
        if not poll.allow_multiple_votes:
            return JsonResponse({
                'success': False,
                'message': _("Vous avez déjÃ  répondu Ã  cette option.")
            }, status=400)
    ...
    if form.is_valid():
        ...
        # Retourner les statistiques mises Ã  jour pour cette option
...
def finalize_poll(request, poll_id, option_id):
    ...
    # Vérifier que le sondage n'est pas déjÃ  finalisé ou annulé
    if poll.status in ['finalized', 'cancelled']:
        return JsonResponse({
            'success': False, 
            'message': _("Ce sondage est déjÃ  finalisé ou annulé.")
        }, status=400)
...
def public_poll(request, share_code):
    """
    Accède Ã  un sondage via son code de partage public.
    """
...
def create_reminder(request, event_id):
    ...
    # Participants Ã  l'événement pour le champ des destinataires
    participants = list(
...
def edit_reminder(request, reminder_id):
    ...
    # Ne pas permettre l'édition des rappels déjÃ  envoyés
    if reminder.is_sent:
        messages.error(request, _("Ce rappel a déjÃ  été envoyé et ne peut plus Ãªtre modifié."))
        return redirect('competitions:event_planning:event_reminders', event_id=event.id)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, _("Le rappel a été mis Ã  jour avec succès."))
            return redirect('competitions:event_planning:event_reminders', event_id=event.id)
...
def delete_reminder(request, reminder_id):
    ...
    # Ne pas permettre la suppression des rappels déjÃ  envoyés
    if reminder.is_sent:
        return JsonResponse({
            'success': False, 
            'message': _("Ce rappel a déjÃ  été envoyé et ne peut pas Ãªtre supprimé.")
        }, status=400)
