...
def scan_practitioner_qr(request):
    ...
    # Vérification des permissions avec message plus détaillé
    if not (club.owner == request.user or request.user.has_perm('competitions.change_club')):
        messages.error(request, _(
            "Accès refusé: Vous devez Ãªtre administrateur du club '{}' pour scanner des QR codes. "
            "Contactez le propriétaire du club ({}) pour obtenir les permissions."
        ).format(club.name, club.owner.get_full_name() or club.owner.username))
        return redirect('competitions:dashboard:index')
        
    # Ajouter des informations de debug en mode DEBUG
    context = {
        'page_title': _("Scanner QR Code"),
        'scan_types': QRCodeScan.SCAN_TYPE_CHOICES,
        'club': club,  # Ajouter le club au contexte pour debug
        'user_permissions': {
            'is_owner': club.owner == request.user,
            'has_change_perm': request.user.has_perm('competitions.change_club'),
        },
    ...
    # Ajouter des infos de debug en mode développement
    if settings.DEBUG:
        context['debug_info'] = {
            'user_id': request.user.id,
            'club_id': club.id,
            'club_name': club.name,
            'club_owner': str(club.owner),
            'has_qrcode_module': HAS_QRCODE,
        }
...
@login_required
def view_qr(request, practitioner_id):
    """Affiche la page avec le QR code d'un pratiquant"""
    practitioner = get_object_or_404(Practitioner, id=practitioner_id)
    ...
    # Vérifier si l'utilisateur est administrateur du club du pratiquant
    try:
        if hasattr(request.user, 'club') and request.user.club and practitioner.organization:
            is_club_admin = practitioner.organization == request.user.club.organization
        elif hasattr(practitioner, 'club') and hasattr(request.user, 'club'):
            is_club_admin = practitioner.club == request.user.club
    except AttributeError:
        pass
...
def scan_history(request):
    ...
    context = {
        'scans': scans[:100],  # Limiter Ã  100 derniers scans
        'scan_types': QRCodeScan.SCAN_TYPE_CHOICES,
...
def qr_code_offline_token(request, practitioner_id):
    ...
    return JsonResponse({
        ...
        "expires_in_days": 7,  # Valeur en dur correspondant Ã  TOKEN_VALIDITY_DAYS dans qr_offline.py
        "federation_validated": qr_code.is_federation_validated,
    })
...
def verify_offline_profile(request):
    ...
    try:
        ...
        return JsonResponse({"error": "Format de requÃªte invalide"}, status=400)
    

def view_offline_profile_public(request, token=None):
    """
    Affiche le profil d'un pratiquant Ã  partir d'un token hors-ligne.
...
def mark_attendance_ajax(request):
    """Marque la présence d'un pratiquant via AJAX (pour scan QR code)"""
    try:
        data = json.loads(request.body)
        ...
        if not request.user.is_authenticated:
            return JsonResponse({
                'success': False,
                'message': _("Vous devez Ãªtre connecté")
            }, status=401)
        ...
        # Vérifier que l'utilisateur a les permissions pour ce club
        if not (request.user.club and session.training_slot.club == request.user.club):
            return JsonResponse({
                'success': False,
                'message': _("Vous n'avez pas les permissions pour cette session")
            }, status=403)
        
        # Créer ou mettre Ã  jour la présence
        attendance, created = Attendance.objects.get_or_create(
            session=session,
            practitioner=practitioner,
            defaults={
                'status': status,
                'arrival_time': timezone.now().time() if status == 'present' else None
            }
        )
        
        if not created:
            attendance.status = status
            if status == 'present' and not attendance.arrival_time:
                attendance.arrival_time = timezone.now().time()
            attendance.save()
        
        # Créer un scan QR code pour l'historique
        ...
        return JsonResponse({
            'success': True,
            'message': _("Présence marquée avec succès"),
            'attendance': {
                'id': attendance.id,
...
def sync_offline_scans(request):
    ...
    try:
        ...
        if not offline_scans:
            return JsonResponse({
                "success": True,
                "message": "Aucun scan Ã  synchroniser",
                "synced": 0,
                "ignored": 0
            })
