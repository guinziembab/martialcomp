...
def liste_equipes(request, competition_id=None):
    ...
    if competition_id:
        ...
        equipes = Equipe.objects.order_by('-created_at', 'nom')
...
def ajouter_membre_equipe(request, equipe_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Vérifier si ce pratiquant n'est pas déjÃ  membre de cette équipe
            membre = form.save(commit=False)
            if MembreEquipe.objects.filter(equipe=equipe, pratiquant=membre.pratiquant).exists():
                messages.error(request, _("Ce pratiquant est déjÃ  membre de cette équipe."))
            else:
                membre.equipe = equipe
                membre.save()
                messages.success(request, _("Le membre a été ajouté Ã  l'équipe avec succès."))
                return redirect('competitions:combat:detail_equipe', equipe_id=equipe.id)
    else:
    ...
    return render(request, 'competitions/combat/form_membre_equipe.html', {
        'form': form,
        'equipe': equipe,
        'title': _("Ajouter un membre Ã  l'équipe")
    })
...
def generer_poules(request, competition_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Sélectionner les équipes et/ou pratiquants individuels qui ne sont pas déjÃ  dans une poule
            ...
            if equipes_disponibles.exists():
                ...
                # Algorithme pour éviter les équipes du mÃªme club/pays dans la mÃªme poule
...
def supprimer_combat(request, combat_id):
    ...
    if request.method == 'POST':
        ...
        if poule_id:
            return redirect('competitions:combat:liste_combats', poule_id=poule_id)
        else:
            return redirect('competitions:combat:liste_combats', competition_id=competition_id)
...
def ajouter_action(request, combat_id):
    ...
    if combat.status != 'en_cours':
        ...
        return JsonResponse({'success': False, 'error': "Combat non actif"}, status=400) if request.is_ajax() else redirect('competitions:combat:detail_combat', combat_id=combat.id)
    
    if request.method == 'POST':
        # Gérer Ã  la fois les requÃªtes AJAX et les soumissions de formulaire
        if request.is_ajax():
        ...
        # Mettre Ã  jour le combat (fait automatiquement par le modèle ActionCombat)
    ...
    return JsonResponse({'success': False, 'error': "Méthode non autorisée"}, status=405) if request.is_ajax() else redirect('competitions:combat:detail_combat', combat_id=combat.id)
