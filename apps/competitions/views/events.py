...
from apps.competitions.forms.event_forms import EventForm, EventFilterForm, EventParticipantForm
...
def create_event(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            event.save()
            # RafraÃ®chir l'objet depuis la base de données pour s'assurer que l'UUID est bien généré
...
def edit_event(request, event_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            event = form.save()
            messages.success(request, _("L'événement a été mis Ã  jour avec succès."))
            return redirect('competitions:events:event_detail', event_id=event.id)
    else:
...
def export_events(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            events = get_organization_queryset(Event, self.request.user)
