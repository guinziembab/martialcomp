...
def judge_dashboard(request):
    ...
    Affiche les compétitions oÃ¹ le juge est inscrit et ses affectations.
    """
    ...
    try:
        ...
        messages.error(request, _("Vous n'Ãªtes pas enregistré comme pratiquant."))
        return redirect('competitions:dashboard')
    ...
    # Récupérer toutes les inscriptions du pratiquant oÃ¹ il est juge/arbitre
    ...
    # Récupérer les compétitions Ã  venir oÃ¹ le juge peut s'inscrire
...
def edit_judge_profile(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                # Ne pas modifier les niveaux de qualification qui doivent Ãªtre validés par les administrateurs
                judge_updated = form.save(commit=False)
                
                # Ces champs ne doivent pas Ãªtre modifiables par l'utilisateur
                ...
                # D'autres champs qui ne devraient pas Ãªtre modifiés par l'utilisateur
                ...
                messages.success(request, _("Votre profil de juge a été mis Ã  jour avec succès."))
                return redirect('competitions:judge:profile')
            except Exception as e:
                logger.error(f"Erreur lors de la mise Ã  jour du profil de juge: {str(e)}")
                messages.error(request, _("Une erreur est survenue lors de la mise Ã  jour de votre profil: {}").format(str(e)))
        else:
...
def add_qualification(request):
    ...
    # Vérifier si l'utilisateur a le rÃ´le de juge
    if not hasattr(request.user, 'role') or request.user.role != 'judge':
        messages.error(request, _("Vous n'avez pas accès Ã  cette fonctionnalité."))
        return redirect('competitions:dashboard')
...
def upcoming_competitions(request):
    """
    Affiche les compétitions Ã  venir pour lesquelles le juge peut s'inscrire.
    """
    # Vérifier si l'utilisateur a le rÃ´le de juge
    if not hasattr(request.user, 'role') or request.user.role != 'judge':
        messages.error(request, _("Vous n'avez pas accès Ã  cette fonctionnalité."))
    ...
    # Récupérer les compétitions oÃ¹ le juge est déjÃ  inscrit
...
def competition_detail(request, competition_id):
    ...
    try:
        ...
        # Vérifier si le juge est inscrit Ã  cette compétition
...
def competition_assignments(request, competition_id):
    ...
    if not registration:
        messages.error(request, _("Vous n'Ãªtes pas inscrit comme juge pour cette compétition."))
...
def register_for_competition(request, competition_id):
    """
    Permet Ã  un juge de s'inscrire pour officier lors d'une compétition.
    """
    # Vérifier si l'utilisateur a le rÃ´le de juge
    if not hasattr(request.user, 'role') or request.user.role != 'judge':
        messages.error(request, _("Vous n'avez pas accès Ã  cette fonctionnalité."))
    ...
    # Vérifier si le juge est déjÃ  inscrit Ã  cette compétition
    ...
    if request.method == 'POST':
        # Recueillir les informations sur les types de rÃ´les de juge/arbitre
        is_technical_judge = 'is_technical_judge' in request.POST
        is_combat_referee = 'is_combat_referee' in request.POST
        
        # Au moins un type doit Ãªtre sélectionné
        if not is_technical_judge and not is_combat_referee:
            messages.error(request, _("Vous devez sélectionner au moins un type de rÃ´le (juge technique ou arbitre de combat)."))
            return render(request, 'competitions/judge/register_for_competition.html', {
                'competition': competition,
                'existing_registration': existing_registration,
                'title': _("Inscription Ã  la compétition")
            })
        
        try:
            with transaction.atomic():
                if existing_registration:
                    # Mettre Ã  jour l'inscription existante
                    existing_registration.is_technical_judge = is_technical_judge
                    existing_registration.is_combat_referee = is_combat_referee
                    
                    # Définir les niveaux de qualification Ã  "pending" s'ils ne sont pas déjÃ  définis
                    ...
                    messages.success(request, _("Votre inscription a été mise Ã  jour avec succès."))
                else:
                    ...
                    # Définir les niveaux de qualification Ã  "pending"
            ...
            logger.error(f"Erreur lors de l'inscription Ã  la compétition: {str(e)}")
...
def apply_as_judge(request):
    """
    Permet Ã  un utilisateur de poser sa candidature pour devenir juge technique.
    """
    # Vérifier si l'utilisateur a déjÃ  le statut de juge
    if hasattr(request.user, 'role') and request.user.role == 'judge':
        try:
            practitioner = Practitioner.objects.get(user=request.user)
            judge = Judge.objects.get(practitioner=practitioner)
            messages.info(request, _("Vous Ãªtes déjÃ  enregistré comme juge."))
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    # On ne laisse donc pas l'utilisateur les définir lui-mÃªme
