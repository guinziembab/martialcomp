...
def get_grade_systems_by_discipline(request, discipline_id):
    ...
    try:
        discipline = get_object_or_404(Discipline, pk=discipline_id)
        systems = GradingSystem.objects.filter(discipline=discipline)
...
def get_grades_by_discipline(request, discipline_id):
    ...
    try:
        ...
        system = GradingSystem.objects.filter(discipline=discipline, is_default=True).first()
        if not system:
            system = GradingSystem.objects.filter(discipline=discipline).first()
...
def import_grades(request):
    """Importe des grades Ã  partir de données JSON."""
...
def grade_systems(request):
    discipline_id = request.GET.get('discipline')
    if discipline_id:
        systems = GradingSystem.objects.filter(discipline_id=discipline_id)
...
def get_grades_for_disciplines(request):
    ...
    try:
        ...
        try:
            from apps.grades.models import Grade as GradesGrade
        except ImportError:
...
def generate_license_number(request):
    if request.method == 'POST':
        try:
            ...
            base = f"{disciplines[0][:2].upper()}-MC-{birth_date.replace('-', '')}-{last_name[:4].upper()}"
            # Chercher les numéros existants qui commencent par ce préfixe
            existing = Practitioner.objects.filter(license_number__startswith=base).count()
            if existing == 0:
                license_number = base
            else:
                license_number = f"{base}-{existing+1:02d}"
