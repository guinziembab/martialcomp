...
def club_dashboard(request):
    ...
    if not club.organization:
        # Tenter d'attacher depuis as_organization
        if getattr(club, 'as_organization', None):
            club.organization = club.as_organization
        else:
            # Créer une Organization associée si elle n'existe pas
            try:
    ...
    # Récupérer TOUS les pratiquants pour la fonctionnalité d'inscription en masse
    if club_organization:
        all_practitioners = Practitioner.objects.filter(
            organization=club_organization
        ).order_by('last_name', 'first_name')
    else:
        # SOLUTION TEMPORAIRE: Créer des pratiquants de démonstration pour le développement
        # En production, on devrait s'assurer qu'un club a toujours une organisation
        logger.warning(f"Club {club.name} n'a pas d'organisation - utilisation de données de démonstration")
        
        # Créer des objets pratiquant temporaires pour la démonstration (non sauvés en DB)
        from datetime import date
        
        class DemoPractitioner:
        ...
        demo_practitioners = [
            DemoPractitioner(
                id=1, first_name='Jean', last_name='Martin', 
                birth_date=date(1990, 5, 15), email='jean.martin@demo.com',
                license_number='JEMA2024', status='active', pk=1
            ),
            DemoPractitioner(
                id=2, first_name='Marie', last_name='Dupont',
                birth_date=date(1985, 8, 22), email='marie.dupont@demo.com',
                license_number='MADU2024', status='active', pk=2
            ),
            DemoPractitioner(
                id=3, first_name='Pierre', last_name='Durand',
                birth_date=date(2000, 12, 10), email='pierre.durand@demo.com',
                license_number='PIDU2024', status='active', pk=3
            ),
            DemoPractitioner(
                id=4, first_name='Sophie', last_name='Bernard',
                birth_date=date(1995, 3, 3), email='sophie.bernard@demo.com',
                license_number='SOBE2024', status='active', pk=4
            ),
            DemoPractitioner(
                id=5, first_name='Lucas', last_name='Petit',
                birth_date=date(2005, 7, 18), email='lucas.petit@demo.com',
                license_number='LUPE2024', status='active', pk=5
            ),
        ]
        ...
        for p in demo_practitioners:
            ...
            p.grade = f"Ceinture {'bleue' if p.id % 2 == 0 else 'verte'}"
            p.grade_text = None
        
        all_practitioners = demo_practitioners
    ...
    try:
        ...
        training_programs = TrainingProgram.objects.filter(
            is_active=True
        ).prefetch_related('disciplines')[:5]
    ...
    try:
        ...
        # Statistiques boutique étendues
        ...
        pending_orders = Order.objects.filter(club=club, status='pending').count()
    ...
    try:
        ...
        if club_organization:
            ...
            # Override currency based on organization country if available
            try:
                org_country = getattr(club_organization, 'country', '') or ''
                code = COUNTRY_TO_CURRENCY.get(org_country) or COUNTRY_TO_CURRENCY.get(str(org_country).upper())
                if code:
                    financial_stats['currency'] = code
            except Exception:
                pass
    except Exception as e:
    ...
    try:
        ...
        if club_organization:
            ...
            # Équipes du club
            if club:
                club_teams = Equipe.objects.filter(club=club).select_related('competition')[:5]
                club_teams_count = Equipe.objects.filter(club=club).count()
            
            # Statistiques
            total_combats = Combat.objects.filter(
                Q(equipe_rouge__club=club) | Q(equipe_blanc__club=club) |
                Q(pratiquant_rouge__organization=club_organization) | 
                Q(pratiquant_blanc__organization=club_organization)
            ).count()
            ...
            # Combats ce mois
            from datetime import timedelta
            month_combats = Combat.objects.filter(
                Q(equipe_rouge__club=club) | Q(equipe_blanc__club=club) |
                Q(pratiquant_rouge__organization=club_organization) | 
                Q(pratiquant_blanc__organization=club_organization),
            ...
            # Calcul victoires/défaites (approximatif)
            victories = Combat.objects.filter(
                Q(equipe_rouge__club=club, vainqueur='rouge') |
                Q(equipe_blanc__club=club, vainqueur='blanc') |
                Q(pratiquant_rouge__organization=club_organization, vainqueur='rouge') |
                Q(pratiquant_blanc__organization=club_organization, vainqueur='blanc'),
                status='termine'
            ).count()
            
            defeats = Combat.objects.filter(
                Q(equipe_rouge__club=club, vainqueur='blanc') |
                Q(equipe_blanc__club=club, vainqueur='rouge') |
                Q(pratiquant_rouge__organization=club_organization, vainqueur='blanc') |
                Q(pratiquant_blanc__organization=club_organization, vainqueur='rouge'),
                status='termine'
            ).count()
    ...
    try:
        ...
        logger.error(f"Erreur lors de la récupération des données d'adhésion: {str(e)}")
        membership_stats = {
            'total_active': 0,
            'total_packages': 0,
            'expiring_soon': 0,
    ...
    context = {
        ...
        'competitions_to_manage_count': competitions_to_manage.count() if competitions_to_manage else 0,
