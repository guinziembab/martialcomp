...
def judge_dashboard(request):
    ...
    stats = {
        ...
        'qualification_level': judge.get_qualification_level_display() if judge and hasattr(judge,
'get_qualification_level_display') else 'N/A',
...
def competition_detail(request, competition_id):
    ...
    context = {
        ...
        'is_past': competition.end_date < timezone.now().date() if hasattr(competition, 'end_date') and competition.end_date else False
    }
...
def competition_assignments(request, competition_id):
    ...
    if not registration:
        ...
        return redirect('competitions:judge:dashboard')

    # Récupérer les affectations du juge pour cette compétition
    try:
        # Utiliser JudgeCompetitionAssignment au lieu de JudgeAssignment
        assignments = JudgeCompetitionAssignment.objects.filter(
            registration=registration
                    ...
                    for organizer in organizers:
                        Notification.objects.create(
                            ...
                            message=_("{} s'est inscrit(e) en tant que {} pour la compétition {}").format(
                                ...
                                roles_text,
                                competition.title if hasattr(competition, 'title') else competition.name
                            ),
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    return redirect('competitions:judge:applications_status')
            except Exception as e:
