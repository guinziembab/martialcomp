...
from apps.competitions.models import Practitioner, Event, EventParticipant
...
def training_dashboard(request):
    """Tableau de bord d'entraÃ®nement du pratiquant."""
    try:
    ...
    recent_evaluations = []  # Ã€ implémenter avec un modèle Evaluation
...
def practitioner_training_schedule(request):
    """Vue du planning d'entraÃ®nement du pratiquant."""
    try:
        practitioner = request.user.practitioners.first()
        if not practitioner:
            messages.error(request, _("Aucun profil de pratiquant trouvé."))
        ...
        # Vérifier que le pratiquant a un club associé
        if not practitioner.club:
            messages.error(request, _("Votre organisation n'est pas associée à un club. Contactez l'administrateur."))
            return redirect('welcome')
    except Exception as e:
        messages.error(request, _("Erreur lors de la récupération du profil."))
        return redirect('welcome')
    ...
    try:
        ...
        week_offset = int(week_param) if week_param and week_param.strip() else 0
...
def practitioner_reserve_training(request, slot_id):
    """Réserver un créneau d'entraÃ®nement."""
    try:
        practitioner = request.user.practitioners.first()
        if not practitioner:
            messages.error(request, _("Aucun profil de pratiquant trouvé."))
    ...
    slot = get_object_or_404(TrainingSlot, id=slot_id, club=practitioner.club)
    ...
    if request.method == 'POST':
        ...
        if created:
            ...
            messages.info(request, _("Vous avez déjÃ  une réservation pour ce créneau."))
...
def practitioner_cancel_reservation(request, reservation_id):
    ...
    if reservation.date <= timezone.now().date() + timedelta(days=1):
        messages.error(request, _("Impossible d'annuler moins de 24h Ã  l'avance."))
        return redirect('competitions:practitioner_training_schedule')
...
def practitioner_attendance_history(request):
    """Historique de présence aux entraÃ®nements."""
    try:
        practitioner = request.user.practitioners.first()
        if not practitioner:
            messages.error(request, _("Aucun profil de pratiquant trouvé."))
...
def practitioner_training_progress(request):
    """Suivi de la progression dans les programmes d'entraÃ®nement."""
    try:
        practitioner = request.user.practitioners.first()
        if not practitioner:
            messages.error(request, _("Aucun profil de pratiquant trouvé."))
    ...
    program_history = []  # Ã€ implémenter avec un modèle ProgramEnrollment
    
    # Objectifs personnels
    personal_goals = []  # Ã€ implémenter avec un modèle PersonalGoal
    
    # Statistiques de progression
    stats = {
        'total_training_hours': 0,  # Ã€ calculer depuis les présences
        'modules_completed': sum(1 for p in progress_data.values() if p['percentage'] == 100),
        'current_streak': 0,  # Ã€ calculer depuis les présences
        'best_streak': 0,  # Ã€ calculer depuis l'historique
    }
...
def practitioner_training_programs(request):
    """Liste des programmes d'entraÃ®nement disponibles."""
    try:
        practitioner = request.user.practitioners.first()
        if not practitioner:
            messages.error(request, _("Aucun profil de pratiquant trouvé."))
...
def program_detail(request, program_id):
    """Détails d'un programme d'entraÃ®nement."""
    try:
        practitioner = request.user.practitioners.first()
        if not practitioner:
            messages.error(request, _("Aucun profil de pratiquant trouvé."))
    ...
    # Avis et évaluations
    program_reviews = []  # Ã€ implémenter avec un modèle ProgramReview
    average_rating = 0  # Ã€ calculer depuis les avis
...
def practitioner_enroll_program(request, program_id):
    """S'inscrire Ã  un programme d'entraÃ®nement."""
    try:
        practitioner = request.user.practitioners.first()
        if not practitioner:
            messages.error(request, _("Aucun profil de pratiquant trouvé."))
    ...
    if request.method == 'POST':
        ...
        # Ã€ implémenter avec un modèle ProgramEnrollment
...
def practitioner_training_calendar_api(request):
    """API pour le calendrier d'entraÃ®nement."""
    try:
        practitioner = request.user.practitioners.first()
        if not practitioner:
    ...
    # Sessions d'entraÃ®nement confirmées (présence)
    attendances = Attendance.objects.filter(
        practitioner=practitioner,
