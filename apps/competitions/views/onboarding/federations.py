...
def handle_federation_creation(request):
    ...
    if existing_federation:
        ...
        return redirect('competitions:dashboard:federation_detail', federation_id=existing_federation.id)
        ...
        if form.is_valid():
            ...
            try:
                with transaction.atomic():
                    ...
                    logger.info(f"=== FEDERATION CREATED SUCCESSFULLY [{timestamp}] - ID: {federation.id} ===")

                    # Redirection directe vers le dashboard des fédérations (comme pour les clubs)
                    return redirect('competitions:dashboard:federation_detail', federation_id=federation.id)
