...
from apps.competitions.models.support import SupportTicket, SupportMessage
...
def practitioner_notification_preferences(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, _("Vos préférences ont été mises Ã  jour."))
...
def practitioner_notifications(request):
    ...
    context = {
        'practitioner': practitioner,
        'notifications': notifications[:50],  # Limiter Ã  50 dernières
...
@login_required
def practitioner_calendar(request, year=None, month=None):
    ...
    # Récupérer le mois et l'année depuis les paramètres URL ou GET
    if year is None:
...
def practitioner_event_detail(request, event_id):
    ...
    if not event.is_public and event.organization != practitioner.organization:
        messages.error(request, _("Vous n'avez pas accès Ã  cet événement."))
...
def practitioner_event_register(request, event_id):
    ...
    if event.participants.filter(id=practitioner.id).exists():
        messages.warning(request, _("Vous Ãªtes déjÃ  inscrit Ã  cet événement."))
        return redirect('competitions:practitioner_event_detail', event_id=event.id)
    ...
    Notification.objects.create(
        ...
        message=_("Votre inscription Ã  l'événement '{}' a été confirmée.").format(event.title),
        is_read=False
    )
...
# Utilisez practitioner_notification_preferences Ã  la place.
