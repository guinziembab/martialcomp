...
class Membership(models.Model):
    ...
    updated_at = models.DateTimeField(_("Dernière mise Ã  jour"), auto_now=True)
    ...
    def __str__(self):
        return f"{self.practitioner.full_name} - {self.get_membership_type_display()} ({self.start_date} Ã  {self.end_date})"
    
    @property
    def is_active(self):
        """Vérifie si l'adhésion est active Ã  la date actuelle."""
...
class License(models.Model):
    ...
    @property
    def is_valid(self):
        """Vérifie si la licence est valide Ã  la date actuelle."""
        from django.utils import timezone
