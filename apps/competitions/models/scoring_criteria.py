...
class ScoringCriterion(models.Model):
    ...
    Exemple: Technique, Puissance, Ã‰quilibre, Respect du kata, etc.
    """
    ...
    def clean(self):
        ...
        if self.max_score <= self.min_score:
            raise ValidationError(_("Le score maximum doit Ãªtre supérieur au score minimum."))
        ...
        if self.step > score_range:
            raise ValidationError(_("Le pas de notation ne peut pas Ãªtre supérieur Ã  la plage de scores."))
...
class ScoringConfiguration(models.Model):
    ...
    allow_ties = models.BooleanField(
        _("Autoriser les ex-Ã¦quo"), 
        default=True,
        help_text=_("Autoriser les ex-Ã¦quo en 3ème place")
    )
    ...
    training_judges_included = models.BooleanField(
        ...
        default=False,
        help_text=_("Inclure les notes des juges en formation (Ã  des fins de comparaison)")
    )
    ...
    def set_default_criteria(self):
        ...
        default_criteria = {
            'karaté': [
                ...
                {'name': _('Rythme'), 'weight': 1.0, 'description': _('Tempo et fluidité de l\'enchaÃ®nement')},
            ...
            'judo': [
                ...
                {'name': _('ContrÃ´le'), 'weight': 1.0, 'description': _('MaÃ®trise et contrÃ´le des mouvements')},
                {'name': _('Coordination'), 'weight': 0.8, 'description': _('Coordination et fluidité des enchaÃ®nements')}
            ],
            'taekwondo': [
                {'name': _('Technique'), 'weight': 1.5, 'description': _('Précision et exécution correcte des mouvements')},
                {'name': _('Force'), 'weight': 1.0, 'description': _('Puissance des coups')},
                {'name': _('Ã‰quilibre'), 'weight': 1.0, 'description': _('Stabilité et équilibre pendant l\'exécution')},
            ...
            'qwan ki do': [
                ...
                {'name': _('Puissance'), 'weight': 1.0, 'description': _('Force et énergie dans l\'exécution')},
                {'name': _('Ã‰quilibre'), 'weight': 1.0, 'description': _('Stabilité et équilibre pendant l\'exécution')},
                {'name': _('Rythme'), 'weight': 0.8, 'description': _('Tempo et fluidité de l\'enchaÃ®nement')},
