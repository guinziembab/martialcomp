...
class ClubRole(models.Model):
    """Définit des rÃ´les personnalisés pour les utilisateurs au sein d'une organisation."""
    
    name = models.CharField(_("Nom"), max_length=100)
    organization = models.ForeignKey('organizations.Organization', on_delete=models.CASCADE, related_name='roles')
    description = models.TextField(_("Description"), blank=True)
    is_default = models.BooleanField(_("RÃ´le par défaut"), default=False)
    ...
    can_manage_roles = models.BooleanField(_("Gérer les rÃ´les"), default=False)
    ...
    class Meta:
        ...
        verbose_name = _("RÃ´le d'organisation")
        verbose_name_plural = _("RÃ´les d'organisation")
        unique_together = ['name', 'organization']  # Corrigé de 'club' Ã  'organization'


class UserClubRole(models.Model):
    """Associe un utilisateur Ã  un ou plusieurs rÃ´les dans une organisation."""
    ...
    def __str__(self):
        return f"{self.user.username} - {self.role.name} ({self.organization.name})"  # Corrigé de 'club' Ã  'organization'
    
    class Meta:
        app_label = 'competitions'
        verbose_name = _("RÃ´le d'utilisateur")
        verbose_name_plural = _("RÃ´les d'utilisateur")
        unique_together = ['user', 'organization', 'role']  # Corrigé de 'club' Ã  'organization'
