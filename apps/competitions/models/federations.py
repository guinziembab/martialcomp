...
class Federation(AdministratorMixin, models.Model):
    ...
    def get_website_url(self):
        """Retourne l'URL du site public de la fédération."""
        return f"https://fed-{self.slug}.martialcomp.com"
    
    def get_admin_site_url(self):
        """Retourne l'URL de gestion du site de la fédération."""
        return f"https://fed-{self.slug}.martialcomp.com/admin/site/"
    
    def get_qr_site_url(self):
        """Retourne l'URL des codes QR du site de la fédération."""
        return f"https://fed-{self.slug}.martialcomp.com/qr/"

    def save(self, *args, **kwargs):
        """Génère un slug unique si nécessaire."""
        if not self.slug:
            if self.name:
                self.slug = slugify(self.name)
                # Si le slugify retourne une chaÃ®ne vide (caractères spéciaux uniquement)
                if not self.slug:
                    self.slug = f"federation-{self.pk or 'new'}"
            else:
                # Si le nom est vide, utiliser un slug par défaut
                self.slug = f"federation-{self.pk or 'new'}"
            
            # Vérifier l'unicité du slug
            counter = 1
            original_slug = self.slug
            while Federation.objects.filter(slug=self.slug).exclude(pk=self.pk).exists():
    ...
    def from_organization(cls, organization):
        """
        Crée ou met Ã  jour une instance Federation Ã  partir d'une Organization.
        ...
        try:
            with transaction.atomic():
                ...
                # Si la fédération existait déjÃ , mettre Ã  jour ses attributs
                if not created:
                ...
                if hasattr(organization, 'disciplines'):
                    ...
                    # Ajouter les disciplines de l'organisation Ã  la fédération
            ...
            logger.error(f"Erreur lors de la création/mise Ã  jour de Federation Ã  partir d'Organization: {str(e)}")
            return None
