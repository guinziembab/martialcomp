...
class EventPoll(models.Model):
    ...
    event = models.OneToOneField(
        ...
        blank=True, 
        related_name='poll',
        verbose_name=_('Ã‰vénement associé')
    )
    ...
    def finalize_with_option(self, option):
        """Finalise le sondage avec l'option choisie et crée/met Ã  jour l'événement."""
        ...
        with models.transaction.atomic():
            ...
            # Mettre Ã  jour le statut du sondage
            self.status = 'finalized'
            self.finalized_at = timezone.now()
            
            # Créer ou mettre Ã  jour l'événement
            if self.event:
                # Mettre Ã  jour l'événement existant
                self.event.start_date = option.date
                self.event.end_date = option.date  # Par défaut mÃªme jour
...
class PollOption(models.Model):
    ...
    def score(self):
        ...
        # On attribue 2 points pour chaque "oui", 1 point pour chaque "peut-Ãªtre"
...
class EventReminder(models.Model):
    ...
    # Options pour l'envoi Ã  des groupes spécifiques
    SEND_TO_CHOICES = [
    ...
    send_to = models.CharField(_('Envoyer Ã '), max_length=20, choices=SEND_TO_CHOICES, default='all')
    ...
    @property
    def is_due(self):
        """Vérifie si le rappel est dÃ» pour Ãªtre envoyé."""
    ...
    def calculated_send_time(self):
        ...
        if self.event.start_time:
            ...
            # Par défaut, on considère que l'événement commence Ã  9h du matin
            event_datetime = timezone.make_aware(
...
class EventStatistics(models.Model):
    ...
    # Analyse temporelle
    first_response_at = models.DateTimeField(_('Première réponse Ã '), null=True, blank=True)
    last_response_at = models.DateTimeField(_('Dernière réponse Ã '), null=True, blank=True)
    ...
    def update_from_event(self, event):
        """Met Ã  jour les statistiques Ã  partir d'un événement."""
        if not event:
            return
...
class PollQuestion(models.Model):
    ...
    Permet d'ajouter des questions supplémentaires au-delÃ  des disponibilités.
    """
    ...
    # Options pour les questions Ã  choix multiple
    choices = models.JSONField(
        _('Choix disponibles'),
        default=list,
        blank=True,
        help_text=_('Liste des options pour les questions Ã  choix multiple')
    )
...
class PollQuestionResponse(models.Model):
    ...
    class Meta:
        app_label = 'competitions'
        verbose_name = _('Réponse Ã  une question')
