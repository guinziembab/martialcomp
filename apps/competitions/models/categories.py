...
class CompetitionCategory(models.Model):
    """Catégorie spécifique Ã  une compétition."""
    ...
    # Champs spécifiques Ã  la compétition
    ...
    def clean(self):
        ...
        if self.min_age and self.max_age and self.min_age > self.max_age:
            raise ValidationError({'max_age': _("L'Ã¢ge maximum doit Ãªtre supérieur ou égal Ã  l'Ã¢ge minimum.")})
        
        if self.min_weight and self.max_weight and self.min_weight > self.max_weight:
            raise ValidationError({'max_weight': _("Le poids maximum doit Ãªtre supérieur ou égal au poids minimum.")})
    ...
    def can_register_participant(self, participant):
        ...
        if self.max_age and participant.age > self.max_age:
            return False, _("Le participant est trop Ã¢gé pour cette catégorie.")
        
        # Vérification du genre
        if self.gender != 'mixed' and participant.gender != self.gender:
            return False, _("Le genre du participant ne correspond pas Ã  cette catégorie.")
