...
class UserProfile(models.Model):
    ...
    Cette classe contient les informations de rÃ´le et d'onboarding pour les utilisateurs.
    """
...
def create_user_profile(sender, instance, created, **kwargs):
    ...
    if created:
        # Ã‰viter de créer un profil s'il existe déjÃ 
...
def user_role(self):
    """Accès au rÃ´le via la relation profile"""
    if hasattr(self, 'profile'):
        return self.profile.role
    return None

def user_onboarding_completed(self):
    """Accès Ã  l'état d'onboarding via la relation profile"""
    if hasattr(self, 'profile'):
        return self.profile.onboarding_completed
    return False

def user_get_practitioners(self):
    """Accès aux pratiquants associés Ã  cet utilisateur"""
