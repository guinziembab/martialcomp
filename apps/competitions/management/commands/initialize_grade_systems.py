...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        from apps.grades.models import GradeCategory, Grade  # Import local pour éviter les problèmes d'ordre d'import
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(
                "âŒ Erreur: Le modèle GradeCategory n'a pas été trouvé.\n"
        ...
        for discipline_name, system_data in grade_systems.items():
            try:
                ...
                with transaction.atomic():
                    ...
                    # Créer ou mettre Ã  jour la catégorie de grade
                    ...
                    for grade_data in system_data['grades']:
                        try:
                            ...
                            if grade_created:
                                created_grades += 1
                                self.stdout.write(f"  âœ“ Grade créé: {grade.name}")
                        except Exception as e:
                            self.stdout.write(self.style.ERROR(
                                f"  âœ— Erreur lors de la création du grade {grade_data['name']}: {str(e)}"
                            ))
            except Exception as e:
                self.stdout.write(self.style.ERROR(
