...
def save_user_profile(sender, instance, **kwargs):
    try:
        ...
        # Log l'erreur mais ne pas empÃªcher la sauvegarde de l'utilisateur
        import logging
        logger = logging.getLogger(__name__)
...
# TEMPORAIREMENT DÃ‰SACTIVÃ‰ pour éviter les conflits lors de l'onboarding
...
def create_practitioner_qr_code_disabled(sender, instance, created, **kwargs):
    if created:
        ...
        try:
            # Vérifier si un QR code existe déjÃ  pour ce practitioner
            ...
            # Log l'erreur mais ne pas empÃªcher la création du pratiquant
            import logging
            logger = logging.getLogger(__name__)
...
# TEMPORAIREMENT DÃ‰SACTIVÃ‰ pour éviter les conflits avec la création manuelle de Federation
    ...
    if created:
        try:
            ...
            if not existing_tenant:
                # Déclencher manuellement la création si pas déjÃ  fait
...
def create_federation_subdomain(sender, instance, created, **kwargs):
    ...
    if created:
        try:
            ...
            base_subdomain = slugify(instance.name).replace('-', '')[:20]  # Limiter Ã  20 caractères
            ...
            # Vérifier si le sous-domaine existe déjÃ 
...
def notify_competition_registration(sender, instance, created, **kwargs):
    """Envoie un email de confirmation lors de l'inscription Ã  une compétition"""
    if created:
        try:
...
def notify_competition_update(sender, instance, created, **kwargs):
    """Envoie des e-mails lors de la mise Ã  jour d'une compétition"""
