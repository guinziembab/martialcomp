    ...
    try:
        ...
        try:
            ...
            if 'score_rouge' in data:
                ...
                fields_to_update.append('score_rouge')
            if 'score_blanc' in data:
                combat.score_blanc = float(data['score_blanc'])
                fields_to_update.append('score_blanc')

            # BUG #6 FIX: Gérer le statut et le vainqueur à la fin du combat
            if 'status' in data and data['status'] == 'termine':
                combat.status = 'termine'
                fields_to_update.append('status')

                # Déterminer et enregistrer le vainqueur
                if 'vainqueur' in data and data['vainqueur'] in ['rouge', 'blanc']:
                    combat.vainqueur = data['vainqueur']
                    combat.est_nul = False
                elif combat.score_rouge > combat.score_blanc:
                    combat.vainqueur = 'rouge'
                    combat.est_nul = False
                elif combat.score_blanc > combat.score_rouge:
                    combat.vainqueur = 'blanc'
                    combat.est_nul = False
                else:
                    combat.vainqueur = None
                    combat.est_nul = True

                fields_to_update.extend(['vainqueur', 'est_nul'])

                # Enregistrer l'heure de fin si disponible
                if hasattr(combat, 'fin_combat'):
                    from django.utils import timezone
                    combat.fin_combat = timezone.now()
                    fields_to_update.append('fin_combat')

                logger.info(f"Combat {combat_id} terminé - Vainqueur: {combat.vainqueur}, Scores: {combat.score_rouge}-{combat.score_blanc}")

            # Sauvegarder
            if fields_to_update:
                combat.save(update_fields=fields_to_update)
...
def get_combat_status(request, combat_id):
    ...
    try:
        ...
        # Préparer les données des participants selon le type de combat
        if combat.type_combat == 'equipe':
            rouge_data = {
                'nom': combat.equipe_rouge.nom if combat.equipe_rouge else 'Non définie',
                'club': combat.equipe_rouge.club.name if combat.equipe_rouge and combat.equipe_rouge.club else None,
                'pays': combat.equipe_rouge.club.country if combat.equipe_rouge and combat.equipe_rouge.club else None,
                'logo_club': combat.equipe_rouge.club.logo.url if combat.equipe_rouge and combat.equipe_rouge.club and combat.equipe_rouge.club.logo else None
            }
            blanc_data = {
                'nom': combat.equipe_blanc.nom if combat.equipe_blanc else 'Non définie',
                'club': combat.equipe_blanc.club.name if combat.equipe_blanc and combat.equipe_blanc.club else None,
                'pays': combat.equipe_blanc.club.country if combat.equipe_blanc and combat.equipe_blanc.club else None,
                'logo_club': combat.equipe_blanc.club.logo.url if combat.equipe_blanc and combat.equipe_blanc.club and combat.equipe_blanc.club.logo else None
            }
        else:
            rouge_data = {
