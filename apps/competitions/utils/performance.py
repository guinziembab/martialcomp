...
Fournit des fonctions pour optimiser les requÃªtes et la mise en cache.
"""
...
from apps.competitions.models.event_planning import EventPoll, PollOption, PollResponse
...
def count_queries(func):
    """
    Décorateur pour compter le nombre de requÃªtes SQL exécutées par une fonction.
    ...
    def wrapper(*args, **kwargs):
        ...
        # Réinitialiser le compteur de requÃªtes
        initial_queries = len(connection.queries)
        
        # Exécuter la fonction
        result = func(*args, **kwargs)
        
        # Calculer le nombre de requÃªtes exécutées
        final_queries = len(connection.queries)
        num_queries = final_queries - initial_queries
        
        # Calculer le temps total des requÃªtes
        query_time = sum(float(q.get('time', 0)) for q in connection.queries[initial_queries:final_queries])
        
        logger.debug(f"{func.__name__} a exécuté {num_queries} requÃªtes en {query_time:.4f} secondes")
        
        # Loguer les requÃªtes qui prennent le plus de temps
        if num_queries > 5:
            slow_queries = sorted(
            ...
            for i, query in enumerate(slow_queries):
                logger.debug(f"RequÃªte lente #{i+1} ({float(query.get('time', 0)):.4f}s): {query.get('sql', '')[:100]}...")
...
def get_cached_upcoming_events(days=30, user=None):
    """
    Récupère les événements Ã  venir avec mise en cache.
    Optimisé pour les performances avec préchargement des relations.
    
    Args:
        days: Nombre de jours Ã  considérer dans le futur
        user: Utilisateur actuel pour filtrage par organisation
    
    Returns:
        QuerySet d'événements Ã  venir
    """
    ...
    # Calculer les événements Ã  venir
...
def get_cached_event_detail(event_id):
    ...
    Returns:
        Ã‰vénement avec relations préchargées ou None
    """
...
def get_cached_event_participants(event_id, limit=None):
    ...
    Args:
        event_id: ID de l'événement
        limit: Nombre maximum de participants Ã  récupérer
...
def invalidate_event_cache(event_id):
    """
    Invalide tous les caches liés Ã  un événement spécifique.
    Ã€ appeler après toute modification d'un événement.
...
def optimize_poll_query(poll_id):
    """
    Optimise la requÃªte pour récupérer un sondage avec toutes ses relations.
...
def optimize_event_list_query(request=None):
    """
    Optimise la requÃªte pour la liste des événements en fonction des filtres.
    
    Args:
        request: Objet de requÃªte HTTP pour les filtres
    ...
    # Filtrage selon les paramètres de la requÃªte
...
def bulk_prefetch_event_data(event_ids):
    ...
    Optimisé pour réduire le nombre de requÃªtes lors de l'affichage de listes.
...
def get_optimized_survey_results(survey_id):
    ...
    """
    from apps.competitions.models.event import EventSurvey, SurveyQuestion, SurveyResponse, QuestionResponse
