...
def generate_match_schedule(category_schedule):
    ...
    """
    # Import Ã  l'intérieur de la fonction pour éviter des imports circulaires
    from apps.competitions.models.schedule import MatchTimeSlot
    
    # Récupérer les matchs de la catégorie
    matches = category_schedule.category.matches.all().order_by('order')
    
    if not matches.exists():
        raise ValueError("Aucun match Ã  planifier pour cette catégorie.")
    ...
    # Mettre Ã  jour l'heure de fin estimée de la catégorie
...
def optimize_tatami_usage(competition_schedule):
    ...
    Returns:
        Liste des plannings de catégorie mis Ã  jour
    """
...
def detect_schedule_conflicts(competition_schedule):
    ...
    # 2. Vérifier les conflits de participants (un mÃªme participant dans deux catégories en mÃªme temps)
...
def add_minutes_to_time(time_obj, minutes):
    """
    Ajoute des minutes Ã  un objet time.
    
    Args:
        time_obj: Objet time
        minutes: Nombre de minutes Ã  ajouter
...
def reorder_schedule_by_priority(competition_schedule):
    ...
    def sort_key(cs):
        ...
        # Les catégories de jeunes passent tÃ´t
    ...
    # Mettre Ã  jour les horaires pour chaque tatami
    updated_schedules = []
