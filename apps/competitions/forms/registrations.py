...
class CompetitionRegistrationForm(forms.ModelForm):
    """
    Formulaire d'inscription Ã  une compétition.
    Gère les champs dynamiques pour les types de compétition et catégories,
    ainsi que la mise Ã  jour des informations personnelles du pratiquant.
    """
    ...
    # Ajout de champs pour les rÃ´les supplémentaires
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.practitioner:
            ...
            # Calculer l'Ã¢ge du pratiquant (si birth_date est disponible)
        ...
        # Pré-remplir les autres champs si une inscription existe déjÃ 
        ...
        if self.competition:
            # Filtrer les rÃ´les disponibles pour cet événement
            try:
            ...
            for comp_type in self.competition.competition_types.all():
                ...
                # Pré-sélectionner si déjÃ  inscrit Ã  ce type de compétition
                ...
                if categories.exists():
                    ...
                    # Si l'utilisateur est déjÃ  inscrit, pré-sélectionner sa catégorie
                    if self.instance and self.instance.pk:
    ...
    def clean(self):
        ...
        # Vérifier la cohérence des rÃ´les d'arbitrage
        ...
        # Si l'utilisateur est compétiteur, au moins un type doit Ãªtre sélectionné
        if is_competitor:
            ...
            for comp_type in self.competition.competition_types.all():
                ...
                if type_field_name in cleaned_data and cleaned_data.get(type_field_name):
                    ...
                    # Si le type est sélectionné, une catégorie doit Ãªtre choisie
        ...
        # Vérifier que l'utilisateur a sélectionné au moins un rÃ´le
        if not (is_competitor or is_technical_judge or is_combat_referee or 
            ...
            self.add_error(
                None,
                _("Vous devez sélectionner au moins un rÃ´le pour cette compétition.")
            )
    ...
    def save(self, commit=True):
        ...
        """
        # Mettre Ã  jour les informations du pratiquant
        if self.practitioner:
        ...
        # Créer ou mettre Ã  jour l'inscription
        ...
        # Assigner les rÃ´les supplémentaires
        ...
        # Définir les niveaux de qualification Ã  "pending" si juge ou arbitre
        ...
        if commit:
            ...
            # Mettre Ã  jour les catégories si le modèle a cette relation
...
class CategoryAssignmentForm(forms.Form):
    """
    Formulaire pour assigner des participants Ã  des catégories de compétition.
    """
    ...
    def save(self):
        ...
        registration = CompetitionRegistration.objects.filter(
            ...
            competition=selected_categories.first().competition  # Toutes les catégories sont de la mÃªme compétition
        ).first()
