...
class PractitionerGradeForm(forms.ModelForm):
    ...
    def __init__(self, *args, practitioner=None, **kwargs):
        ...
        # pré-remplir les disciplines déjÃ  associées au pratiquant
        if practitioner and not self.instance.pk:
            # Récupérer les disciplines déjÃ  associées
...
class UpdateMainGradeForm(forms.ModelForm):
    """Formulaire pour mettre Ã  jour le grade principal d'un pratiquant."""
...
class BulkGradeUpdateForm(forms.Form):
    """Formulaire pour mettre Ã  jour en masse les grades de plusieurs pratiquants."""
    ...
    update_main_grade = forms.BooleanField(
        label=_("Mettre également Ã  jour le grade principal"),
        required=False,
        initial=True,
