...
class CoachProfileForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Si une instance existe déjÃ , initialiser les champs personnalisés
...
class DisciplineExpertiseForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Ajouter l'option "Autre" Ã  la liste des fédérations
        ...
        # Si une instance existe déjÃ , initialiser les valeurs
        if self.instance and self.instance.pk:
...
class BaseDisciplineExpertiseFormSet(BaseInlineFormSet):
    ...
    def clean(self):
        ...
        # Vérifier qu'une mÃªme discipline n'est pas présente plusieurs fois
        disciplines = []
        for form in self.forms:
            if form.cleaned_data and not form.cleaned_data.get('DELETE', False):
                discipline = form.cleaned_data.get('discipline')
                if discipline in disciplines:
                    raise ValidationError(_('Vous ne pouvez pas ajouter la mÃªme discipline plusieurs fois.'))
...
class CoachAvailabilityForm(forms.ModelForm):
    ...
    travel_radius = forms.IntegerField(
        ...
        help_text=_("Distance maximale que vous Ãªtes prÃªt Ã  parcourir pour enseigner (en km)")
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Si une instance existe déjÃ , initialiser les champs personnalisés depuis les paramètres
        if self.instance and self.instance.pk:
    ...
    def save(self, commit=True):
        ...
        # Mettre Ã  jour les paramètres de visibilité
...
class MultiDisciplineSelectionForm(forms.Form):
    ...
    secondary_disciplines = forms.ModelMultipleChoiceField(
        ...
        required=False,
        help_text=_("Sélectionnez jusqu'Ã  9 disciplines secondaires que vous enseignez")
    )
    ...
    def clean(self):
        ...
        if primary and primary in secondaries:
            raise forms.ValidationError(
                _("La discipline principale ne peut pas Ãªtre aussi une discipline secondaire.")
            )
