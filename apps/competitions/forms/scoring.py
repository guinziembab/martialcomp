    ...
    def clean(self):
        ...
        if min_score is not None and max_score is not None:
            if min_score >= max_score:
                raise forms.ValidationError(_("Le score minimum doit Ãªtre inférieur au score maximum."))
        
        if step is not None and step <= 0:
            raise forms.ValidationError(_("Le pas de notation doit Ãªtre positif."))
...
class TechnicalPerformanceForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if competition and category:
            # Récupérer les participants déjÃ  assignés Ã  une performance
            ...
            # Si c'est une édition, ajouter le participant actuel Ã  la liste
...
class ScoringForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if performance:
            # Récupérer les juges assignés Ã  cette catégorie
            ...
            if self.fields['criterion'].queryset.exists():
                ...
                # Mettre Ã  jour le step en fonction du premier critère
    ...
    def clean(self):
        ...
        if criterion and value is not None:
            if value < criterion.min_score or value > criterion.max_score:
                raise forms.ValidationError(_("La note doit Ãªtre comprise entre {} et {}.").format(
            ...
            if criterion.step > 0:
                ...
                if remainder != 0 and abs(remainder - step_float) > 0.001:
                    raise forms.ValidationError(_("La note doit Ãªtre un multiple de {}.").format(criterion.step))
