...
class GradeForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if level is not None and level < 1:
            self.add_error('level', _('Le niveau doit Ãªtre supérieur ou égal Ã  1'))
        
        if order is not None and order < 0:
            self.add_error('order', _('L\'ordre doit Ãªtre un nombre positif ou nul'))
    ...
    def clean(self):
        ...
        if obtained_date:
            ...
            if obtained_date > timezone.now().date():
                self.add_error('obtained_date', _("La date d'obtention ne peut pas Ãªtre dans le futur"))
...
class GradeSystemForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if is_default and is_active is not None and not is_active:
            self.add_error('is_default', _('Un système par défaut doit Ãªtre actif'))
            self.add_error('is_active', _('Ce système est défini comme par défaut et doit Ãªtre actif'))
...
class GradeCategoryForm(forms.ModelForm):
    ...
    def clean_order(self):
        ...
        if order is not None and order < 0:
            raise forms.ValidationError(_('L\'ordre doit Ãªtre un nombre positif ou nul'))
        return order
...
class CategoryGradeForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if grade_min and grade_max and grade_min == grade_max:
            self.add_error('grade_max', _("Le grade maximum doit Ãªtre différent du grade minimum"))
...
class GradeImportForm(forms.Form):
    """Formulaire pour importer des systèmes de grades Ã  partir d'un JSON"""
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Mise Ã  jour sécurisée du queryset
