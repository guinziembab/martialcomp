    ...
    def clean(self):
        ...
        if expires_at_date:
            ...
            if expires_at_aware <= timezone.now():
                self.add_error('expires_at_date', _("La date d'expiration doit Ãªtre dans le futur."))
...
class PollOptionForm(forms.ModelForm):
    """
    Formulaire pour l'ajout d'options de date/heure Ã  un sondage.
    """
    class Meta:
        model = PollOption
    ...
    def clean(self):
        ...
        # Sinon, on valide que l'heure de fin est après l'heure de début
            ...
            self.add_error('end_time', _("L'heure de fin doit Ãªtre postérieure Ã  l'heure de début."))
        ...
        for form in self.forms:
            if form.cleaned_data and not form.cleaned_data.get('DELETE', False):
                ...
                if key in seen_dates:
                    form.add_error('date', _("Cette combinaison date/heure est déjÃ  proposée."))
                else:
                    seen_dates[key] = True
...
PollOptionFormSet = inlineformset_factory(
    ...
    extra=3,  # Nombre de formulaires vides Ã  afficher
    can_delete=True,
...
class PollResponseForm(forms.ModelForm):
    """
    Formulaire pour répondre Ã  une option de sondage.
    """
    class Meta:
        model = PollResponse
...
class BulkPollResponseForm(forms.Form):
    """
    Formulaire pour répondre Ã  toutes les options d'un sondage en une fois.
    """
    def __init__(self, *args, **kwargs):
    ...
    def save(self):
        ...
        for option in self.poll.options.all():
            ...
            if response_value:
                # Vérifier si une réponse existe déjÃ  pour cette option et cet utilisateur
                try:
                    response = PollResponse.objects.get(
                        option=option,
                        user=self.user
                    )
                    # Mettre Ã  jour la réponse existante
    ...
    def clean(self):
        ...
        if send_at_date:
            ...
            if cleaned_data['send_at'] <= timezone.now():
                self.add_error('send_at_date', _("La date d'envoi doit Ãªtre dans le futur."))
            ...
            if self.event and self.event.start_date:
                ...
                if cleaned_data['send_at'] >= event_datetime:
                    self.add_error('send_at_date', _("La date d'envoi doit Ãªtre antérieure Ã  la date de l'événement."))
...
class PollQuestionForm(forms.ModelForm):
    ...
    choices_text = forms.CharField(
        ...
        widget=forms.Textarea(attrs={
            'rows': 3,
            'placeholder': _("Une option par ligne (pour les questions Ã  choix multiple)")
        }),
        help_text=_("Saisissez une option par ligne pour les questions Ã  choix multiple")
    )
    ...
    def clean(self):
        ...
        # Vérifier que les options sont fournies pour les questions Ã  choix
        if question_type == 'choice' and not choices_text:
            self.add_error('choices_text', _("Vous devez fournir des options pour les questions Ã  choix multiple."))
        ...
        if choices_text:
            ...
            if question_type == 'choice' and len(choices) < 2:
                self.add_error('choices_text', _("Vous devez fournir au moins 2 options pour les questions Ã  choix multiple."))
    ...
    def clean(self):
        ...
        for form in self.forms:
            if form.cleaned_data and not form.cleaned_data.get('DELETE', False):
                ...
                if order in orders:
                    form.add_error('order', _("Plusieurs questions ont le mÃªme ordre."))
                orders.append(order)
...
PollQuestionFormSet = inlineformset_factory(
    ...
    extra=2,  # Nombre de formulaires vides Ã  afficher
    can_delete=True,
...
class PollQuestionResponseForm(forms.ModelForm):
    """
    Formulaire pour répondre Ã  une question personnalisée.
    """
    class Meta:
