...
from apps.competitions.models import Competition, Discipline, Club, CompetitionType
...
class CompetitionForm(forms.ModelForm):
    ...
    competition_types = forms.ModelMultipleChoiceField(
        queryset=CompetitionType.objects.all(),  # Load all types for validation
        ...
        required=False  # Will be validated dynamically
    )
    ...
    def clean(self):
        ...
        competition_types = cleaned_data.get('competition_types')
        
        # Debug information
        import logging
        logger = logging.getLogger(__name__)
        logger.debug(f"Form validation - Discipline: {discipline}, Competition types: {competition_types}")
        
        if discipline:
            # Vérifier si des types de compétition existent pour cette discipline
            available_types = CompetitionType.objects.filter(discipline=discipline)
            logger.debug(f"Available types for discipline {discipline}: {list(available_types.values_list('name', flat=True))}")
            
            if available_types.exists():
                # Des types sont disponibles, donc au moins un doit être sélectionné
                if not competition_types:
                    logger.debug("No competition types selected, adding error")
                    self.add_error('competition_types', _("Au moins un type de compétition doit être sélectionné."))
                else:
                    logger.debug(f"Competition types selected: {list(competition_types.values_list('name', flat=True))}")
                    # Vérifier que tous les types sélectionnés appartiennent à la discipline choisie
                    invalid_types = competition_types.exclude(discipline=discipline)
                    if invalid_types.exists():
                        logger.debug(f"Invalid types found: {list(invalid_types.values_list('name', flat=True))}")
                        self.add_error('competition_types', _("Les types sélectionnés ne correspondent pas à la discipline choisie."))
            else:
                logger.debug(f"No types available for discipline {discipline}")
            # Si aucun type n'est disponible pour cette discipline, on ne fait pas d'erreur de validation
...
class CompetitionRegistrationForm(forms.ModelForm):
    ...
    # Participation Ã  la compétition
...
class CompetitionCategoryForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['min_age'].help_text = _("Ã‚ge minimum, laisser vide si pas de limite")
        self.fields['max_age'].help_text = _("Ã‚ge maximum, laisser vide si pas de limite")
    ...
    def clean(self):
        ...
        # Vérifier que l'Ã¢ge maximum est supérieur Ã  l'Ã¢ge minimum
        min_age = cleaned_data.get('min_age')
        max_age = cleaned_data.get('max_age')
        
        if min_age is not None and max_age is not None and max_age < min_age:
            self.add_error('max_age', _("L'Ã¢ge maximum doit Ãªtre supérieur ou égal Ã  l'Ã¢ge minimum."))
        ...
        if min_weight is not None and max_weight is not None and max_weight < min_weight:
            self.add_error('max_weight', _("Le poids maximum doit Ãªtre supérieur ou égal au poids minimum."))
...
class TatamiScheduleForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if start_hour and end_hour and start_hour >= end_hour:
            raise forms.ValidationError(_("L'heure de fin doit Ãªtre postérieure Ã  l'heure de début."))
