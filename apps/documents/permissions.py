...
class IsOwnerOrReadOnly(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        # Les autorisations de lecture sont autorisées pour toute requÃªte
...
class HasDocumentPermission(permissions.BasePermission):
    """
    Permission qui vérifie si un utilisateur a accès Ã  un document spécifique.
    """
    def has_object_permission(self, request, view, obj):
        user = request.user
        ...
        if user_share:
            if request.method in permissions.SAFE_METHODS:
                ...
                # Modification autorisée pour les niveaux d'accès Ã‰criture et ContrÃ´le total
                return True
            elif user_share.access_level == 'F' and request.method == 'DELETE':
                # Suppression autorisée uniquement pour le niveau d'accès ContrÃ´le total
                return True
...
class HasFolderPermission(permissions.BasePermission):
    """
    Permission qui vérifie si un utilisateur a accès Ã  un dossier spécifique.
    """
    def has_object_permission(self, request, view, obj):
        user = request.user
        ...
        # Vérifier les accès selon le rÃ´le de l'utilisateur
        try:
