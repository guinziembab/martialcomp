...
class DocumentFolderViewSet(viewsets.ModelViewSet, OrganizationIsolationMixin):
    """API endpoint pour les dossiers de documents"""
    queryset = get_organization_queryset(DocumentFolder, self.request.user)
    ...
    def get_queryset(self):
        """Filtre les dossiers selon les permissions de l'utilisateur avec isolation organisationnelle"""
        user = self.request.user
        
        # Isolation par organisation
        base_queryset = get_organization_queryset(DocumentFolder, user)
        
        # Les administrateurs voient tous les dossiers de leur organisation
        ...
        try:
            ...
            from apps.competitions.models import FederationRole
            ...
            from apps.competitions.models import ClubRole
        ...
        # Segmentation par organisation explicite (header / contexte)
        try:
            ...
            if org is not None:
                ...
                # Dossiers contenant des documents partagés avec l'organisation
                shared_doc_ids = DocumentShare.objects.filter(
                    group_content_type=ct,
                    group_object_id=org_id
                ).values_list('document_id', flat=True)
                if shared_doc_ids:
                    queryset = queryset | base_queryset.filter(documents__id__in=shared_doc_ids)
        except Exception:
            pass
    ...
    def documents(self, request, pk=None):
        """Récupère les documents contenus dans le dossier avec isolation organisationnelle"""
        folder = self.get_object()
        # Isolation par organisation
        base_queryset = get_organization_queryset(Document, request.user)
        documents = base_queryset.filter(folder=folder)
    ...
    def children(self, request, pk=None):
        """Récupère les sous-dossiers du dossier avec isolation organisationnelle"""
        folder = self.get_object()
        # Isolation par organisation
        base_queryset = get_organization_queryset(DocumentFolder, request.user)
        children = base_queryset.filter(parent=folder)
...
class DocumentTagViewSet(viewsets.ModelViewSet, OrganizationIsolationMixin):
    """API endpoint pour les tags de documents"""
    queryset = get_organization_queryset(DocumentTag, self.request.user)
    ...
    def get_queryset(self):
        """Filtre les tags selon l'organisation de l'utilisateur"""
        return get_organization_queryset(DocumentTag, self.request.user)
    
    @action(detail=True, methods=['get'])
    def documents(self, request, pk=None):
        """Récupère les documents associés au tag avec isolation organisationnelle"""
        tag = self.get_object()
        # Isolation par organisation
        base_queryset = get_organization_queryset(Document, request.user)
        documents = base_queryset.filter(tags=tag)
        page = self.paginate_queryset(documents)
        if page is not None:
            serializer = DocumentSerializer(page, many=True, context={'request': request})
            return self.get_paginated_response(serializer.data)
...
class DocumentViewSet(viewsets.ModelViewSet, OrganizationIsolationMixin):
    """API endpoint pour les documents"""
    queryset = get_organization_queryset(Document, self.request.user)
    ...
    def get_queryset(self):
        ...
        """Filtre les documents accessibles Ã  l'utilisateur"""
        user = self.request.user
        
        # Les administrateurs voient tous les documents
        if user.is_superuser or user.is_staff:
            return get_organization_queryset(Document, self.request.user)
        ...
        try:
            # Club
            from apps.competitions.models import Club, ClubRole
            club_ids = []
            ...
            # Fédération
            from apps.competitions.models import Federation, FederationRole
            federation_ids = []
        ...
        # Segmentation explicite par organisation (header / middleware)
        try:
            from apps.finances.currency_service import _get_request_organization
            from django.contrib.contenttypes.models import ContentType
            org = _get_request_organization(self.request)
            if org is not None:
                ct = ContentType.objects.get_for_model(org.__class__)
                org_id = str(getattr(org, 'pk', getattr(org, 'id', '')))
                org_shared = DocumentShare.objects.filter(
                    group_content_type=ct,
                    group_object_id=org_id
                ).values_list('document_id', flat=True)
                queryset = queryset | Document.objects.filter(id__in=org_shared)
        except Exception:
            pass
    ...
    def perform_update(self, serializer):
        """Met Ã  jour l'utilisateur qui a modifié le document"""
...
class DocumentShareViewSet(viewsets.ModelViewSet):
    ...
    queryset = get_organization_queryset(DocumentShare, self.request.user)
    ...
    def get_queryset(self):
        ...
        """Filtre les partages accessibles Ã  l'utilisateur"""
        user = self.request.user
        
        # Les administrateurs voient tous les partages
        if user.is_superuser or user.is_staff:
            return get_organization_queryset(DocumentShare, self.request.user)
        
        # Partages créés par l'utilisateur ou partages où l'utilisateur est le destinataire
        # avec isolation organisationnelle
        user_partages = base_queryset.filter(
            Q(created_by=user) | Q(user=user)
        )
        
        return user_partages
...
class DocumentCommentViewSet(viewsets.ModelViewSet):
    ...
    queryset = get_organization_queryset(DocumentComment, self.request.user)
    ...
    def get_queryset(self):
        ...
        """Filtre les commentaires selon les documents accessibles Ã  l'utilisateur"""
        user = self.request.user
        
        # Les administrateurs voient tous les commentaires
        if user.is_superuser or user.is_staff:
            return get_organization_queryset(DocumentComment, self.request.user)
        
        # Commentaires sur les documents accessibles Ã  l'utilisateur
        # (ceci dépend de la logique d'accès aux documents)
        # avec isolation organisationnelle
        document_ids = base_queryset.filter(
            Q(created_by=user) | Q(is_public=True) | 
            Q(shares__user=user)
        ).values_list('id', flat=True).distinct()
        
        user_commentaires = base_queryset.filter(
            Q(document_id__in=document_ids) | Q(author=user)
        )
        
        return user_commentaires
...
class DocumentAccessLogViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    queryset = get_organization_queryset(DocumentAccessLog, self.request.user)
    ...
    def get_queryset(self):
        ...
        if user.is_superuser or user.is_staff:
            return get_organization_queryset(DocumentAccessLog, self.request.user)
        
        # Logs des documents créés par l'utilisateur avec isolation organisationnelle
        document_ids = base_queryset.filter(created_by=user).values_list('id', flat=True)
        user_logs = base_queryset.filter(document_id__in=document_ids)
