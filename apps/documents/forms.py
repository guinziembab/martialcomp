...
class DocumentFolderForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        # Ã‰viter les boucles de dossiers parents
        instance = getattr(self, 'instance', None)
        if instance and instance.pk and parent:
            parent_id = parent.pk
            while parent_id:
                if parent_id == instance.pk:
                    raise ValidationError(_("Un dossier ne peut pas Ãªtre son propre parent ou ancÃªtre."))
...
class DocumentUploadForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.user and not self.user.is_superuser:
            ...
            self.fields['tags'].queryset = DocumentTag.objects.all()  # Ã€ adapter selon les besoins
