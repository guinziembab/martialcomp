...
def family_access_required(view_func):
    """
    Décorateur pour vérifier l'accès Ã  une famille.
    L'utilisateur doit Ãªtre soit le responsable principal, soit un membre actif de la famille.
    """
    @wraps(view_func)
    def wrapper(request, family_id, *args, **kwargs):
        ...
        if not has_access:
            raise PermissionDenied("Vous n'avez pas accès Ã  cette famille.")
        
        # Ajouter la famille au request pour éviter une nouvelle requÃªte
...
class FamilyPermissionMixin:
    ...
    def has_family_access(self, user):
        """Vérifie l'accès de base Ã  la famille"""
        return (
