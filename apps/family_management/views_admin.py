...
def family_group_registration(request, family_id):
    """
    Vue pour les inscriptions groupées d'une famille Ã  des compétitions.
    """
...
def process_group_registration(request, family_id):
    """
    Traite une inscription groupée Ã  une compétition.
    """
    ...
    try:
        ...
        if results['success']:
            ...
            # Ajouter les détails sur les coÃ»ts
            if results['total_cost'] > 0:
                messages.info(
                    request,
                    _("CoÃ»t total: %(cost)sâ‚¬") % {'cost': results['total_cost']}
                )
        else:
            messages.error(request, _("Aucune inscription n'a pu Ãªtre effectuée"))
...
def create_family_event(request, family_id):
    ...
    try:
        ...
        return JsonResponse({
            'success': True,
            'event_id': event.id,
            'message': _("Ã‰vénement créé avec succès")
        })
