...
from apps.grades.forms import GradeCategoryForm, GradeForm
...
# Les fonctions suivantes doivent Ãªtre adaptées pour fonctionner sans GradeSystem
...
def edit_grade_category(request, category_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, _("Catégorie mise Ã  jour avec succès."))
            return redirect('grades:system_detail', discipline_id=discipline.id)
    else:
...
def add_grade(request, category_id):
    """
    Ajout d'un nouveau grade Ã  une catégorie.
    """
    # Récupérer le club de l'utilisateur
    club = request.club
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            grade.discipline = discipline  # S'assurer que le grade est associé Ã  la discipline
...
def delete_grade(request, grade_id):
    ...
    return render(request, 'grades/confirm_delete.html', {
        ...
        'message': _("ÃŠtes-vous sÃ»r de vouloir supprimer ce grade ?"),
...
def reorder_grades(request, category_id):
    ...
    if request.method == 'POST' and request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        try:
            ...
            # Mettre Ã  jour l'ordre des grades
