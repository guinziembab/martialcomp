...
"""
Vues liées Ã  la gestion des affiliations entre organisations.
"""
...
class AffiliationCreateView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    ...
    def get_form_kwargs(self):
        ...
        if organization_id:
            ...
            if affiliation_direction == 'parent':
                # Affiliation Ã  une organisation parente
    ...
    def get_context_data(self, **kwargs):
        ...
        if affiliation_direction == 'parent':
            context['title'] = _("Ajouter une affiliation Ã  une organisation parente")
            context['subtitle'] = _("Affilier {} Ã  une autre organisation").format(organization.name)
        else:
            context['title'] = _("Ajouter une organisation affiliée")
            context['subtitle'] = _("Affilier une organisation Ã  {}").format(organization.name)
...
class AffiliationUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def form_valid(self, form):
        """Traitement lorsque le formulaire est valide."""
        messages.success(self.request, _("L'affiliation a été mise Ã  jour avec succès."))
        return super().form_valid(form)
