...
class AffiliationForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if parent_org and child_org:
            # Vérifier que ce ne sont pas la mÃªme organisation
            if parent_org == child_org:
                raise forms.ValidationError(_("Une organisation ne peut pas s'affilier Ã  elle-mÃªme."))
            
            # Vérifier qu'il n'existe pas déjÃ  une affiliation inverse
            if Affiliation.objects.filter(
                ...
                raise forms.ValidationError(
                    _("Une affiliation inverse existe déjÃ . {0} est déjÃ  affiliée Ã  {1}.").format(
...
class OrganizationMemberForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Définir la date d'adhésion par défaut Ã  aujourd'hui
        ...
        if organization:
            ...
            # Filtrer les utilisateurs qui ne sont pas déjÃ  membres de cette organisation
            ...
            if self.instance.pk:
                ...
                # Pour un nouveau membre, exclure tous les utilisateurs déjÃ  membres
                self.fields['user'].queryset = User.objects.exclude(
                    id__in=existing_users
    ...
    def clean(self):
        ...
        if user and organization and not self.instance.pk:
            # Vérifier que l'utilisateur n'est pas déjÃ  membre
            if OrganizationMember.objects.filter(user=user, organization=organization).exists():
                raise forms.ValidationError(
                    _("Cet utilisateur est déjÃ  membre de cette organisation.")
                )
        
        # Si c'est le propriétaire, vérifier qu'il n'y a pas déjÃ  un propriétaire
        if role == 'owner' and not self.instance.pk:
            if OrganizationMember.objects.filter(
                ...
                raise forms.ValidationError(
                    _("Cette organisation a déjÃ  un propriétaire. Il ne peut y avoir qu'un seul propriétaire par organisation.")
                )
...
class OrganizationSearchForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for country in countries:
            if country:  # Ã‰viter les pays vides
