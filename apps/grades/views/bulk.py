...
def bulk_grade_assignment_form(request):
    """
    Affiche le formulaire pour attribuer un grade Ã  plusieurs pratiquants en une fois.
    """
    ...
    if selected_discipline:
        from apps.grades.utils_module import get_grades_for_discipline
...
def batch_update_grades(request):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            messages.success(
                request, 
                _("{} pratiquant(s) ont été mis Ã  jour avec le grade '{}'.").format(updated_count, grade.name)
            )
