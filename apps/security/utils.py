...
def generate_secure_token(length=32):
    ...
    Returns:
        Une chaÃ®ne hexadécimale représentant le token
    """
...
def hash_password(password, salt=None):
    ...
    Args:
        password: Le mot de passe Ã  hacher
        salt: Le sel Ã  utiliser (généré aléatoirement si non fourni)
...
def validate_password_strength(password):
    ...
    Args:
        password: Le mot de passe Ã  valider
        
    Returns:
        Un tuple (est_valide, message) oÃ¹ est_valide est un booléen et message est une chaÃ®ne
    """
    # Longueur minimale
    if len(password) < 8:
...
def sanitize_input(input_string):
    """
    Nettoie une chaÃ®ne d'entrée pour éviter les injections.
    
    Args:
        input_string: La chaÃ®ne Ã  nettoyer
