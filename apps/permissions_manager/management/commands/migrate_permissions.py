...
from apps.competitions.models.administrators import FederationAdministrator, ClubAdministrator

class Command(BaseCommand):
    help = 'Migre les données des administrateurs existants vers le nouveau système de rÃ´les'
    ...
    def create_default_permissions(self):
        ...
        club_permissions = [
            ...
            ('club.assign_roles', 'Attribuer des rÃ´les dans le club', 'Club'),
            ('club.manage_competitions', 'Gérer les compétitions du club', 'Club'),
            ('club.register_competition', 'Inscrire le club Ã  des compétitions', 'Club'),
        ...
        federation_permissions = [
            ...
            ('federation.assign_roles', 'Attribuer des rÃ´les dans la fédération', 'Federation'),
        ...
        participant_permissions = [
            ('participant.register_competition', "S'inscrire Ã  des compétitions", 'Participant'),
        ...
        permission_permissions = [
            ('permission.view_role', 'Voir les rÃ´les', 'Permission'),
            ('permission.add_role', 'Ajouter des rÃ´les', 'Permission'),
            ('permission.change_role', 'Modifier des rÃ´les', 'Permission'),
            ('permission.delete_role', 'Supprimer des rÃ´les', 'Permission'),
            ('permission.view_userroleassignment', 'Voir les attributions de rÃ´les', 'Permission'),
            ('permission.add_userroleassignment', 'Attribuer des rÃ´les', 'Permission'),
            ('permission.change_userroleassignment', 'Modifier des attributions de rÃ´les', 'Permission'),
            ('permission.delete_userroleassignment', 'Supprimer des attributions de rÃ´les', 'Permission'),
        ]
    ...
    def create_default_roles(self):
        """Crée les rÃ´les par défaut avec leurs permissions"""
        # Club roles
        club_roles = [
            {
                'name': 'Propriétaire de club',
                'description': 'Accès complet Ã  la gestion du club',
                'context_type': 'club',
                'is_system_role': True,
                'permissions': [
        ...
        federation_roles = [
            {
                ...
                'description': 'Accès complet Ã  la gestion de la fédération',
    ...
    def migrate_federation_administrators(self):
        ...
        for fed_admin in FederationAdministrator.objects.all():
            # Déterminer le rÃ´le en fonction du flag is_primary
            role = president_role if fed_admin.is_primary else admin_role
            
            # Créer l'affectation de rÃ´le
    ...
    def migrate_club_administrators(self):
        ...
        for club_admin in ClubAdministrator.objects.all():
            # Déterminer le rÃ´le en fonction du rÃ´le dans l'ancien système
            if club_admin.role == 'owner':
                ...
                # RÃ´le administratif par défaut
                role = admin_role
