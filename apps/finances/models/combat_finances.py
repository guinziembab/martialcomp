...
from . import AccountingCategory, FinancialAccount
from . import Transaction
from . import PaymentMethod
from . import Invoice, InvoiceItem


class CombatFee(models.Model):
    """
    Configure les frais de participation pour les compétitions de combat.
    Chaque frais est lié Ã  une compétition et définit les montants pour chaque type de participant.
    """
    FEE_TYPES = [
...
class CombatRegistrationPayment(models.Model):
    """
    Représente un paiement pour une ou plusieurs inscriptions Ã  une compétition de combat.
    ...
    team = models.ForeignKey('CombatTeam', on_delete=models.SET_NULL, null=True, blank=True, 
                            related_name='payments', verbose_name=_('Ã‰quipe'))
    
    # Informations du payeur (peut Ãªtre un participant, un club, etc.)
    ...
    def create_invoice(self):
        """Crée une facture associée Ã  ce paiement"""
        if not self.invoice:
            ...
            invoice = Invoice.objects.create(
                ...
                notes=f"Paiement pour participation Ã  la compétition: {self.competition.nom}"
            )
            
            # Ajouter les éléments de facture pour chaque type de frais
            InvoiceItem.objects.create(
                invoice=invoice,
                description=f"Inscription Ã  la compétition {self.competition.nom}",
                quantity=1,
                unit_price=self.amount,
                amount=self.amount,
            )
            
            # Mettre Ã  jour la référence
    ...
    def confirm_payment(self, payment_method=None, transaction=None):
        """Confirme le paiement et met Ã  jour le statut"""
        if self.status == 'pending':
            ...
            # Mettre Ã  jour ou créer une transaction associée
            if transaction:
    ...
    def create_transaction(self):
        ...
        if not self.transaction:
            ...
            transaction = Transaction.objects.create(
                ...
                description=f"Paiement pour inscription Ã  la compétition {self.competition.nom}",
        ...
        if self.status == 'confirmed':
            ...
            if self.transaction:
                ...
                refund_transaction = Transaction.objects.create(
                    ...
                    date=timezone.now(),
                    status='validated',
                    description=f"Remboursement pour inscription Ã  la compétition {self.competition.nom}",
