...
class CombatPool(models.Model):
    ...
    TYPE_CHOICES = [
        ('pool', _('Poule')),
        ('elimination', _('Ã‰limination directe')),
    ...
    def get_standings(self):
        ...
        """
        # Logique Ã  implémenter en fonction des règles spécifiques
...
class CombatTeam(models.Model):
    ...
    class Meta:
        app_label = 'finances'
        verbose_name = _('Ã‰quipe de combat')
        verbose_name_plural = _('Ã‰quipes de combat')
    ...
    def add_member(self, content_type, object_id):
        ...
        if self.members_count < self.max_members:
            ...
            return True
        return False  # Ã‰quipe déjÃ  complète
...
class CombatTeamMember(models.Model):
    ...
    team = models.ForeignKey(CombatTeam, on_delete=models.CASCADE, related_name='members', verbose_name=_('Ã‰quipe'))
...
class Combat(models.Model):
    ...
    # Utilisation du GenericForeignKey pour supporter Ã  la fois des participants internes et externes
    ...
    red_team = models.ForeignKey('CombatTeam', on_delete=models.SET_NULL, related_name='red_combats', 
                                null=True, blank=True, verbose_name=_('Ã‰quipe rouge'))
    white_team = models.ForeignKey('CombatTeam', on_delete=models.SET_NULL, related_name='white_combats', 
                                  null=True, blank=True, verbose_name=_('Ã‰quipe blanche'))
    ...
    # Scores finaux (calculés Ã  partir des CombatScore)
    ...
    def __str__(self):
        red_name = "Ã‰quipe Rouge" if self.red_team else "Participant Rouge"
        white_name = "Ã‰quipe Blanche" if self.white_team else "Participant Blanc"
    ...
    def save(self, *args, **kwargs):
        ...
        # Mettre Ã  jour les heures de début/fin en fonction du statut
    ...
    def update_final_scores(self):
        """Met Ã  jour les scores finaux Ã  partir des scores et pénalités enregistrés"""
        ...
        self.red_final_score = max(0, red_score - red_penalties)  # Les scores ne peuvent pas Ãªtre négatifs
        ...
        self.white_final_score = max(0, white_score - white_penalties)  # Les scores ne peuvent pas Ãªtre négatifs
...
class CombatScore(models.Model):
    ...
    Chaque instance correspond Ã  une attribution de points, avec la valeur, le temps, et l'arbitre.
    """
    SCORE_TYPES = [
        ('quarter', _('Â¼ point (PhÃ¢n Tu Ãiá»ƒm)')),
        ('half', _('Â½ point (Ná»­a Ãiá»ƒm)')),
        ('one', _('1 point (Má»™t Ãiá»ƒm)')),
        ('one_half', _('1.5 points (Má»™t Ãiá»ƒm DÆ°á»›i)')),
        ('two', _('2 points (Hai Ãiá»ƒm)')),
    ...
    participant_side = models.CharField(max_length=10, choices=PARTICIPANT_SIDES, verbose_name=_('CÃ´té du participant'))
    ...
    def save(self, *args, **kwargs):
        ...
        # Mettre Ã  jour les scores finaux du combat
...
class CombatRound(models.Model):
    ...
    # Scores Ã  la fin du round
    red_score = models.DecimalField(max_digits=5, decimal_places=2, default=0, verbose_name=_('Score Rouge Ã  la fin du round'))
    white_score = models.DecimalField(max_digits=5, decimal_places=2, default=0, verbose_name=_('Score Blanc Ã  la fin du round'))
    ...
    def end_round(self):
        ...
        if self.status in ['in_progress', 'paused']:
            ...
            # Calculer les scores Ã  la fin du round
    ...
    def update_round_scores(self):
        """Met Ã  jour les scores pour ce round spécifique"""
        ...
        # Mettre Ã  jour les scores du round
...
class CombatPenalty(models.Model):
    ...
    """
    PENALTY_TYPES = [
        ('canh_cao', _('Cáº£nh CÃ¡o (Avertissement verbal)')),
        ('kem_diem', _('Kém Ãiá»ƒm (Retrait de Â½ point)')),
        ('phat', _('Pháº¡t (Retrait de 1 point)')),
        ('phat_hai', _('Pháº¡t Hai (Retrait de 1 point + Disqualification)')),
        ('loai', _('Loáº¡i (Disqualification immédiate)')),
    ...
    def save(self, *args, **kwargs):
        ...
        if (self.penalty_type == 'phat_hai' or 
            ...
            # Définir le résultat du combat comme disqualification du cÃ´té approprié
