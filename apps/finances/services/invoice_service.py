...
class InvoiceService:
    ...
    def mark_invoice_as_sent(invoice_id, user, sent_date=None):
        ...
        Returns:
            Invoice: L'objet facture mis Ã  jour
        ...
        if invoice.status not in ['draft']:
            raise ValidationError(_('Seules les factures en brouillon peuvent Ãªtre marquées comme envoyées'))
    ...
    def mark_invoice_as_paid(invoice_id, payment_data, user):
        ...
        Returns:
            tuple: (Invoice, PaymentAttempt) Les objets mis Ã  jour
            
        Raises:
            ValidationError: Si la facture ne peut pas Ãªtre marquée comme payée
        """
        try:
            invoice = Invoice.objects.get(pk=invoice_id)
        except Invoice.DoesNotExist:
        ...
        if invoice.status not in ['unpaid', 'sent']:
            raise ValidationError(_('Seules les factures impayées peuvent Ãªtre marquées comme payées'))
    ...
    def cancel_invoice(invoice_id, reason, user):
        ...
        Raises:
            ValidationError: Si la facture ne peut pas Ãªtre annulée
        """
        try:
            invoice = Invoice.objects.get(pk=invoice_id)
        except Invoice.DoesNotExist:
        ...
        if invoice.status == 'paid':
            raise ValidationError(_('Impossible d\'annuler une facture déjÃ  payée'))
        
        if invoice.status == 'cancelled':
            raise ValidationError(_('Cette facture est déjÃ  annulée'))
    ...
    def generate_credit_note(invoice_id, reason, user):
        """
        Génère un avoir Ã  partir d'une facture existante.
