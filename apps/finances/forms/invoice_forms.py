...
from apps.finances.models import Invoice, InvoiceItem, AccountingCategory
...
class InvoiceItemFormSet(forms.BaseInlineFormSet):
    ...
    def clean(self):
        """
        Validation du formset: au moins un élément doit Ãªtre présent.
        """
        super().clean()
...
class InvoiceForm(forms.ModelForm):
    """
    Formulaire pour créer ou mettre Ã  jour une facture.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Mettre Ã  jour les choix pour les émetteurs et destinataires
        ...
        # Si c'est une mise Ã  jour, initialiser les champs
        if self.instance.pk:
            try:
                # Vérifier si issuer_content_type existe avant d'y accéder
                if hasattr(self.instance, 'issuer_content_type_id') and self.instance.issuer_content_type_id:
                ...
                # Si on ne peut pas accéder Ã  issuer_content_type, ignorer silencieusement
                pass
            
            try:
                # Vérifier si recipient_content_type existe avant d'y accéder
                if hasattr(self.instance, 'recipient_content_type_id') and self.instance.recipient_content_type_id:
                ...
                # Si on ne peut pas accéder Ã  recipient_content_type, ignorer silencieusement
                pass
        ...
        if not self.instance.pk:
            ...
            # Termes par défaut
            self.fields['terms'].initial = _("""Paiement Ã  réception de facture.
Tout retard de paiement entraÃ®nera des pénalités de 3% du montant total.
Merci de votre confiance.""")
    ...
    def clean(self):
        ...
        # Valider que la date d'échéance est après ou égale Ã  la date d'émission
        if issued_date and due_date and due_date < issued_date:
            self.add_error('due_date', _("La date d'échéance doit Ãªtre postérieure ou égale Ã  la date d'émission."))
    ...
    def clean(self):
        ...
        if period == 'custom':
            ...
            if not date_to:
                ...
                self.add_error('date_to', _('La date de fin doit Ãªtre postérieure Ã  la date de début.'))
...
class InvoicePaymentForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Ajouter des informations sur les frais (seront mises Ã  jour via JavaScript)
