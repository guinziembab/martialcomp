...
DEFAULT_GRADES = {
    ...
    'Qwan Ki Do': {
        ...
        'Cap Bleu': [
            ...
            {"name": "Écharpe Bleue", "color": "#2196F3", "level": 17},
        ],
        'Dang': [
...
# Grades à utiliser en cas d'erreur critique
FALLBACK_GRADES = [
...
def get_grades_by_disciplines(request):
    """
    Endpoint API pour récupérer les grades correspondant à une ou plusieurs disciplines.
    Accepte à la fois les requêtes GET et POST.
...
def create_grade_for_discipline(request):
    ...
    Args:
        request: Requête HTTP avec un corps JSON contenant les informations du grade
    ...
    try:
        ...
        with transaction.atomic():
            ...
            if existing_grade:
                # Si le grade existe déjà, le retourner
...
def search_grades(request):
    ...
    Args:
        request: Requête HTTP avec des paramètres de recherche
...
def categories_by_discipline(request):
    ...
    Args:
        request: Requête HTTP avec paramètre discipline_id
