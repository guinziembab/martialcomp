...
class GradeListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['categories'] = GradeCategory.objects.all()
...
class GradeUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(self.request, _("Le grade a été mis à jour avec succès."))
        return super().form_valid(form)
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if PractitionerGrade.objects.filter(grade=grade).exists():
            messages.error(request, _(
                "Ce grade ne peut pas être supprimé car il est attribué à des pratiquants. "
                "Veuillez le désactiver plutôt que de le supprimer."
            ))
...
class GradeCategoryUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(self.request, _("La catégorie de grade a été mise à jour avec succès."))
        return super().form_valid(form)
...
class GradeCategoryDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if Grade.objects.filter(category=category).exists():
            messages.error(request, _(
                "Cette catégorie ne peut pas être supprimée car elle est utilisée par des grades. "
                "Veuillez la désactiver plutôt que de la supprimer."
            ))
...
def add_practitioner_grade(request, practitioner_id):
    """Ajoute un grade à un pratiquant."""
    # Récupérer le club de l'utilisateur
    club = request.club
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Si le grade actuel est défini, mettre à jour les autres grades dans la même discipline
            if grade.is_current:
    ...
    context = {
        ...
        'title': _("Ajouter un grade à {}").format(practitioner.full_name),
...
def edit_practitioner_grade(request, grade_id):
    """Modifie un grade attribué à un pratiquant."""
    # Récupérer le club de l'utilisateur
    club = request.club
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            updated_grade.save()
            messages.success(request, _("Le grade a été mis à jour avec succès."))
    ...
    context = {
        ...
        'submit_text': _("Mettre à jour le grade"),
    }
...
def delete_practitioner_grade(request, grade_id):
    """Supprime un grade attribué à un pratiquant."""
    # Récupérer le club de l'utilisateur
    club = request.club
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if success_count > 0:
                ...
                messages.error(
                    request,
                    _("Aucun grade n'a pu être attribué. Veuillez vérifier les critères d'éligibilité.")
                )
...
class GradeExamDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Récupérer les inscriptions à l'examen
...
class GradeExamCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if hasattr(self, 'object') and self.object and self.object.discipline:
            # Si l'objet existe déjà et a une discipline
...
class GradeExamUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        # Mettre à jour les grades disponibles
        if 'available_grades' in form.cleaned_data:
            self.object.available_grades.set(form.cleaned_data['available_grades'])
        
        messages.success(self.request, _("L'examen de passage de grade a été mis à jour avec succès."))
        return response
...
class GradeExamDeleteView(LoginRequiredMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if exam.registrations.exists():
            messages.error(request, _(
                "Cet examen ne peut pas être supprimé car il a des inscriptions. "
                "Veuillez annuler l'examen plutôt que de le supprimer."
            ))
...
def register_for_exam(request, exam_id):
    """Inscrit un pratiquant à un examen de passage de grade."""
    # Récupérer le club de l'utilisateur
    club = request.club
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, _("L'inscription à l'examen a été enregistrée avec succès."))
            return redirect('grades:exam_detail', pk=exam.pk)
    else:
    ...
    context = {
        ...
        'exam': exam,
        'club': club,
        'title': _("Inscription à l'examen de passage de grade"),
        'submit_text': _("S'inscrire à l'examen"),
    }
...
def update_exam_registration_status(request, registration_id):
    """Met à jour le statut d'une inscription à un examen."""
    # Récupérer l'inscription
    registration = get_object_or_404(GradeExamRegistration, pk=registration_id)
    
    # Vérifier les autorisations (à adapter selon votre logique d'autorisation)
    ...
    messages.success(request, _("Le statut de l'inscription a été mis à jour avec succès."))
    return redirect('grades:exam_detail', pk=registration.exam.pk)
...
def cancel_exam_registration(request, registration_id):
    """Annule une inscription à un examen."""
    # Récupérer l'inscription
    registration = get_object_or_404(GradeExamRegistration, pk=registration_id)
    ...
    messages.success(request, _("L'inscription à l'examen a été annulée avec succès."))
    return redirect('grades:exam_detail', pk=registration.exam.pk)
    ...
    def get_form(self, form_class=None):
        ...
        if discipline_id:
            try:
                ...
                # Définir des attributs data sur le champ pour le débogage côté client
            ...
            # pour forcer l'utilisateur à choisir une discipline d'abord
...
class GradeRequirementUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        messages.success(self.request, _("L'exigence de grade a été mise à jour avec succès."))
        return super().form_valid(form)
...
# ========== Vues pour l'API et les requêtes AJAX ==========
