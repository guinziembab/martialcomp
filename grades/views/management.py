...
from competitions.models import Practitioner, Discipline, Club
# Correction: Remplacement de GradeSystem par GradeCategory
from grades.models import Grade, GradeCategory, PractitionerGrade
from grades.utils import get_user_club, get_grades_for_discipline
from django.views.decorators.http import require_POST
# Importation du décorateur club_required
...
def update_practitioner_grade(request, practitioner_id):
    """
    Permet de mettre à jour le grade d'un pratiquant.
    """
    # Import nécessaire
    from django.contrib.auth import get_user_model
    User = get_user_model()
    ...
    try:
        # Essai avec les rôles spécifiques
        ...
        certifiers = list(User.objects.all())
    ...
    if request.method == 'POST':
        ...
        try:
            with transaction.atomic():
                ...
                # Mettre à jour le grade principal du pratiquant
                ...
                messages.success(request, _("Le grade de {} a été mis à jour avec succès.").format(practitioner.full_name))
            ...
            error_msg = str(e)
            print(f"ERREUR lors de la mise à jour du grade: {error_msg}")
            messages.error(request, _("Une erreur est survenue lors de la mise à jour du grade: {}").format(error_msg))
...
def promote_practitioner(request, practitioner_id):
            ...
            if next_grade:
                with transaction.atomic():
                    ...
                    PractitionerGrade.objects.create(
                        ...
                        awarded_by=request.user.get_full_name() or request.user.username,  # Changé awarded_by pour stocker une chaîne
                        notes=notes
                    )
    ...
    # Si accès direct en GET, rediriger vers la page de mise à jour
    return redirect('grades:update_practitioner_grade', practitioner_id=practitioner.id)

@login_required
@club_required
def revoke_grade(request, grade_id):
    """Retire un grade attribué à un pratiquant."""
    # Récupérer le club de l'utilisateur
    club = request.club
    ...
    if request.method == 'POST':
        ...
        with transaction.atomic():
            ...
            if previous_grade:
                ...
                # Mise à jour du grade actuel du pratiquant
...
def set_current_grade(request, grade_id):
    ...
    Cette fonction désactive les autres grades actifs pour la même discipline.
    """
    # Récupérer le club de l'utilisateur
    club = get_club_for_user(request.user)
    
    if not club:
        messages.error(request, _("Vous devez être responsable de club pour effectuer cette action."))
...
def get_club_for_user(user):
    """
    Récupère le club associé à un utilisateur.
    """
...
def log_grade_action(practitioner, user, action_type, grade, notes=""):
    ...
    from grades.models import GradeActionLog
