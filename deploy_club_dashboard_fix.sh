...
# Vérifier que les corrections sont présentes dans le fichier local
if ! grep -q "^    now = timezone.now().date()" "$FILE"; then
    echo "ATTENTION: La correction 'now' n'est pas trouvée dans le fichier local"
    echo "Assurez-vous que le fichier a été corrigé avant le déploiement"
fi

if ! grep -q "^    club_organization = None" "$FILE"; then
    echo "ATTENTION: La correction 'club_organization' n'est pas trouvée dans le fichier local"
    echo "Assurez-vous que le fichier a été corrigé avant le déploiement"
fi
