# 🚀 COMMANDES EXACTES POUR DÉPLOYER LE TEMPLATE PROFESSIONNEL

## 📋 Instructions
Copiez et collez ces commandes **une par une** sur votre serveur de production.
...
## 3️⃣ Création du template avec Python (pour éviter les problèmes)
```bash
python3 << 'PYTHON_END'
...
<head>
    ...
    <style>
        :root {
            --primary: #c41e3a; --primary-dark: #a51a30; --secondary: #1a1a1a; --accent: #d4af37;
            --light: #f8f9fa; --dark: #121212; --gray: #6c757d; --white: #ffffff;
            --success: #28a745; --warning: #ffc107; --google: #4285f4; --facebook: #1877f2;
            --border-radius: 8px; --shadow: 0 4px 15px rgba(0, 0, 0, 0.1);
            --shadow-hover: 0 8px 25px rgba(0, 0, 0, 0.15); --transition: all 0.3s ease;
        }
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            font-family: 'Montserrat', sans-serif; line-height: 1.6;
            background-color: var(--dark); color: var(--light); overflow-x: hidden;
        }
        .header {
            background: linear-gradient(135deg, var(--secondary) 0%, var(--primary) 100%);
            padding: 1rem 0; position: fixed; top: 0; width: 100%; z-index: 1000;
        ...
        .logo {
            display: flex; align-items: center; gap: 1rem; font-size: 1.8rem;
        ...
        .nav-links a {
            ...
            padding: 0.5rem 1rem; border-radius: var(--border-radius); transition: var(--transition);
        }
        .nav-links a:hover { background-color: rgba(212, 175, 55, 0.2); color: var(--accent); }
        .auth-section { display: flex; align-items: center; gap: 1rem; }
        .btn {
            padding: 0.75rem 1.5rem; border-radius: var(--border-radius); text-decoration: none;
        ...
        .btn-google { background: var(--google); color: var(--white); }
        .btn-google:hover { background: #3367d6; transform: translateY(-2px); }
        .btn-facebook { background: var(--facebook); color: var(--white); }
        ...
        .hero-container {
            max-width: 1400px; margin: 0 auto; padding: 0 2rem; display: grid;
        ...
        .hero-content .subtitle { font-size: 1.3rem; color: var(--light); margin-bottom: 2rem; opacity: 0.9; }
        .hero-content .description {
            font-size: 1.1rem; color: var(--gray); margin-bottom: 3rem; line-height: 1.8;
        }
        .social-auth-section {
            background: rgba(255, 255, 255, 0.05); padding: 2rem; border-radius: var(--border-radius);
            border: 1px solid rgba(212, 175, 55, 0.2); margin-bottom: 2rem;
        }
        .social-auth-title {
            color: var(--accent); font-size: 1.2rem; font-weight: 600; margin-bottom: 1rem; text-align: center;
        }
        .social-auth-buttons { display: flex; flex-direction: column; gap: 1rem; }
        .social-btn-large {
            padding: 1rem 2rem; border-radius: var(--border-radius); text-decoration: none;
            font-weight: 600; transition: var(--transition); border: none; cursor: pointer;
            display: flex; align-items: center; justify-content: center; gap: 1rem; font-size: 1rem;
        }
        .hero-stats { display: grid; grid-template-columns: repeat(3, 1fr); gap: 2rem; margin-top: 2rem; }
        .stat-item {
            text-align: center; padding: 1.5rem; background: rgba(255, 255, 255, 0.05);
            border-radius: var(--border-radius); border: 1px solid rgba(212, 175, 55, 0.2);
        }
        ...
        .success-banner {
            background: linear-gradient(45deg, var(--success), #20c997); color: white;
            padding: 1.5rem; border-radius: var(--border-radius); margin-bottom: 2rem; text-align: center;
        }
        .success-banner h3 { font-size: 1.2rem; margin-bottom: 0.5rem; }
        .test-phase-banner {
            background: linear-gradient(135deg, var(--warning), var(--accent)); color: var(--dark);
            text-align: center; padding: 1.5rem; font-weight: 600; position: relative; overflow: hidden;
        }
        ...
        .footer {
            background: var(--dark); padding: 4rem 0 2rem; border-top: 2px solid var(--accent);
        }
        .footer-links { text-align: center; margin-bottom: 2rem; }
        .footer-links a {
            color: var(--gray); text-decoration: none; transition: var(--transition); margin: 0 1rem;
        }
        ...
        @media (max-width: 768px) {
            ...
            .auth-section { flex-direction: column; gap: 0.5rem; }
            .social-auth-buttons { gap: 0.5rem; }
        }
...
<body>
    ...
    <header class="header">
        <div class="nav-container">
            <a href="#" class="logo">
                <div class="logo-icon"><i class="fas fa-fist-raised"></i></div>
    ...
    <section class="hero">
        <div class="hero-container">
            <div class="hero-content fade-in">
                ...
                <p class="subtitle">Simplicité • Professionnalisme • Innovation</p>
                <p class="description">
                    MartialComp révolutionne l'organisation des compétitions d'arts martiaux avec une plateforme 
                    tout-en-un qui simplifie la gestion des événements, des inscriptions, de la notation et des 
                    résultats pour tous les acteurs du milieu.
                </p>
...
print("✅ Template professionnel déployé avec succès !")
PYTHON_END
```

---

## 4️⃣ Redémarrage de Django
```bash
pkill -f "runserver 127.0.0.1:8000" 2>/dev/null || true
sleep 5
source venv/bin/activate
python manage.py check
nohup python manage.py runserver 127.0.0.1:8000 > /tmp/django_new_template.log 2>&1 &
echo "✅ Django redémarré avec le nouveau template"
```
...
if [[ "$django_code" == "200" && "$nginx_code" == "200" ]]; then
    ...
    tail -10 /tmp/django_new_template.log
fi
```
...
- **Header fixe** avec logo MartialComp et boutons Google/Facebook
- **Bannière jaune animée** en haut (phase de test)
- **Section hero** avec titre gradient et description
- **Section authentification sociale** mise en évidence
- **Statistiques** (50+ compétitions, 500+ pratiquants, 16 langues)
- **Design sombre moderne** avec animations
- **Footer** avec liens vers pages légales

## 🎯 URLs à tester après déploiement
- `https://martialcomp.com/` - Page d'accueil professionnelle
- `https://martialcomp.com/accounts/google/login/` - Authentification Google
- `https://martialcomp.com/accounts/facebook/login/` - Authentification Facebook

**Le nouveau template transformera complètement l'apparence de MartialComp !** 🚀