#!/usr/bin/env python3
"""
Script d'audit d'isolation simplifie pour la plateforme MartialComp
Verifie l'isolation des donnees dans les vues Django
"""
...
class IsolationAuditor:
    """Auditeur d'isolation des donnees dans les vues"""
    ...
    def scan_views_directory(self, directory='.'):
        """Scanne un repertoire pour trouver les vues Django"""
        views_files = []
        
        for root, dirs, files in os.walk(directory):
            # Ignorer les repertoires non pertinents
    ...
    def analyze_file(self, file_path):
        """Analyse un fichier pour detecter les problemes d'isolation"""
        try:
            ...
            if has_isolation:
                ...
                if issues:
                    ...
                    self.report['warnings'].append({
                        'file': file_path,
                        'message': 'Aucune isolation detectee'
                    })
    ...
    def analyze_view_class(self, view_content, file_path):
        """Analyse une classe de vue specifique"""
        issues = []
        
        # Verifier les methodes de requete
        ...
        for method in query_methods:
            ...
            if re.search(method_pattern, view_content):
                # Verifier si la methode a une isolation
                if not any(pattern in view_content for pattern in self.isolation_patterns):
                    issues.append(f"Methode {method} sans isolation detectee")
        
        # Verifier les querysets directs
        for model in self.organization_models:
            pattern = rf'{model}\.objects\.all\(\)'
            if re.search(pattern, view_content):
                issues.append(f"Queryset {model}.objects.all() sans filtrage detecte")
        
        return issues
    
    def find_dangerous_queries(self, content, file_path):
        """Trouve les requetes dangereuses sans isolation"""
        issues = []
        ...
        for pattern in self.dangerous_patterns:
            ...
            for match in matches:
                ...
                if not self.is_in_comment_or_docstring(content, match.start()):
                    # Verifier si c'est dans un contexte d'organisation
                    if any(org_model in context for org_model in self.organization_models):
                        issues.append(f"Requete dangereuse: {match.group()} dans {file_path}")
        
        return issues
    
    def is_in_comment_or_docstring(self, content, position):
        """Verifie si une position est dans un commentaire ou docstring"""
    ...
    def generate_recommendations(self):
        ...
        if isolation_rate < 80:
            self.report['recommendations'].append({
                'priority': 'critical',
                'action': f'Ameliorer l\'isolation des vues (taux actuel: {isolation_rate:.1f}%)',
                'impact': 'Securite des donnees'
            })
        
        if self.report['critical_issues']:
            self.report['recommendations'].append({
                'priority': 'critical',
                'action': f'Corriger {len(self.report["critical_issues"])} problemes d\'isolation critiques',
                'impact': 'Securite et conformite'
            })
        
        if non_isolated_views > 0:
            self.report['recommendations'].append({
                'priority': 'high',
                'action': f'Ajouter l\'isolation a {non_isolated_views} vues',
    ...
    def print_report(self):
        ...
        print(f"\nSTATISTIQUES:")
        print(f"   Vues analysees: {self.report['views_analyzed']}")
        ...
        if self.report['critical_issues']:
            print(f"\nPROBLEMES CRITIQUES ({len(self.report['critical_issues'])}):")
    ...
    def run_audit(self, directory='.'):
        ...
        print(f"{len(views_files)} fichiers de vues trouves")
...
def main():
    ...
    print(f"\nRapport sauvegarde dans: audit_isolation_report.json")
