...
class RateLimitingMiddleware(MiddlewareMixin):
    ...
    MAX_REQUESTS = getattr(settings, 'RATE_LIMIT_MAX_REQUESTS', 100)  # 100 requêtes par minute par défaut
    
    # Nombre maximum de requêtes pour les endpoints sensibles
    SENSITIVE_MAX_REQUESTS = getattr(settings, 'RATE_LIMIT_SENSITIVE_MAX_REQUESTS', 10)  # 10 requêtes par minute pour les endpoints sensibles
    ...
    def _clean_old_requests(self, client_ip):
        ...
        if client_ip in self.request_history:
            ...
            # Conserver uniquement les requêtes dans la fenêtre de temps
