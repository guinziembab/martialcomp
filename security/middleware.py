...
class SecurityMiddleware(MiddlewareMixin):
    ...
    Implémente diverses vérifications de sécurité sur les requêtes entrantes.
    """
    
    # Liste des caractères et motifs potentiellement malveillants dans les paramètres de requête
    SUSPICIOUS_PATTERNS = [
        r'<script.*?>',             # Tags de script
        r'javascript:',             # Protocole javascript:
        r'eval\s*\(',               # Fonction eval()
        r'document\.cookie',        # Accès aux cookies
        r'onload=',                 # Événements inline
        r'onerror=',
        r'onclick=',
    ...
    def process_request(self, request):
        """
        Traite la requête entrante et effectue des vérifications de sécurité.
        
        Args:
            request: L'objet HttpRequest à vérifier
            
        Returns:
            HttpResponseForbidden si une menace est détectée, None sinon
        """
        # Vérification des paramètres de requête (GET et POST)
        if self._check_request_params(request):
            return HttpResponseForbidden("Requête bloquée pour des raisons de sécurité.")
            
        # Vérification de l'entête User-Agent
        if self._check_user_agent(request):
            return HttpResponseForbidden("Requête bloquée pour des raisons de sécurité.")
        
        # Vérification de l'entête Referer pour les attaques CSRF
    ...
    def process_response(self, request, response):
        """
        Ajoute des entêtes de sécurité supplémentaires à la réponse.
        
        Args:
            request: L'objet HttpRequest
            response: L'objet HttpResponse à modifier
            
        Returns:
            HttpResponse modifié avec des entêtes de sécurité supplémentaires
        """
        # Ajout des entêtes de sécurité standards si pas déjà présents
    ...
    def _check_request_params(self, request):
        """
        Vérifie les paramètres de requête pour détecter des motifs suspects.
    ...
    def _check_user_agent(self, request):
        """
        Vérifie l'entête User-Agent pour détecter des outils automatisés connus.
    ...
    def _check_referer(self, request):
        """
        Vérifie l'entête Referer pour les requêtes POST.
        ...
        if request.method == 'POST' and not settings.DEBUG:
            ...
            # Si la requête POST a un Referer qui ne provient pas du site lui-même
            ...
            if referer and host and not referer.startswith(f'http://{host}') and not referer.startswith(f'https://{host}'):
                # Exception pour les chemins d'API qui peuvent être appelés par des services externes
    ...
    def _check_organization_access(self, request):
        """
        Vérifie que l'utilisateur n'accède pas à des ressources d'autres organisations.
        ...
        # Vérifier si l'URL correspond à un modèle connu avec isolation organisationnelle
        ...
        # Vérifier si l'URL correspond à un modèle sensible
        detected_model = None
        ...
        # À l'avenir, on pourrait implémenter un blocage actif ici
