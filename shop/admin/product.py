...
from shop.models import Product, ProductImage, ProductVariation, ProductAttributeValue
...
class ProductAdmin(admin.ModelAdmin):
    ...
    def stock_status(self, obj):
        if not obj.is_in_stock:
            return format_html('<span style="color: red">⨯ {}</span>', _("Épuisé"))
        elif obj.is_low_stock:
            return format_html('<span style="color: orange">⚠ {}</span>', _("Stock bas"))
        else:
            return format_html('<span style="color: green">✓ {}</span>', _("En stock"))
