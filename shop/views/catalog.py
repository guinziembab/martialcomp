...
from shop.models import Product, Category, Brand, ProductReview
...
def product_list(request):
    ...
    if filter_form.is_valid():
        ...
        # Filtrer par marque
        brand_slug = filter_form.cleaned_data.get('brand')
        if brand_slug:
    ...
    if page_number == 1:  # Calculer les facettes uniquement sur la première page pour éviter les requêtes inutiles
        ...
        brands = Brand.objects.filter(products__in=products).annotate(
            product_count=Count('products')
        ).distinct().order_by('name')
...
def category_detail(request, slug):
    ...
    if filter_form.is_valid():
        # Appliquer les mêmes filtres que dans product_list
...
def product_detail(request, slug):
    ...
    # Produits associés (même catégorie ou même marque)
    ...
    context = {
        ...
        'reviews': reviews[:5],  # Limiter à 5 avis initiaux
...
def brand_list(request):
    ...
    # Obtenir le nombre de produits par marque
    brands = brands.annotate(product_count=Count('products', filter=Q(products__is_active=True)))
    
    # Filtrer les marques sans produits actifs si nécessaire
    show_empty = request.GET.get('show_empty') == '1'
    if not show_empty:
        brands = brands.filter(product_count__gt=0)
    
    # Mettre en avant les marques premium
    featured_brands = brands.filter(is_premium=True, is_featured=True)[:8]
...
def new_arrivals(request):
    ...
    # Pagination et contexte similaires à product_list
    
    context = {
        'page_title': _('Nouveautés'),
        'page_description': _('Découvrez nos derniers produits'),
        # Autres éléments de contexte similaires à product_list
    }
...
def featured_products(request):
    ...
    context = {
        'page_title': _('Produits à la une'),
...
def discipline_products(request, discipline_id):
    """
    Affiche les produits associés à une discipline spécifique.
    """
    discipline = get_object_or_404(Discipline, id=discipline_id)
    
    # Récupérer les produits liés à cette discipline
    products = Product.objects.filter(
    ...
    context = {
        ...
        'page_title': f"{_('Équipements pour')} {discipline.name}",
        'page_description': f"{_('Tous les produits adaptés à la pratique du')} {discipline.name}",
        # Autres éléments de contexte
    }
...
def filter_options(request):
    ...
    # Cette fonction renvoie les options de filtre disponibles en fonction des filtres déjà appliqués
