...
class FederationCreationForm(forms.ModelForm):
    ...
    class Meta:
        ...
        }tact_phone': forms.TextInput(attrs={'class': 'form-control', 'placeholder': _('Téléphone')})
        }
    ...
    def clean_logo(self):
        ...
        if logo:
            if logo.size > 2 * 1024 * 1024:  # 2 MB en octets
                raise fo    def clean_country(self):
        """Validation du champ pays."""
        country = self.cleaned_data.get('country')
        if not country:
            raise forms.ValidationError(_("Veuillez sélectionner un pays."))
        
        # Validation simplifiée - accepter tout nom de pays non vide
        if len(country.strip()) < 2:
            raise forms.ValidationError(_("Le nom du pays doit contenir au moins 2 caractères."))
        
        return country.strip()     if country not in valid_countries:
    ...
    def clean_website(self):
        ...
        if website and not (    def save(self, commit=True):
        """Surcharge save pour gérer la génération de sous-domaines."""
        federation = super().save(commit=False)
        
        # Les champs address, city, postal_code, founding_date sont maintenant dans Meta.fields
        # donc ils seront automatiquement sauvegardés par super().save()
