...
def restore_practitioners():
    ...
    if claudiu_practitioner:
        ...
        # Create organization membership for CHUATAO
        print("\nCreating organization membership...")
        org_role = OrganizationRole.objects.filter(name='manager').first()
        if not org_role:
            # Create manager role if it doesn't exist
            org_role = OrganizationRole.objects.create(
                name='manager',
                display_name='Manager',
                permissions=['view_practitioners', 'edit_practitioners', 'manage_practitioners']
            )
        ...
        if existing_membership:
            ...
            print(f"Created membership with role: {org_role.name}")
    ...
    print(f"3. To recover the 4 missing practitioners, you may need to:")
    print("   - Check backup files for the original practitioner data")
    print("   - Re-create them manually with the correct information")
    print("   - Import them from a data export if available")
