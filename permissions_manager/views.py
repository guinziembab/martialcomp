...
def role_list(request):
    ...
    roles = Role.objects.all().order_by('context_type', 'name')
...
def role_detail(request, pk):
    """Détails d'un rôle et de ses permissions"""
...
def role_update(request, pk):
    ...
    if role.is_system_role and not request.user.is_superuser:
        messages.error(request, _("Les rôles système ne peuvent pas être modifiés."))
        return redirect('permissions_manager:role_detail', pk=role.pk)
...
def role_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        if active_assignments > 0 and not request.POST.get('confirm_with_assignments'):
            # Demander confirmation si le rôle est utilisé
            messages.warning(request, _(
                "Ce rôle est attribué à %(count)d utilisateurs. "
...
def user_role_list(request):
    """Liste des attributions de rôles aux utilisateurs"""
...
def user_role_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            assignment.save()
            messages.success(request, _("Le rôle a été attribué avec succès."))
...
def user_role_update(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, _("L'attribution de rôle a été mise à jour avec succès."))
            return redirect('permissions_manager:user_role_list')
    else:
    ...
    context = {
        ...
        'title': _("Modifier l'attribution de rôle"),
    }
    return render(request, 'permissions_manager/user_role_form.html', context)
...
def user_role_delete(request, pk):
    ...
    if request.method == 'POST':
        assignment.delete()
        messages.success(request, _("L'attribution de rôle a été supprimée avec succès."))
        return redirect('permissions_manager:user_role_list')
...
def entity_roles(request, content_type, object_id):
    """Vue des rôles attribués dans une entité spécifique"""
    ...
    # Récupérer les attributions de rôle pour cette entité
    assignments = UserRoleAssignment.objects.filter(
...
def user_roles(request, user_id):
    """Vue des rôles attribués à un utilisateur spécifique"""
    user = get_object_or_404(User, pk=user_id)
    
    # Récupérer toutes les attributions de rôle pour cet utilisateur
    assignments = UserRoleAssignment.objects.filter(
        user=user,
        is_active=True
    ).select_related('role')
