...
class FamilyNotificationService:
    ...
    def send_family_notification(self, notification_type, context=None, recipients=None, 
        ...
        """
        Envoie une notification à la famille.
        
        Args:
            notification_type (str): Type de notification
            context (dict): Contexte pour le template
            recipients (list): Liste des membres à notifier (None = tous)
        ...
        try:
            with transaction.atomic():
                ...
                logger.info(f"Notification familiale envoyée: {notification_type} à {target_members.count()} membres")
    ...
    def _send_email_notification(self, member, notification_config, context):
        ...
        logger.debug(f"Email envoyé à {member.user.email}: {subject}")
    ...
    def _send_sms_notification(self, member, notification_config, context):
        ...
        logger.debug(f"SMS envoyé à {phone}: {message[:50]}...")
    ...
    def _send_sms_via_provider(self, phone, message):
        ...
        # Exemple avec Twilio, à adapter selon les besoins
    ...
    def _log_notification(self, notification_type, context, target_members, results):
        ...
        # Créer un log de notification (peut être un modèle dédié)
        log_data = {
            'family_id': str(self.family.id),
            'notification_type': notification_type,
    ...
    def schedule_reminder(self, event, reminder_time, reminder_type='event_reminder'):
        ...
        # Programmer le rappel (à implémenter avec Celery ou équivalent)
    ...
    def _schedule_notification_task(self, notification_type, context, scheduled_time):
        """Programme une tâche de notification (avec Celery par exemple)."""
        # Implémentation avec un système de tâches asynchrones
        
        try:
            # Exemple avec Celery (à adapter)
            ...
            logger.warning("Système de tâches non disponible, notification immédiate")
    ...
    def _simple_schedule(self, notification_type, context, scheduled_time):
        ...
        # Stockage simple des tâches programmées
        # À implémenter selon les besoins
        pass
    
    def send_weekly_summary(self):
        """Envoie le résumé hebdomadaire à la famille."""
    ...
    def get_notification_preferences(self, member):
        ...
        # Peut être étendu avec un modèle dédié aux préférences
        return {
            'email': True,
...
class FamilyNotificationManager:
    ...
    def send_to_multiple_families(families, notification_type, context=None, **kwargs):
        """Envoie une notification à plusieurs familles."""
        results = []
    ...
    def send_organization_announcement(organization, message, families=None):
        """Envoie une annonce de l'organisation à ses familles."""
        if families is None:
