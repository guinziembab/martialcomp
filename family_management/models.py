...
class Family(models.Model):
    ...
    def has_upcoming_events(self):
        """Vérifie s'il y a des événements à venir pour les membres de la famille"""
...
class FamilyRole(models.Model):
    """
    Définit les rôles possibles au sein d'une famille
    """
    ROLE_CHOICES = [
        ('parent', _('Parent')),
    ...
    class Meta:
        app_label = 'family_management'
        verbose_name = _("Rôle familial")
        verbose_name_plural = _("Rôles familiaux")
...
class FamilyMember(models.Model):
    """
    Représente un membre d'une famille avec son rôle et ses permissions
    """
    ...
    def clean(self):
        ...
        if not self.practitioner and not self.user:
            raise ValidationError(_("Un membre doit être lié soit à un pratiquant soit à un utilisateur"))
...
class FamilyPaymentGroup(models.Model):
    ...
    def __str__(self):
        return f"{self.description} - {self.family.family_name} ({self.total_amount}€)"


class FamilyEvent(models.Model):
    """
    Événements spécifiques à une famille (réunions, sorties, etc.)
    """
    family = models.ForeignKey(
        Family,
        on_delete=models.CASCADE,
    ...
    class Meta:
        app_label = 'family_management'
        verbose_name = _("Événement familial")
        verbose_name_plural = _("Événements familiaux")
        ordering = ['start_date']
