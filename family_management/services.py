...
class FamilyRegistrationService:
    ...
    def register_family_to_competition(family, competition, selected_members=None, 
        ...
        """
        Inscrit plusieurs membres d'une famille à une compétition.
        
        Args:
            family: Instance de Family
            competition: Instance de Competition
            selected_members: Liste des IDs de FamilyMember à inscrire (None = tous les pratiquants)
        ...
        try:
            with transaction.atomic():
                # Récupérer les membres à inscrire
                if selected_members:
                ...
                for member in members:
                    ...
                    try:
                        ...
                        if CompetitionRegistration.objects.filter(
                            ...
                            results['errors'].append(
                                f"{practitioner.full_name}: Déjà inscrit à cette compétition"
                            )
                            continue
                ...
                if successful_registrations and total_cost > 0:
                    payment_group = FamilyPaymentGroup.objects.create(
                        family=family,
                        description=f"Inscriptions à {competition.name}",
...
class FamilyPaymentService:
    ...
    def create_family_payment_group(family, description, items=None, created_by=None):
        ...
        Args:
            ...
            items: Liste des éléments à payer avec montants
    ...
    def process_family_payment(payment_group, payment_method_id, payment_data=None):
        ...
        try:
            ...
            if result['success']:
                logger.info(f"Paiement familial réussi: {payment_group.id} - {payment_group.total_amount}€")
            else:
                logger.error(f"Échec paiement familial: {payment_group.id} - {result.get('error')}")
...
class FamilyEventService:
    ...
    def create_family_event(family, title, start_date, end_date=None, 
        ...
        logger.info(f"Événement familial créé: {event.title} pour {family.family_name}")
    ...
    def get_family_calendar_events(family, start_date, end_date):
        ...
        Returns:
            dict: Événements organisés par type
        """
        events = {
            'family_events': [],
            'competition_events': [],
        ...
        # TODO: Ajouter les événements de compétitions et d'entraînements
...
class FamilyManagementService:
    ...
    def create_family_from_practitioner(practitioner, family_name=None):
        """
        Crée une famille à partir d'un pratiquant existant.
        ...
        try:
            with transaction.atomic():
                ...
                logger.info(f"Famille créée à partir du pratiquant {practitioner.full_name}: {family.family_name}")
    ...
    def add_practitioner_to_family(family, practitioner, role='child', can_manage=False):
        """
        Ajoute un pratiquant existant à une famille.
        
        Returns:
            FamilyMember instance
        """
        if practitioner.family and practitioner.family != family:
            raise ValueError("Le pratiquant appartient déjà à une autre famille")
        
        try:
            with transaction.atomic():
                # Mettre à jour le pratiquant
                practitioner.family = family
                practitioner.family_role = role
                practitioner.save()
                
                # Créer ou mettre à jour le membre de famille
                ...
                logger.info(f"Pratiquant ajouté à la famille: {practitioner.full_name} -> {family.family_name}")
                
                return member
                
        except Exception as e:
            logger.error(f"Erreur lors de l'ajout du pratiquant {practitioner.id} à la famille {family.id}: {e}")
            raise
