...
def folder_view(request, folder_id=None):
    ...
    if folder_id:
        ...
        if not (folder.is_public or folder.owner == request.user or request.user.is_superuser):
            messages.error(request, _("Vous n'avez pas accès à ce dossier."))
            return redirect('documents:dashboard')
    
    # Sous-dossiers
    subfolders = DocumentFolder.objects.filter(
        parent=folder,
        # Permissions d'accès selon le rôle
    ).filter(
        Q(owner=request.user) | Q(is_public=True)
...
# Intégration spécifique pour les dashboards de rôles
...
def club_documents(request):
    ...
    from competitions.models import Club, Organization
