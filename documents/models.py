...
class DocumentFolder(TenantAwareModel):
    ...
    def save(self, *args, **kwargs):
        """Met à jour le chemin du dossier lors de la sauvegarde"""
        if self.parent:
        ...
        # Met à jour le chemin des sous-dossiers
...
class Document(TenantAwareModel):
    ...
    # Liaison générique pour associer un document à n'importe quel modèle
    ...
    document_type = models.CharField(_("Type de document"), max_length=50, blank=True, choices=[
        ...
        ('training', _("Document d'entraînement")),
    ...
    def save(self, *args, **kwargs):
        """Mise à jour automatique des métadonnées lors de la sauvegarde"""
        # Si c'est un nouveau document
        if self._state.adding:
            # Si le fichier existe déjà, récupérer sa taille
            if self.file:
...
class DocumentShare(TenantAwareModel):
    ...
    ACCESS_CHOICES = [
        ...
        (ACCESS_WRITE, _("Écriture")),
        (ACCESS_FULL, _("Contrôle total")),
    ]
    ...
    def __str__(self):
        ...
        return f"{self.document.title} → {shared_with} ({self.get_access_level_display()})"
...
class CompetitionDocumentMetadata(models.Model):
    ...
    TYPE_CHOICES = [
        ...
        ('results', _("Résultats")),
        ('report', _("Rapport")),
        ('certificate', _("Certificat/Diplôme")),
    ]
