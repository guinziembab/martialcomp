...
Analyse les requêtes, identifie les problèmes de performances et fournit des conseils d'optimisation.
"""
...
QUERY_THRESHOLD_MS = getattr(settings, 'QUERY_THRESHOLD_MS', 100)  # Seuil pour les requêtes SQL lentes
MAX_QUERIES_WARNING = getattr(settings, 'MAX_QUERIES_WARNING', 30)  # Nombre max de requêtes avant avertissement

# URLs liées aux événements à surveiller
EVENT_URL_PATTERNS = [
...
class EventPerformanceMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        """Initialise le suivi des performances pour cette requête."""
        if not settings.DEBUG:
            return None
        
        # Vérifier si l'URL correspond à un motif d'événement
    ...
    def process_response(self, request, response):
        """Analyse les performances après le traitement de la requête."""
        ...
        if execution_time_ms > PERFORMANCE_THRESHOLD_MS or num_queries > MAX_QUERIES_WARNING:
            ...
            if num_queries > 0:
                # Analyser uniquement les requêtes de cette requête
                ...
                for query in relevant_queries:
                    ...
                    # Détecter les requêtes similaires (potentiel N+1)
                    # Simplifier la requête pour la comparaison
                    sql = query.get('sql', '')
            ...
            if num_queries > MAX_QUERIES_WARNING:
                performance_report['optimization_suggestions'].append(
                    f"Nombre élevé de requêtes ({num_queries}). "
                    f"Considérez utiliser select_related/prefetch_related pour réduire le nombre de requêtes."
                )
            
            if n_plus_one_candidates:
                performance_report['optimization_suggestions'].append(
                    f"Potentiel problème N+1 détecté: {len(n_plus_one_candidates)} modèles de requêtes répétés. "
                    f"Utilisez prefetch_related ou select_related."
                )
            
            if slow_queries:
                performance_report['optimization_suggestions'].append(
                    f"{len(slow_queries)} requêtes lentes détectées (>100ms). "
                    f"Considérez optimiser ces requêtes avec des index ou refactoriser."
                )
            ...
            # En mode debug, ajouter les informations de performance à la réponse
            if settings.DEBUG and 'text/html' in response.get('Content-Type', ''):
                ...
                <div id="performance-report" style="position: fixed; bottom: 0; right: 0; background: #fff8e1; border: 1px solid #ffecb3; padding: 10px; z-index: 10000; max-width: 600px; max-height: 300px; overflow: auto; font-family: monospace; font-size: 12px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
                    ...
                    <button onclick="document.getElementById('performance-report').style.display='none';" style="position: absolute; top: 5px; right: 5px; background: none; border: none; cursor: pointer;">✕</button>
                </div>
                """
...
class EventCacheMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request):
        ...
        """
        # Ne pas mettre en cache les requêtes POST, les pages admin, ou les utilisateurs connectés
        if (request.method != 'GET' or
            request.path.startswith('/admin/') or
            request.user.is_authenticated):
            return None
        
        # Vérifier si l'URL correspond à un motif d'événement public
        if not any(re.match(pattern, request.path) for pattern in EVENT_URL_PATTERNS):
            return None
