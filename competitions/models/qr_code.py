...
class PractitionerQRCode(models.Model):
    ...
    def generate_offline_token(self):
        ...
        # Mettre à jour les données de token
    ...
    def generate_offline_qr_code(self):
        ...
        # Les données QR seront le token lui-même (approche simple)
...
class QRCodeScan(models.Model):
    ...
    def validate_scan(self):
        ...
        # Si c'est un scan hors-ligne déjà validé, on ne réexécute pas la validation
        ...
        if self.scan_type == 'attendance' and self.training_session:
            # Vérifier si un scan existe déjà pour ce pratiquant, cette session et cette date
            ...
            if existing_scan:
                self.is_valid = False
                self.validation_message = _("Présence déjà enregistrée pour cette session")
                return False
    ...
    def save(self, *args, **kwargs):
        # Valider automatiquement lors de la création si ce n'est pas un scan hors-ligne déjà validé
    ...
    def create_from_offline_data(cls, scan_data, qr_code):
        """
        Crée un scan à partir des données hors-ligne
        ...
        scan = cls(
            ...
            is_valid=True,  # La validation a été faite côté client
