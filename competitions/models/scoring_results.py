...
from competitions.models import Practitioner, CompetitionCategory
...
class TechnicalPerformanceResult(models.Model):
    ...
    def final_score(self):
        """Calcule le score final à partir des scores individuels des juges."""
        try:
            return self.result.final_score
        except PerformanceResult.DoesNotExist:
            # Si aucun résultat n'existe encore, calculer à la volée
    ...
    def calculate_final_score(self):
        ...
        # Déterminer si on exclut les notes extrêmes
    ...
    def _calculate_average_score(self, exclude_extreme=False):
        ...
        for criterion in criteria:
            ...
            # Calculer la moyenne pour ce critère (avec ou sans exclusion des extrêmes)
    ...
    def save_result(self):
        ...
        Crée ou met à jour l'objet PerformanceResult associé.
        """
...
class TechnicalScoreResult(models.Model):
    ...
    is_locked = models.BooleanField(
        _("Verrouillé"), 
        default=False,
        help_text=_("Si True, le score ne peut plus être modifié")
    )
    is_training_score = models.BooleanField(
        _("Score d'entraînement"), 
        default=False,
    ...
    def lock(self):
        """Verrouille le score pour empêcher toute modification ultérieure."""
        self.is_locked = True
        ...
        if self.criterion:
            if self.value < self.criterion.min_score or self.value > self.criterion.max_score:
                raise ValidationError({
                    'value': _(f"La valeur doit être comprise entre {self.criterion.min_score} et {self.criterion.max_score}")
                })
            ...
            if self.criterion.step > 0:
                ...
                if remainder > tolerance and abs(remainder - step_decimal) > tolerance:
                    raise ValidationError({
                        'value': _(f"La valeur doit être un multiple de {self.criterion.step} à partir de {self.criterion.min_score}")
                    })
...
class PerformanceResult(models.Model):
    ...
    def correct(self, new_score, user, reason=''):
        ...
        if reason:
            self.notes = f"{self.notes}\nCorrection de {old_score} à {new_score}: {reason}"
...
class CategoryRanking(models.Model):
    ...
    def generate_ranking(self):
        """
        Génère le classement à partir des résultats.
        Crée ou met à jour les entrées RankingEntry pour chaque participant.
        """
        # Obtenir la configuration de notation pour savoir si on autorise les ex-æquo
        try:
            config = self.category.scoring_configuration
        ...
        for idx, result in enumerate(results):
            ...
            # Si le score est identique au précédent, utiliser le même rang (ex-æquo)
            if previous_score is not None and result.final_score == previous_score:
                # Si les ex-æquo sont autorisés ou si ce n'est pas pour la 3e place
                if allow_ties or current_rank != 3:
                    is_tie = True
                else:
                    # Si le score est identique mais qu'on n'autorise pas les ex-æquo pour la 3e place
                    current_rank = idx + 1
            else:
            ...
            # Mettre à jour le rang dans le résultat
            result.rank = current_rank
            result.is_tie = is_tie
            result.save(update_fields=['rank', 'is_tie'])
            
            # Mettre à jour le score précédent pour la comparaison suivante
            previous_score = result.final_score
        
        # Créer toutes les entrées en une seule requête
...
class RankingEntry(models.Model):
    ...
    is_tie = models.BooleanField(_("Ex-æquo"), default=False)
