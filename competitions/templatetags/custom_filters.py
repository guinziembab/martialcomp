...
def trans(text):
    """
    Filtre pour traduire une chaîne dans un template.
    Usage: {{ "texte"|trans }}
    """
    return _(text)
...
def get_item(dictionary, key):
    ...
    # Si la clé est un nombre sous forme de chaîne, convertir en entier
...
def divide(value, arg):
    ...
    try:
        if float(arg) == 0:  # Éviter la division par zéro
            return 0
        return float(value) / float(arg)
    except (ValueError, TypeError):
        return 0
...
def percentof(value, total):
    ...
    try:
        if float(total) == 0:  # Éviter la division par zéro
            return 0
...
def get_category_object(category_id):
    """
    Récupère un objet de catégorie à partir de son ID.
...
def format_age(age):
    """
    Formate un âge pour l'affichage
...
def has_category_for_type(registration, competition_type_id):
    ...
    try:
        from competitions.models import ParticipantCategoryRegistration
