...
from competitions.utils.decorators import federation_required
...
def competition_categories(request, competition_id):
    ...
    # Vérifier les permissions selon le rôle et la propriété de la compétition
...
def competition_list(request):
    ...
    return render(request, 'competitions/competitions/list.html', context)
...
def competition_create(request, federation_id=None):
    ...
    if role not in ['federation_admin', 'club_manager', 'admin']:
    ...
    # Récupérer le club ou la fédération associés à l'utilisateur
    club = None
    federation = None
    
    # Si un federation_id est fourni, récupérer cette fédération spécifique
    if federation_id:
        federation = get_object_or_404(Federation, id=federation_id)
        # Vérifier que l'utilisateur a le droit d'accéder à cette fédération
        ...
        # Tenter de récupérer le club de différentes façons possibles
        ...
        if not club:
            messages.error(request, _("Vous devez être associé à un club pour créer une compétition."))
            return redirect('dashboard:index')
        ...
        try:
            ...
            if not federation:
                messages.error(request, _("Vous devez être associé à une fédération pour créer une compétition."))
                return redirect('dashboard:index')
        except:
            # Si la table Federation n'existe pas
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    # Associer au club ou à la fédération selon le profil
                    ...
                    # Ces données serviront à créer les catégories après la redirection
                    ...
                    # CORRECTION ICI - Rediriger vers la page des catégories si des données ont été fournies
        ...
        # Admin fédération peut voir les compétitions de sa discipline
            ...
            try:
                ...
                if competition.discipline != federation.discipline:
                    messages.error(request, _("Vous n'avez pas accès à cette compétition."))
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    # Mise à jour du statut selon la case "publier immédiatement"
                    is_published = form.cleaned_data.get('is_published', False)
                    
                    # Ne pas changer le statut si déjà complété ou annulé
                    ...
                    messages.success(request, _("La compétition a été mise à jour avec succès."))
                    return redirect('competitions:detail', pk=competition.id)
                    
            except Exception as e:
                messages.error(request, _("Une erreur est survenue lors de la mise à jour de la compétition: {}").format(str(e)))
        else:
            # Afficher les erreurs spécifiques du formulaire
            for field, errors in form.errors.items():
                for error in errors:
...
def register_for_competition(request, competition_id):
    """
    Permet à un utilisateur de s'inscrire à une compétition.
    ...
    if competition.registration_deadline and competition.registration_deadline < now:
        messages.error(request, _("La date limite d'inscription à cette compétition est dépassée."))
        return redirect('competitions:competitions:detail', pk=competition.id)
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    # Assigner explicitement le practitioner à l'inscription
                    registration.practitioner = practitioner
                    
                    # Enregistrer les qualifications si rôles sélectionnés
                ...
                logger.error(f"Erreur d'inscription à la compétition {competition_id}: {str(e)}")
        else:
            # Afficher les erreurs spécifiques du formulaire
            for field, errors in form.errors.items():
                for error in errors:
...
def manage_competition_registrations(request, competition_id):
    """
    Gère les inscriptions à une compétition (approbation, rejet, suppression).
    ...
    # Récupération des inscriptions avec les relations nécessaires pour éviter les requêtes N+1
...
def export_competition_registrations(request, competition_id):
    """
    Exporte les inscriptions à une compétition au format CSV ou Excel.
    Permet de filtrer les inscriptions à exporter et de choisir les champs à inclure.
    """
    competition = get_object_or_404(Competition, id=competition_id)
    ...
    # Appliquer les mêmes filtres que dans la vue manage_competition_registrations
    status_filter = request.GET.get('status')
    role_filter = request.GET.get('role')
    club_filter = request.GET.get('club')
    search_query = request.GET.get('q')
    ...
    if export_format == 'excel':
        ...
        try:
            ...
            messages.warning(request, _("Module openpyxl non disponible. Export en CSV à la place."))
            export_format = 'csv'
