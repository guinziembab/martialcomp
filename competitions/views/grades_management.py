...
et de mettre à jour les grades des pratiquants.
"""
...
# Import des modèles
from competitions.models import Practitioner, Club, Discipline, CategoryGrade
try:
    # Import des nouveaux modèles de grade si disponibles
    from grades.models import Grade, GradeCategory as GradeSystem, PractitionerGrade as PractitionerGradeHistory
except ImportError:
    # Créer des classes de remplacement pour les cas où les modèles n'existent pas encore
    class Grade:
        pass
    class GradeSystem:
        pass
        ...
        try:
            ...
            if default_system:
                ...
                for grade_category in grades:
                    # Générer des grades à partir des min/max
    ...
    if request.method == 'POST':
        ...
        try:
            with transaction.atomic():
                # Mise à jour du grade principal
            ...
            logger.error(f"Erreur lors de la mise à jour du grade: {e}", exc_info=True)
            messages.error(request, _("Une erreur est survenue lors de la mise à jour du grade: {}").format(str(e)))
...
def batch_update_grades(request):
    ...
    try:
        ...
        with transaction.atomic():
            ...
            messages.success(
                request, 
                _("{} pratiquant(s) ont été mis à jour avec le grade '{}'.").format(updated_count, grade)
            )
    except Exception as e:
...
def grade_systems_management(request):
    ...
    try:
        ...
        grade_systems = GradeSystem.objects.all().order_by('discipline__name', 'name')
        ...
        try:
            grade_categories = CategoryGrade.objects.all().order_by('discipline__name', 'order', 'nom')
