...
from competitions.utils.permission_helpers import manual_check_competition_permission
...
TAEKWONDO_POINT_VALUES = {
    ...
    'kick_head': 3,      # Coup de pied à la tête
    'turning_kick_body': 4,  # Coup de pied retourné au tronc
    'turning_kick_head': 5,  # Coup de pied retourné à la tête
}
...
def interface_combat_taekwondo(request, combat_id):
    ...
    # Déterminer si un concurrent doit être disqualifié (10 Kyong-go ou 5 Gam-jeom)
...
def ajouter_action_taekwondo(request, combat_id):
    """
    API pour ajouter une action à un combat de Taekwondo.
    ...
    if request.method == 'POST':
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            try:
                ...
                # Mettre à jour et rafraîchir le combat
            ...
            messages.error(request, _("Requête non AJAX non supportée pour cette action"))
