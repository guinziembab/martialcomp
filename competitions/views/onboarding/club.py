...
def handle_club_creation(request):
    ...
    if existing_club:
        messages.info(request, _("Vous avez déjà créé un club. Vous pouvez modifier ses détails."))
        # Passage à l'étape suivante
        request.user.profile.onboarding_step = 'club_details'
        request.user.profile.save()
        request.session['onboarding_step'] = 'club_details'
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    # Mise à jour du profil utilisateur avec ce club
                    try:
...
def handle_club_details(request):
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    messages.success(request, _("Détails du club mis à jour avec succès!"))
                    
                    # Passage à l'étape suivante (catégories)
                ...
                logger.error(f"Erreur lors de la mise à jour des détails du club: {str(e)}")
    ...
    try:
        if hasattr(club, 'disciplines'):
            # Si club a l'attribut disciplines, on exclut celles déjà associées
...
# Ajoutez cette fonction à votre fichier club.py
