...
def create_user_for_practitioner(request, practitioner_id):
    ...
    if practitioner.user:
        messages.warning(request, _("Ce pratiquant possède déjà un compte utilisateur."))
        return redirect('competitions:club:practitioners')
    ...
    try:
        ...
        if hasattr(practitioner, 'create_user_account'):
            ...
            # Vérifier si l'utilisateur existe déjà et ajouter un nombre si nécessaire
...
def link_user_to_practitioner(request, practitioner_id):
    ...
    if request.method == 'POST':
        ...
        if user_id:
            try:
                ...
                messages.success(
                    request, 
                    _("Le pratiquant {} a été associé à l'utilisateur {}.").format(
    ...
    return render(request, 'competitions/club/link_user_form.html', {
        ...
        'title': _("Associer un utilisateur à {}").format(practitioner.full_name),
        'club': club,
    })
