...
from competitions.utils.decorators import club_required, federation_or_club_required
...
# La fonction get_user_club est déjà importée de notre helper
...
def select_practitioner_for_registration(request, competition_id):
    """Sélectionne un pratiquant pour l'inscrire à une compétition."""
    club = get_user_club(request)
    ...
    # Identifier les pratiquants déjà inscrits
...
def register_practitioner(request, competition_id, practitioner_id=None):
    """Inscription d'un pratiquant à une compétition."""
    ...
    if is_club_manager:
        ...
        if 'club_id' in request.GET:
            # Permettre à l'admin de fédération de sélectionner un club
            try:
                club = get_object_or_404(Club, pk=request.GET['club_id'], federation=request.federation)
            except (ValueError, Club.DoesNotExist):
                messages.error(request, _("Club non trouvé ou non affilié à votre fédération."))
...
def available_competitions(request):
    ...
    Cette vue est accessible à la fois pour les responsables de club et les administrateurs de fédération.
    ...
    if user_role == 'club_manager':
        club = get_user_club(request)
        if not club:
            messages.error(request, _("Vous devez être associé à un club pour accéder à cette page."))
            return redirect('competitions:dashboard:index')
    ...
    if user_role == 'federation_admin' and not club:
        ...
        # Récupérer toutes les compétitions à venir (pour la liste)
        now = timezone.now().date()
        competitions = Competition.objects.filter(
            end_date__gte=now,
            status='published'
    ...
    # À ce stade, nous avons un club (soit directement pour club_manager, soit sélectionné pour federation_admin)
    # Vérifier si le club a une organisation associée
    club_organization = club.organization or getattr(club, 'as_organization', None)
    ...
    # Récupérer toutes les compétitions à venir filtrées par discipline
    now = timezone.now().date()
    competitions = Competition.objects.filter(
        end_date__gte=now,
        status='published',
        discipline__in=club_disciplines  # Filtrer selon les disciplines du club
    ).order_by('start_date')
    
    # Récupérer les inscriptions existantes du club en une seule requête
...
def register_multiple_practitioners(request, competition_id):
    """Inscription de plusieurs pratiquants à une compétition."""
    # Vérifier manuellement si l'utilisateur a les permissions nécessaires
    club = get_user_club(request)
    ...
    if request.method == 'POST':
        ...
        for practitioner_id in practitioner_ids:
            try:
                ...
                club_organization = club.organization or getattr(club, 'as_organization', None)
                if not club_organization:
                    # Passer à l'itération suivante sans erreur
                    error_count += 1
                    continue
                ...
                # Créer l'inscription même si non éligible mais avec un avertissement
                if not is_eligible:
        ...
        if success_count > 0:
            messages.success(request, _("{} pratiquant(s) inscrit(s) avec succès à {}.").format(
                success_count, competition.title
            ))
        if already_registered > 0:
            messages.info(request, _("{} pratiquant(s) étaient déjà inscrits.").format(already_registered))
        if error_count > 0:
...
def competition_registration_form(request, competition_id):
    """Page d'inscription des pratiquants à une compétition avec sélection multiple"""
    # Vérifier manuellement si l'utilisateur a les permissions nécessaires
    club = get_user_club(request)
    if not club:
        messages.error(request, _("Vous devez être responsable de club pour accéder à cette page."))
    ...
    # Créer un dictionnaire des pratiquants déjà inscrits et leurs types
    ...
    if request.method == 'POST':
        ...
        messages.success(request, f"{count} pratiquant(s) inscrit(s) avec succès à {competition.title}")
        if errors > 0:
...
def club_bulk_registration(request):
    ...
    # Récupérer toutes les compétitions à venir
    now = timezone.now().date()
    competitions = Competition.objects.filter(
        start_date__gte=now,
...
def cancel_registration(request, registration_id):
    """Annule une inscription à une compétition."""
    # Vérifier manuellement si l'utilisateur a les permissions nécessaires
    club = get_user_club(request)
    ...
    try:
        # Vérifier que l'inscription appartient bien à un pratiquant du club
