...
from competitions.models import Federation, Club, Discipline, Practitioner
from competitions.forms import ClubAffiliationForm  # Créez ce formulaire si nécessaire
from competitions.utils.decorators import federation_admin_by_param_required
...
def manage_clubs(request, federation_id):
    ...
    from organizations.models import Affiliation
...
def add_club_to_federation(request, federation_id):
    """Vue pour ajouter un club à une fédération."""
    federation = get_object_or_404(Federation, id=federation_id)
    ...
    if request.method == 'POST':
        ...
        if 'existing_club' in request.POST:
            ...
            try:
                ...
                # Vérifier si le club est déjà affilié à cette fédération
                from organizations.models import Affiliation
                if Affiliation.objects.filter(
                    ...
                    messages.success(request, _("Le club {} a été affilié avec succès à votre fédération.").format(club.name))
                return redirect('competitions:federations:manage_clubs', federation_id=federation.id)
            except Club.DoesNotExist:
                messages.error(request, _("Club introuvable."))
        else:
            ...
            if form.is_valid():
                ...
                messages.success(request, _("Le club {} a été créé et affilié avec succès à votre fédération.").format(club.name))
                return redirect('competitions:federations:manage_clubs', federation_id=federation.id)
    else:
    ...
    # Récupérer les IDs des organisations déjà affiliées à cette fédération
    affiliated_org_ids = Affiliation.objects.filter(
        parent_organization=federation_org,
        is_active=True
    ).values_list('child_organization_id', flat=True)
    
    # Récupérer les clubs qui ne sont pas déjà affiliés
