...
def dashboard(request):
    ...
    try:
        ...
        if not practitioner:
            ...
            if practitioner and not practitioner.user:
                # Associer le pratiquant à l'utilisateur
    ...
    # Entraînements ce mois (simulation - à remplacer par de vraies données quand disponibles)
    ...
    # Prochains entraînements (simulation - à remplacer par de vraies données)
    next_trainings = []  # À implémenter quand le modèle Training sera disponible
    
    # Programmes actifs (simulation)
    active_programs = []  # À implémenter quand le modèle Program sera disponible
...
def update_profile(request):
    """Vue pour mettre à jour le profil du pratiquant."""
    ...
    try:
        ...
        # Mise à jour de la nationalité si fournie
        nationality = request.POST.get('nationality')
        if nationality:
            practitioner.nationality = nationality
        
        # Mise à jour du certificat médical si fourni
        ...
        # Mise à jour éventuelle du nom d'utilisateur
        user = request.user
        ...
        messages.error(request, _("Erreur lors de la mise à jour du profil: {}").format(str(e)))
...
def activities(request):
    """Vue des activités et entraînements du pratiquant."""
    try:
        practitioner = request.user.practitioners.first()
        if not practitioner:
            # Si aucun pratiquant n'existe, essayons de chercher par l'email ou créer un nouveau profil
    ...
    # Ici, on pourrait ajouter plus tard un modèle TrainingSession pour suivre les entraînements
...
def memberships(request):
    ...
    payments = []  # À implémenter avec le module finances
...
def competition_detail(request, competition_id):
    ...
    # Vérifier si le pratiquant est inscrit à cette compétition
    registration = CompetitionRegistration.objects.filter(
        practitioner=practitioner,
        competition=competition
    ...
    if not registration:
        ...
        if competition.start_date < timezone.now().date():
            registration_closed_reason = _("Les inscriptions sont fermées (compétition déjà commencée)")
    ...
    # Récupérer d'autres participants dans la même catégorie (si inscrit)
