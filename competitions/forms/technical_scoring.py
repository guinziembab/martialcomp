    ...
    def clean(self):
        ...
        if min_score is not None and max_score is not None:
            if min_score >= max_score:
                self.add_error('max_score', _("La note maximale doit être supérieure à la note minimale."))
            
            if score_step <= 0:
                self.add_error('score_step', _("Le pas de notation doit être strictement positif."))
            elif (max_score - min_score) / score_step > 100:
                self.add_error('score_step', _("Le pas est trop petit par rapport à l'amplitude des notes."))
...
# Formset pour gérer plusieurs critères à la fois
...
class JudgeAssignmentForm(forms.Form):
    """Formulaire pour assigner des juges à une catégorie de compétition"""
    ...
    def clean(self):
        ...
        if chief_judge and chief_judge in judges:
            self.add_error('chief_judge', _("Le juge en chef ne peut pas être également un juge normal."))
        
        # Vérifier qu'un juge n'est pas à la fois juge normal et juge en formation
        if judges and training_judges:
            common_judges = set(judges) & set(training_judges)
            if common_judges:
                self.add_error('judges', _("Un juge ne peut pas être à la fois juge officiel et juge en formation."))
                self.add_error('training_judges', _("Un juge ne peut pas être à la fois juge officiel et juge en formation."))
...
class StartPerformanceForm(forms.Form):
    ...
    confirm = forms.BooleanField(
        required=True,
        label=_("Je confirme que le pratiquant est prêt à commencer"),
...
class JudgeApplicationForm(forms.ModelForm):
    ...
    def clean_experience_years(self):
        ...
        if years < 0:
            raise forms.ValidationError(_("Le nombre d'années d'expérience ne peut pas être négatif."))
        return years
