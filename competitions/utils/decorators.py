...
from competitions.models import Club, Federation, FederationAdministrator

def club_required(view_func):
    """
    Décorateur qui vérifie si l'utilisateur est associé à un club.
    Le club est ajouté à l'objet request comme attribut 'club'.
    ...
    def _wrapped_view(request, *args, **kwargs):
        ...
        # Méthode 1: Vérifie si l'utilisateur a un attribut club
        ...
        # Méthode 3: Vérifie si l'utilisateur est affilié à un club via son profil
        ...
        # Ajouter le club à l'objet request au lieu de le passer comme paramètre
        request.club = club
...
def federation_required(view_func):
    """
    Décorateur qui vérifie si l'utilisateur est associé à une fédération,
    ...
    def _wrapped_view(request, *args, **kwargs):
        ...
        try:
            ...
            if profile.role == 'federation_admin':
                ...
                # Ajouter la fédération à la requête
                ...
                messages.error(request, _("Vous n'avez pas le rôle requis."))
                return redirect('competitions:dashboard:index')
...
def federation_or_club_required(view_func):
    ...
    S'il est responsable de club, ajoute club à request.
    S'il est responsable de fédération, ajoute federation à request.
    """
    @wraps(view_func)
    def _wrapped_view(request, *args, **kwargs):
        ...
        messages.error(request, _("Vous devez être responsable de club ou de fédération pour accéder à cette page."))
        return redirect('competitions:dashboard:index')
    ...
    def decorator(view_func):
        ...
        def wrapper(request, *args, **kwargs):
            ...
            if not hasattr(request.user, 'is_federation_admin') or not request.user.is_federation_admin(federation):
                raise PermissionDenied("Vous n'êtes pas administrateur de cette fédération.")
            
            # Ajouter la fédération à la requête pour éviter de la requêter à nouveau
            request.federation = federation
    ...
    def decorator(view_func):
        ...
        def wrapper(request, *args, **kwargs):
            ...
            if not hasattr(request.user, 'is_club_admin') or not request.user.is_club_admin(club):
                raise PermissionDenied("Vous n'êtes pas administrateur de ce club.")
            
            # Ajouter le club à la requête pour éviter de le requêter à nouveau
            request.club = club
...
def judge_required(view_func):
    ...
    def wrapper(request, *args, **kwargs):
        ...
        if not hasattr(request.user, 'role') or request.user.role != 'judge':
            messages.error(request, "Vous devez être juge pour accéder à cette page.")
            return redirect('competitions:dashboard')
            ...
            if federation_id:
                ...
                try:
                    ...
                    if hasattr(request.user, 'has_federation_permission') and request.user.has_federation_permission(federation, permission):
                        # Ajouter la fédération à la requête pour faciliter l'accès dans la vue
                        request.federation = federation
                        return view_func(request, *args, **kwargs)
                except Federation.DoesNotExist:
                    pass
...
def competition_management_permission_required(view_func):
    ...
    - Un gestionnaire assigné à la compétition
    """
    @wraps(view_func)
    def wrapper(request, competition_id, *args, **kwargs):
        # Import ici pour éviter l'import circulaire
        from competitions.models import Competition, CompetitionRole
...
def permission_required(permission_name):
    ...
    Args:
        permission_name: Nom de la permission à vérifier (ex: 'can_manage_practitioners')
    """
    def decorator(view_func):
        @wraps(view_func)
        def wrapper(request, *args, **kwargs):
            ...
            from competitions.models.permissions import UserClubRole
            ...
            # Vérifier si l'un des rôles a la permission requise
