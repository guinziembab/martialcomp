...
def get_competition_progress(competition):
    ...
    Args:
        competition: L'instance de Competition à analyser
        
    Returns:
        dict: Dictionnaire contenant les informations de progression
    """
    # Import ici pour éviter l'import circulaire
    from competitions.models import TechnicalPerformance, Match
...
def get_competition_statistics(competition):
    ...
    if hasattr(competition, 'technical_performances'):
        from competitions.models import TechnicalPerformance
...
def generate_match_schedule(competition, categories=None, date=None, start_time=None, end_time=None):
    ...
    for category in categories:
        ...
        # Arrêter si on a atteint la fin de la journée
...
def optimize_tatami_usage(competition, date=None, available_tatamis=None):
    ...
    de manière à minimiser le temps total de la compétition.
    ...
    # Cette estimation pourrait être basée sur le nombre de participants, le type de compétition, etc.
    category_durations = {}
    for category in categories:
    ...
    for category in sorted_categories:
        ...
        # Attribuer la catégorie à ce tatami
        tatami_assignments[category.id] = min_load_tatami
        
        # Mettre à jour la charge de ce tatami
    ...
    for category in categories:
        ...
        # Mettre à jour l'heure de début pour le prochain match sur ce tatami
...
def detect_schedule_conflicts(schedules, participants=None):
    ...
    Cette fonction identifie les situations où:
    - Un même participant est prévu dans deux catégories qui se chevauchent
    - Un même tatami est utilisé pour deux catégories qui se chevauchent
    - Un même juge est affecté à deux catégories qui se chevauchent
    ...
    for i, slot1 in enumerate(schedules):
        for j, slot2 in enumerate(schedules):
            ...
            # Si les deux créneaux sont sur le même tatami et se chevauchent
