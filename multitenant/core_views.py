...
def tenant_settings_view(request):
    ...
    if request.user != tenant.owner:
        messages.error(request, _("Vous n'êtes pas autorisé à modifier ces paramètres."))
...
def tenant_billing_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, _("Informations de facturation mises à jour."))
...
def payment_cancel_view(request):
    ...
    messages.warning(
        request,
        _("Paiement annulé. Vous pouvez réessayer à tout moment.")
    )
