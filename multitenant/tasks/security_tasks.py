"""
Tâches Celery pour les audits de sécurité multi-tenant.
"""
...
from multitenant.security import run_security_audit, SecurityAuditor
...
def run_scheduled_security_audit(tenant_id=None):
    ...
    Args:
        tenant_id: ID du tenant à auditer (None pour un audit global)
...
def check_tenant_security_compliance(tenant_id):
    ...
    try:
        ...
        # Mettre à jour le statut de sécurité du tenant
...
def update_tenant_security_status(tenant_id, compliance_results):
    """
    Met à jour le statut de sécurité d'un tenant.
    ...
    try:
        ...
        # Mettre à jour le tenant (ajouter ces champs au modèle si nécessaire)
        # tenant.security_status = security_status
        # tenant.last_security_audit = timezone.now()
        # tenant.save()
        
        logger.info(f"Statut de sécurité mis à jour pour {tenant.name}: {security_status} ({score}%)")
        ...
        logger.error(f"Erreur lors de la mise à jour du statut de sécurité: {str(e)}")
        raise
