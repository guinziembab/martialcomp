...
def subscription_dashboard(request):
    ...
    # Récupérer les fonctionnalités à l'usage
    ...
    # Calculer les coûts supplémentaires du mois en cours
...
def subscribe(request):
    """
    Traite la souscription à un nouveau plan.
    """
    tenant = getattr(request, 'tenant', None)
    if not tenant:
        messages.error(request, _("Aucun tenant trouvé pour cet utilisateur."))
    ...
    try:
        ...
        if result.get('success', False):
            if immediate:
                ...
                messages.success(request, _("Votre abonnement sera annulé à la fin de la période de facturation."))
        else:
...
def feature_usage_history(request):
    """
    Affiche l'historique d'utilisation des fonctionnalités à l'usage.
    """
    tenant = getattr(request, 'tenant', None)
    if not tenant:
        messages.error(request, _("Aucun tenant trouvé pour cet utilisateur."))
    ...
    # Récupérer les fonctionnalités à l'usage pour le filtre
