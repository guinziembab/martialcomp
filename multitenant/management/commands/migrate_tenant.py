...
from multitenant.middleware import TenantContext
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--tenant',
            type=str,
            help='Slug du tenant spécifique à migrer'
        )
        parser.add_argument(
            '--all',
            action='store_true',
        ...
        parser.add_argument(
            '--app',
            type=str,
            help='Application spécifique à migrer'
        )
        parser.add_argument(
            '--fake',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if tenant_slug and migrate_all:
            raise CommandError('Vous ne pouvez pas spécifier à la fois --tenant et --all')
        
        # Obtenir les tenants à migrer
        if tenant_slug:
            try:
        ...
        for tenant in tenants:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f'  ✓ Migration terminée pour {tenant.name}')
                )
                
            except Exception as e:
                self.stdout.write(
                    self.style.ERROR(f'  ✗ Erreur pour {tenant.name}: {str(e)}')
                )
