...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('✓ Système d\'audit de sécurité initialisé avec succès!'))
...
- `security_audit_tenant_<slug>_YYYYMMDD_HHMMSS.json` : Audit spécifique à un tenant

## Sécurité

⚠️ Ces rapports contiennent des informations sensibles. 
Assurez-vous que ce répertoire est correctement sécurisé et exclu du contrôle de version.
""")
        ...
        self.stdout.write(self.style.SUCCESS(f'  ✓ Répertoires créés: {report_dir}'))
    ...
    def check_configuration(self):
        ...
        if required_middleware not in middleware_classes:
            self.stdout.write(self.style.ERROR(
                f'  ✗ {required_middleware} non trouvé dans MIDDLEWARE'
            ))
        else:
            self.stdout.write(self.style.SUCCESS(
                f'  ✓ {required_middleware} configuré'
            ))
        ...
        if 'django_celery_beat' in settings.INSTALLED_APPS:
            self.stdout.write(self.style.SUCCESS('  ✓ Celery Beat configuré'))
        else:
            self.stdout.write(self.style.WARNING(
                '  ⚠ django_celery_beat non installé - audits planifiés non disponibles'
            ))
    ...
    def create_config_files(self):
        ...
        if not os.path.exists(policy_file):
            ...
            self.stdout.write(self.style.SUCCESS(f'  ✓ Politique de sécurité créée: {policy_file}'))
...
""")
            ...
            self.stdout.write(self.style.SUCCESS(f'  ✓ Tests personnalisés créés: {custom_tests_file}'))
    ...
    def init_permissions(self):
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING(f'  ⚠ Impossible de créer les permissions: {e}'))
        ...
        try:
            ...
            if created:
                ...
                self.stdout.write(self.style.SUCCESS('  ✓ Groupe "Security Auditors" créé'))
            else:
                self.stdout.write(self.style.SUCCESS('  ✓ Groupe "Security Auditors" existant'))
        except Exception as e:
            self.stdout.write(self.style.WARNING(f'  ⚠ Impossible de créer le groupe: {e}'))
