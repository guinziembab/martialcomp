...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=10,
            help='Nombre de clubs à migrer par batch (défaut: 10)'
        )
        parser.add_argument(
            '--start-from',
            type=int,
            default=0,
            help='Index du club à partir duquel commencer'
        )
        parser.add_argument(
            '--report-dir',
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        if total_clubs == 0:
            self.stdout.write(self.style.SUCCESS("Aucun club à migrer."))
            return
        ...
        try:
            ...
            for i in range(0, total_clubs, batch_size):
                ...
                self.stdout.write(
                    ...
                    f"({i+1} à {min(i+batch_size, total_clubs)} sur {total_clubs})"
                )
                ...
                for club in batch_clubs:
                    try:
                        ...
                        self.stdout.write(
                            self.style.SUCCESS(f"✓ {club.name} migré avec succès")
                        )
                        processed += 1
                        
                    except Exception as e:
                        error_msg = f"Échec pour {club.name} (ID: {club.id}): {str(e)}"
    ...
    def handle_rollback(self, club_id: int):
        ...
        try:
            ...
            from multitenant.migrations.migrate_existing_clubs import MigrationRollback
    ...
    def confirm_migration(self, total_clubs: int) -> bool:
        ...
        self.stdout.write(
            self.style.WARNING(
                f"\nVous êtes sur le point de migrer {total_clubs} clubs."
            )
        )
    ...
    def display_summary(self, report: dict):
        ...
        self.stdout.write(
            self.style.ERROR(f"Échoués: {summary['failed']}")
        )
