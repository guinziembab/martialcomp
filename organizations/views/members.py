"""
Vues liées à la gestion des membres des organisations.
"""
...
class OrganizationMemberCreateView(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    """Vue pour ajouter un membre à une organisation."""
    model = OrganizationMember
...
class OrganizationMemberUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def form_valid(self, form):
        """Traitement lorsque le formulaire est valide."""
        messages.success(self.request, _("Le membre a été mis à jour avec succès."))
        return super().form_valid(form)
    ...
    if member.role == OrganizationRole.OWNER:
        ...
        if OrganizationMember.objects.filter(
            ...
            messages.error(request, _(
                ...
                "Veuillez d'abord transférer la propriété à un autre membre."
            ))
...
def transfer_ownership(request, pk):
    ...
    # Vérifier que le membre choisi n'est pas déjà le propriétaire
    if member.role == OrganizationRole.OWNER:
        messages.error(request, _("Ce membre est déjà le propriétaire de l'organisation."))
        return redirect('organizations:members', organization_id=organization.id)
