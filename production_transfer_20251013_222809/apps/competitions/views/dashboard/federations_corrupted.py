...
def federation_dashboard(request, federation_id):
    ...
    # Ajouter des compétitions de démonstration si aucune n'existe
    if not upcoming_competitions.exists() and isinstance(affiliated_clubs, list):
        ...
        class DemoCompetition:
            def __init__(self, **kwargs):
                ...
                if not hasattr(self, 'registrations'):
                    self.registrations = type('MockManager', (), {'count': lambda: self.participants_count})()

        demo_competitions = [
            DemoCompetition(
                id=1, title='Championnat Régional Karaté',
                start_date=now + timedelta(days=15),
                location='Palais des Sports', participants_count=45
            ),
            DemoCompetition(
                id=2, title='Tournoi Inter-Clubs Judo',
                start_date=now + timedelta(days=28),
                location='Gymnase Municipal', participants_count=32
            ),
            DemoCompetition(
                id=3, title='Open de Taekwondo',
                start_date=now + timedelta(days=42),
                location='Centre Sportif', participants_count=58
            ),
        ]
        
        upcoming_competitions = demo_competitions
        competitions_count = len(demo_competitions)
        upcoming_events = len(demo_competitions)
    ...
    # Ajouter des données de démonstration si aucun club affilié pour améliorer l'expérience utilisateur
    if not affiliated_clubs.exists():
        # Import nécessaire
        from datetime import date
        
        # Créer une classe pour les clubs de démonstration
        class DemoClub:
            def __init__(self, **kwargs):
                for key, value in kwargs.items():
                    setattr(self, key, value)
                # Ajouter des attributs nécessaires
                if not hasattr(self, 'practitioners'):
                    self.practitioners = type('MockManager', (), {'count': lambda: 0})()
        ...
        demo_clubs = [
            DemoClub(
                id=1, name='Dojo Karaté Saint-Denis', 
                city='Saint-Denis', created_at=date(2023, 1, 15),
                pk=1
            ),
            DemoClub(
                id=2, name='Club Judo Marseille', 
                city='Marseille', created_at=date(2023, 3, 10),
                pk=2
            ),
            DemoClub(
                id=3, name='Taekwondo Lyon Centre', 
                city='Lyon', created_at=date(2023, 5, 22),
                pk=3
            ),
            DemoClub(
                id=4, name='Arts Martiaux Toulouse', 
                city='Toulouse', created_at=date(2023, 7, 8),
                pk=4
            ),
            DemoClub(
                id=5, name='Aikido Bordeaux Elite', 
                city='Bordeaux', created_at=date(2023, 9, 14),
                pk=5
            ),
        ]
        
        # Utiliser les données de démonstration
        affiliated_clubs = demo_clubs
        displayed_clubs = demo_clubs
        clubs_count = len(demo_clubs)
        
        club_registrations = {}
        try:
            if isinstance(affiliated_clubs, list):
                # Données de démonstration, pas de juges réels
                judge_count = 0
            else:
                # Compter les juges
    ...
    try:
        ...
        if hasattr(federation, 'shop_products'):
            ...
            recent_orders = get_organization_queryset(Order, request.user).order_by('-created_at')[:5]
    except Exception as e:
    ...
    try:
        ...
        recent_payments = get_organization_queryset(PaymentAttempt, request.user).order_by('-initiated_at')[:10]
    ...
    # Documents récents et statistiques (aligné sur le club)
    document_stats = {
        'my_documents': 0,
        'shared_with_me': 0,
        'total_size': 0,
    ...
    try:
        ...
        total_size = sum(getattr(doc, 'file_size', 0) or 0 for doc in user_documents)
        from datetime import timedelta
        ...
        logger.error(f"Erreur documents fédération: {str(e)}")

    # Adhésions (Membership) aligné club
    membership_stats = {
        'total_active': 0,
        'total_packages': 0,
        'expiring_soon': 0,
    ...
    try:
        ...
        federation_org = getattr(federation, 'organization', None) or getattr(federation, 'as_organization', None)
        if federation_org:
            membership_stats = MembershipService.get_membership_stats(federation_org)
            recent_subscriptions = MembershipSubscription.objects.filter(
                package__organization=federation_org
            ).select_related('practitioner', 'package').order_by('-created_at')[:10]
            membership_alerts = MembershipAlert.objects.filter(
                subscription__package__organization=federation_org,
                is_resolved=False
            ).order_by('-created_at')[:5]
            membership_stats['popular_packages'] = MembershipAnalyticsService.get_popular_packages(federation_org)
            membership_stats['revenue_trend'] = MembershipAnalyticsService.get_revenue_trend(federation_org, 6)
            membership_stats['retention_rate'] = MembershipAnalyticsService.get_retention_rate(federation_org)
    except ImportError:
        logger.warning("Module membership non disponible pour la fédération")
    except Exception as e:
        logger.error(f"Erreur adhésions fédération: {str(e)}")
    ...
    try:
        ...
        federation_org = getattr(federation, 'organization', None)
        if federation_org:
            orders_qs = Order.objects.filter(products__organization=federation_org).distinct()
            shop_stats['total_orders'] = orders_qs.count()
            shop_stats['total_revenue'] = float(orders_qs.filter(
                status__in=['delivered', 'shipped', 'processing']
            ).aggregate(total=Sum('total'))['total'] or 0)
            shop_stats['pending_orders'] = orders_qs.filter(status='pending').count()
            products_qs = Product.objects.filter(organization=federation_org, is_active=True)
            shop_stats['active_products'] = products_qs.count()
            shop_stats['low_stock_products'] = products_qs.filter(stock_quantity__lte=F('low_stock_threshold')).count()
            shop_stats['out_of_stock'] = products_qs.filter(stock_quantity=0).count()
            thirty_days_ago = timezone.now() - timezone.timedelta(days=30)
            popular_products = (
                OrderItem.objects
                .filter(order__in=orders_qs, order__created_at__gte=thirty_days_ago)
                .values('product_name')
                .annotate(total_sold=Sum('quantity'), total_revenue=Sum(F('price') * F('quantity')))
                .order_by('-total_revenue')[:5]
            )
    except Exception as e:
        logger.error(f"Erreur statistiques boutique fédération: {str(e)}")
    ...
    # Préparer les statistiques pour le template, en ajoutant la taille des compétitions à gérer
    stats = {
        ...
        'competitions_to_manage': competitions_to_manage.count() if hasattr(competitions_to_manage, 'count') else 0,
    }

    # Alias pour correspondre au template
    recent_activities = recent_activity

    # Tâches à faire (sera alimenté si dispo côté task management)
    todo_items = []
    ...
    context = {
        ...
        # Expose task_data container to match template usage
        'task_data': {},
    }
    
    # Ajouter les données de gestion de tâches au contexte si disponibles
    if task_data:
        # Garder l'objet sous la clé task_data pour le template
        context['task_data'] = task_data
        # Et aussi exposer à plat si nécessaire pour d'autres inclusions
        context.update(task_data)
        # Alimenter todo_items si fourni par l'intégration
        if isinstance(task_data, dict) and task_data.get('todo_items'):
            context['todo_items'] = task_data['todo_items']
...
def federation_settings(request, federation_id):
    ...
    # Vérifier les permissions
    has_access =    return render(request, 'competitions/federations/settings.html', context)


@login_required
@federation_admin_required
def federation_import_export(request, federation_id):
    """
    Vue pour l'import/export de données de la fédération.
    """
    federation = get_object_or_404(Federation, id=federation_id)
    ...
    context = {
        ...
        'federation': federation,
        'title': _('Import/Export - {}').format(federation.name)
    }
...
def federation_certifications(request, federation_id):
    """
    Vue pour gérer les certifications de la fédération.
    """
    federation = get_object_or_404(Federation, id=federation_id)
    
    # Récupérer les juges certifiés
    ...
    context = {
        'page_title': _('Certifications'),
        'section': 'certifications',
        'federation': federation,
        'certified_judges': certified_judges,
        'title': _('Certifications - {}').format(federation.name)
    }
...
def federation_examens(request, federation_id):
    """
    Vue pour gérer les examens de grades de la fédération.
    """
    federation = get_object_or_404(Federation, id=federation_id)
    
    context = {
        'page_title': _('Examens & Grades'),
        'section': 'examens',
        'federation': federation,
        'title': _('Examens & Grades - {}').format(federation.name)
    }
    
    return render(request, 'competitions/federations/examens.html', context)


@login_required
@federation_admin_required
def federation_calendar(request, federation_id):
    """
    Vue pour le calendrier des événements de la fédération.
    """
    federation = get_object_or_404(Federation, id=federation_id)
    
    # Récupérer les compétitions futures
    upcoming_competitions = Competition.objects.filter(
        federation=federation,
    ...
    context = {
        ...
        'federation': federation,
        'upcoming_competitions': upcoming_competitions,
        'title': _('Calendrier - {}').format(federation.name)
    }
...
def federation_create_competition(request, federation_id):
    """
    Vue pour créer une nouvelle compétition pour la fédération.
    Redirige vers le formulaire de création de compétition.
    """
    federation = get_object_or_404(Federation, id=federation_id)
    
    # Rediriger vers la création de compétition avec la fédération pré-remplie
    messages.info(request, _('Utilisez le formulaire ci-dessous pour créer une nouvelle compétition pour {}.').format(federation.name))
    return redirect('competitions:competitions:create')


n = federation.administrators.filter(user=request.user).exists()
        if is_admin:
            has_access = True
