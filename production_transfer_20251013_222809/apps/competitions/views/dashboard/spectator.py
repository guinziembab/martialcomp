...
from apps.competitions.models import Competition, Match, Discipline, Club
...
def spectator_dashboard(request, extra_context=None):
    ...
    # Si l'utilisateur a une ville définie, filtrer les clubs de la mÃªme ville
    if user_city:
    ...
    # Récupérer les données de gestion de tâches (limitées pour spectateurs)
    task_data = {}
    if TASK_MANAGEMENT_AVAILABLE:
        try:
            # Les spectateurs ont un accès limité aux tâches
            task_data = get_dashboard_task_data(request.user, limit_tasks=3, limit_boards=2)
        except Exception as e:
            # Silencieusement ignorer les erreurs pour les spectateurs
    ...
    context = {
        ...
        'disciplines': disciplines[:5],  # Limiter Ã  5 pour l'affichage initial
