#!/bin/bash
# Script pour déployer immédiatement les corrections de l'onboarding fédération

echo "================================================"
echo "🚀 DÉPLOIEMENT CORRECTIONS ONBOARDING FÉDÉRATION"
echo "================================================"
echo ""

# Créer un répertoire temporaire pour les scripts
TEMP_DIR="federation_fix_$(date +%Y%m%d_%H%M%S)"
mkdir -p $TEMP_DIR

# Copier les scripts dans le répertoire temporaire
cp diagnose_federation_onboarding_production.sh $TEMP_DIR/
cp fix_federation_onboarding_production.sh $TEMP_DIR/

echo "📦 Scripts préparés dans $TEMP_DIR"
echo ""
echo "🚀 Connexion au serveur de production..."
echo ""

# Créer une archive des scripts
tar -czf ${TEMP_DIR}.tar.gz $TEMP_DIR/

echo "📋 Instructions pour le déploiement :"
echo ""
echo "1. Transférer l'archive :"
echo "   scp ${TEMP_DIR}.tar.gz martialcomp-production:/home/martialc/"
echo ""
echo "2. Se connecter au serveur :"
echo "   ssh martialcomp-production"
echo ""
echo "3. Extraire et exécuter :"
echo "   cd /home/martialc"
echo "   tar -xzf ${TEMP_DIR}.tar.gz"
echo "   cd ${TEMP_DIR}"
echo "   chmod +x *.sh"
echo "   ./diagnose_federation_onboarding_production.sh"
echo "   sudo ./fix_federation_onboarding_production.sh"
echo ""

# Alternative : Script tout-en-un via SSH
cat > execute_federation_fix_remote.sh << 'REMOTE_EOF'
#!/bin/bash
...
echo "================================================"
echo ""
echo "Option 1 - Transfert classique :"
echo "  scp ${TEMP_DIR}.tar.gz martialcomp-production:/home/martialc/"
echo ""
echo "Option 2 - Exécution directe :"
echo "  cat execute_federation_fix_remote.sh | ssh martialcomp-production 'bash -s'"
echo ""