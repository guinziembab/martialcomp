#!/bin/bash
# Script rapide pour corriger les disciplines en production - Version 2
...
$PYTHON_CMD manage.py shell --settings=config.settings.production -c "from apps.competitions.models import Discipline; print(f'Disciplines actuelles: {Discipline.objects.count()}')"

echo ""
echo "2️⃣ Application des migrations..."
$PYTHON_CMD manage.py migrate --settings=config.settings.production --noinput

echo ""
echo "3️⃣ Chargement des disciplines..."
# Essayer la commande load_disciplines
$PYTHON_CMD manage.py load_disciplines --settings=config.settings.production 2>/dev/null || {
    echo "   ⚠️  Commande load_disciplines non trouvée, création manuelle..."
    
    # Création manuelle si la commande n'existe pas
    $PYTHON_CMD manage.py shell --settings=config.settings.production << 'EOF'
...
$PYTHON_CMD manage.py shell --settings=config.settings.production -c "from apps.competitions.models import Discipline; print(f'✅ Disciplines en base: {Discipline.objects.count()}')"
